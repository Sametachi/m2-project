...
void CLightInstance::OnSetDataPointer(CEffectElementBase * pElement)
{
	...
	CLightManager::GetInstance()->RegisterLight(LIGHT_TYPE_DYNAMIC, &m_LightID, Light);
}
...
bool CLightInstance::OnUpdate(float fElapsedTime)
{
	...
	CLight * pLight = CLightManager::GetInstance()->GetLight(m_LightID);
...
void CLightInstance::OnDestroy()
{
	if (m_LightID)
	{
		CLightManager::GetInstance()->DeleteLight(m_LightID);
	}
}
