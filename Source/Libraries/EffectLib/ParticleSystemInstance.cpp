...
void CParticleSystemInstance::CreateParticles(float fElapsedTime)
{
	...
	for (int32_t i = 0; i < iCreatingCount; ++i)
	{
		...
		// Simple Update
		{
			...
			// @fixme129 begin
			#ifdef _DEBUG
			if (m_pParticleProperty->m_TimeEventColorRed.size() <= 0)
				LogBoxf("TimeEventColorRed is empty inside the .mse file: add 4 floats inside to fix");
			if (m_pParticleProperty->m_TimeEventColorGreen.size() <= 0)
				LogBoxf("TimeEventColorGreen is empty inside the .mse file: add 4 floats inside to fix");
			if (m_pParticleProperty->m_TimeEventColorBlue.size() <= 0)
				LogBoxf("TimeEventColorBlue is empty inside the .mse file: add 4 floats inside to fix");
			if (m_pParticleProperty->m_TimeEventAlpha.size() <= 0)
				LogBoxf("TimeEventAlpha is empty inside the .mse file: add 4 floats inside to fix");
			#endif
			pInstance->m_Color.r = (m_pParticleProperty->m_TimeEventColorRed.size()) ? m_pParticleProperty->m_TimeEventColorRed.front().m_Value : 0;
			pInstance->m_Color.g = (m_pParticleProperty->m_TimeEventColorGreen.size()) ? m_pParticleProperty->m_TimeEventColorGreen.front().m_Value : 0;
			pInstance->m_Color.b = (m_pParticleProperty->m_TimeEventColorBlue.size()) ? m_pParticleProperty->m_TimeEventColorBlue.front().m_Value : 0;
			pInstance->m_Color.a = (m_pParticleProperty->m_TimeEventAlpha.size()) ? m_pParticleProperty->m_TimeEventAlpha.front().m_Value : 0;
			// @fixme129 end
#else
