			...
			case MESH_BILLBOARD_TYPE_MOVE:
				{
					...
					m_pMeshScript->GetPosition(m_fLocalTime-CTimer::GetInstance()->GetElapsedSecond(), LastPosition);
...
void CEffectMeshInstance::OnSetDataPointer(CEffectElementBase * pElement)
{
	...
	m_pEffectMesh = CResourceManager::GetInstance()->LoadResource<CEffectMesh>(c_szMeshFileName);
