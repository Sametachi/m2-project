...
BOOL CEffectMesh::__LoadData_Ver002(int32_t iSize, const uint8_t * c_pbBuf)
{
	...
	for (int16_t n = 0; n < m_iGeomCount; ++n)
	{
		...
		if ("ifl" == strExtension)
		{
			auto vfs = CallFS().Open(pMeshData->szDiffuseMapFileName, Buffered);

			if (vfs)
			{
				const uint32_t size = vfs->GetSize();
				storm::View text(storm::GetDefaultAllocator());
				vfs->GetView(0, text, size);
				...
				for (uint32_t i = 0; i < textFileLoader.GetLineCount(); ++i)
				{
					...
					CGraphicImage * pImage = CResourceManager::GetInstance()->LoadResource<CGraphicImage>(strTextureFileName.c_str());
		...
		{
			CGraphicImage * pImage = CResourceManager::GetInstance()->LoadResource<CGraphicImage>(pMeshData->szDiffuseMapFileName);
...
CEffectMesh::CEffectMesh(const FilenameWrapper& c_szFileName) : CResource(c_szFileName)
{
	m_iGeomCount = 0;
	m_iFrameCount = 0;
	m_isData = false;
