...
// For SoLoud
static std::string stl_resource_convert(const std::string& str, bool rev = false)
{
	std::string out = str;
	stl_lowers(out);
	if (rev)
...
template <typename TString> int32_t hex2dec(TString szhex)
{
...
template <typename TString> uint32_t htmlColorStringToARGB(TString str)
{
...
template <typename T> void DeleteVectorItem(std::vector<T>* pVector, uint32_t dwIndex)
{
...
template <typename T> void DeleteVectorItem(T* pVector, uint32_t dwStartIndex, uint32_t dwEndIndex)
{
...
public:
	...
	int32_t size() const { return m_pos; }
...
template <typename TData, typename THandle = int32_t> class stl_circle_pool
{
public:
	typedef bool TFlag;
