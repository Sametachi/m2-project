...
{
	float fElapsedTime = CTimer::GetInstance()->GetCurrentSecond() - m_fLastUpdate;
	m_fLastUpdate = CTimer::GetInstance()->GetCurrentSecond();
  ...
  */
	...
	for(int32_t loop = 0; loop<=1; ++loop)
	{
		...
		for(float t = 0; t<=length; t+=dt)
		{
			...
			v.diffuse = D3DXCOLOR(0.3f,0.8f,1.0f, (loop)? std::min(std::max(float((1.0f-ttt)*(1.0f-ttt)/2.5-0.1f),0.0f),1.0f):0.0f );
			//v.diffuse = D3DXCOLOR(0.0f,0.0f,0.0f, (loop)?std::min(std::max((1.0f-ttt)*(1.0f-ttt)-0.1f,0.0f),1.0f):0.0f );
...
{
	CGraphicImage * pImage = CResourceManager::GetInstance()->LoadResource<CGraphicImage>("lot_ade10-2.tga");
