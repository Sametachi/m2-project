...
bool CFlyingManager::RegisterFlyingData(const char* c_szFilename)
{
	...
	{
		TraceLog("CEffectManager::RegisterFlyingData {} - Failed to load flying data file", c_szFilename);
...
bool CFlyingManager::RegisterFlyingData(const char* c_szFilename, uint32_t & r_dwRetCRC)
{
	...
	{
		TraceLog("CFlyingManager::RegisterFlyingData - Already exists flying data named [{}]", c_szFilename);
		return false;
	}
...
bool CFlyingManager::RegisterIndexedFlyData(uint32_t dwIndex, uint8_t byType, const char * c_szFileName)
{
	...
	{
		TraceLog("CFlyingManager::RegisterIndexFlyData(dwIndex={}, c_szFileName={}) - Failed to load flying data file", dwIndex, c_szFileName);
		return false;
	}
...
void CFlyingManager::CreateIndexedFly(uint32_t dwIndex, CActorInstance * pStartActor, CActorInstance * pEndActor)
{
	...
	{
		TraceLog("CFlyingManager::CreateIndexedFly(dwIndex={}) - Not registered index", dwIndex);
		return;
	}
