...
void CActorInstance::UpdatePointInstance(TCollisionPointInstance* pPointInstance)
{
	if (!pPointInstance)
	{
		assert(!"CActorInstance::UpdatePointInstance - pPointInstance is nullptr");
		return;
	}
	...
	if (pPointInstance->isAttached)
	{
		...
		CGrannyModelInstance* pModelInstance = m_modelInstances[pPointInstance->dwModelIndex];
		if (!pModelInstance)
		{
			return;
		}
...
{
	// When riding a horse, the person has no movement value, so we have to get it from the horse - [levites]
	D3DXVECTOR3 v3Movement = m_v3Movement;
	if (m_pkHorse)
	{
		v3Movement = m_pkHorse->m_v3Movement;
		// Horses don't need to be updated - [levites]
	...
	for (; itor != m_BodyPointInstanceList.end(); ++itor)
	{
		...
		if (rInstance.isAttached)
		{
			...
			{
				SysLog("CActorInstance::UpdateAdvancingPointInstance - rInstance.dwModelIndex={0} >= m_modelInstances.size()={1}", rInstance.dwModelIndex, m_modelInstances.size());
				continue;
			}

			CGrannyModelInstance* pModelInstance = m_modelInstances[rInstance.dwModelIndex];
			if (!pModelInstance)
			{
				continue;
			}

			D3DXMATRIX* pBoneMat = (D3DXMATRIX*)pModelInstance->GetBoneMatrixPointer(rInstance.dwBoneIndex);
...
bool CActorInstance::CreateCollisionInstancePiece(uint32_t dwAttachingModelIndex, const NRaceData::TAttachingData* c_pAttachingData, TCollisionPointInstance* pPointInstance)
{
	if (!c_pAttachingData)
	{
		assert(!"CActorInstance::CreateCollisionInstancePiece - c_pAttachingData is nullptr");
		return false;
	}

	if (!c_pAttachingData->pCollisionData)
	{
		assert(!"CActorInstance::CreateCollisionInstancePiece - c_pAttachingData->pCollisionData is nullptr");
		return false;
	}

	if (!pPointInstance)
	{
		assert(!"CActorInstance::CreateCollisionInstancePiece - pPointInstance is nullptr");
		return false;
	}
	...
	if (c_pAttachingData->isAttaching)
	{
		...
		CGrannyModelInstance* pModelInstance = m_modelInstances[dwAttachingModelIndex];
		...
		{
			//SysLog("Failed to find bone {0} in model {1} for collision attachment", c_pAttachingData->strAttachingBoneName.c_str(), dwAttachingModelIndex);
			pPointInstance->isAttached = true;
			pPointInstance->dwBoneIndex = 0;
		}
	}
...
BOOL CActorInstance::__NormalAttackProcess(CActorInstance & rVictim)
{
	...
	if (CRaceManager::GetInstance()->IsHugeRace(rVictim.GetRace()))
	{
	...
	for (; itorHitData != pad->HitDataContainer.end(); ++itorHitData)
	{
		...
		range_start = c_rHitData.mapHitPosition.lower_bound(motiontime-CTimer::GetInstance()->GetElapsedSecond());
