...
void CMapOutdoor::LoadWaterTexture()
{
	...
	for (int32_t i = 0; i < 30; ++i)
	{
		...
		m_WaterInstances[i].SetImagePointer(CResourceManager::GetInstance()->LoadResource<CGraphicImage>(buf));
	}
}
...
void CMapOutdoor::RenderWater()
{
	...
	static uint32_t s_dwLastHeightChangeTime = CTimer::GetInstance()->GetCurrentMillisecond();
	static uint32_t s_dwBlendtime = 300;

	if ((CTimer::GetInstance()->GetCurrentMillisecond() - s_dwLastHeightChangeTime) > s_dwBlendtime)
	{
		...
		s_dwLastHeightChangeTime = CTimer::GetInstance()->GetCurrentMillisecond();
	}

	s_fWaterHeightCurrent = s_fWaterHeightBegin + (s_fWaterHeightEnd - s_fWaterHeightBegin) * (float)((CTimer::GetInstance()->GetCurrentMillisecond() - s_dwLastHeightChangeTime) / (float)s_dwBlendtime);
	...
	float fFogDistance = __GetFogDistance() + 16000.0f;
