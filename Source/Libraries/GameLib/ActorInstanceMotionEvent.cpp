...
void CActorInstance::MotionEventProcess(uint32_t dwcurFrame, int32_t iIndex, const CRaceMotionData::TMotionEventData * c_pData)
{
	...
	switch (c_pData->iType)
	{
		...
		case MOTION_EVENT_TYPE_SCREEN_WAVING:
			CGameEventManager::GetInstance()->ProcessEventScreenWaving(this, (const CRaceMotionData::TScreenWavingEventData *)c_pData);
			break;
...
void CActorInstance::ProcessMotionEventEffectEvent(const CRaceMotionData::TMotionEventData * c_pData)
{
	...
	{
		int32_t iIndex = CEffectManager::GetInstance()->CreateEffect(c_pEffectData->dwEffectIndex, D3DXVECTOR3(0.0f, 0.0f, 0.0f), D3DXVECTOR3(0.0f, 0.0f, 0.0f));
		...
		CEffectManager::GetInstance()->SetEffectInstanceGlobalMatrix(matWorld);
		return;
	}
	...
	if (c_pEffectData->isAttaching)
	{
		...
		else
		{
			...
			if (FindBoneIndex(dwPartIndex, c_pEffectData->strAttachingBoneName.c_str(), &iBoneIndex))
			{
				...
				int32_t iIndex = CEffectManager::GetInstance()->CreateEffect(c_pEffectData->dwEffectIndex,
														c_pEffectData->v3EffectPosition,
...
void CActorInstance::ProcessMotionEventEffectToTargetEvent(const CRaceMotionData::TMotionEventData * c_pData)
{
	...
	else
	{
		...
		else
		{
			...
			CEffectManager::GetInstance()->CreateEffect(c_pEffectToTargetData->dwEffectIndex, v3Position, D3DXVECTOR3(0.0f, 0.0f, 0.0f));
		}
	}
}
...
void CActorInstance::ProcessMotionEventSound(const CRaceMotionData::TMotionEventData * c_pData)
{
	...
	TraceLog("PLAY SOUND: {}", c_pSoundData->strSoundFileName.c_str());
	CSoundManager::GetInstance()->PlaySound3D(m_x, m_y, m_z, c_pSoundData->strSoundFileName.c_str());
}
...
void CActorInstance::ProcessMotionEventFly(const CRaceMotionData::TMotionEventData * c_pData)
{
	...
	if (m_kFlyTarget.IsValidTarget())
	{
		auto rfm = CFlyingManager::GetInstance();
...
{
	if (MOTION_EVENT_TYPE_WARP != c_pData->iType)
		return;
