...
void CMapOutdoor::SetIndexBuffer()
{
	...
	for (uci = 0; uci < TERRAINPATCH_LODMAX; ++uci)
	{
		...
		std::memset(m_pwaIndices[uci], 0, sizeof(uint16_t) * dwIndexNum);
		count[uci] = 0;
		count2[uci] = 0;
		if (!m_IndexBuffer[uci].Create(dwIndexNum, D3DFMT_INDEX16))
		{
			SysLog("CMapOutdoor::SetIndexBuffer() IndexBuffer Create Error");
		}
	}
	...
	for (uci = 0; uci < TERRAINPATCH_LODMAX; ++uci)
	{
		...
		{
			SysLog("CMapOutdoor::SetIndexBuffer() IndexBuffer Unlock Error");
		}
		std::memcpy(pIndices, m_pwaIndices[uci], count[uci] * sizeof(uint16_t));
