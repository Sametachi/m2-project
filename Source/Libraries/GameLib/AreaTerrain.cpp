...
bool CTerrain::Initialize()
{
	...
	m_v3Pick = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
...
{
	auto r = CResourceManager::GetInstance()->LoadResource<CGraphicImage>(c_pchMiniMapFileName);
	if (!r)
	{
		SysLog("Failed to load {0} for {1}/{2}", c_pchMiniMapFileName, m_wX, m_wY);
		return;
	}
	...
	{
		SysLog("CTerrain::LoadMiniMapTexture - MiniMapTexture Error");
		m_lpMiniMapTexture = nullptr;
	}
}
...
void CTerrain::LoadShadowTexture(const char* ShadowFileName)
{
	...
	auto r = CResourceManager::GetInstance()->LoadResource<CGraphicImage>(ShadowFileName);
	if (!r)
	{
		SysLog("Failed to load {0} for {1}/{2}", ShadowFileName, m_wX, m_wY);
		return;
	}
...
{
	uint32_t dwStart = ELTimer_GetMSec();
	auto vfs = CallFS().Open(c_pszFileName, Buffered);
	if (!vfs)
		return false;
	...
	{
		SysLog("ShadowMap {0} SIZE ERROR", c_pszFileName);
		return false;
	}

	vfs->Read(0, m_awShadowMap, dwShadowMapSize);

	ConsoleLog("LoadShadowMap {0} took {1} ms", c_pszFileName, ELTimer_GetMSec() - dwStart);
	return true;
}
...
bool CTerrain::RAW_LoadTileMap(const char* c_pszFileName)
{
	...
	uint32_t dwStart = ELTimer_GetMSec();
	RAW_AllocateSplats();
	ConsoleLog("CTerrain::RAW_AllocateSplats {0}", ELTimer_GetMSec() - dwStart);
	return true;
}
...
bool CTerrain::LoadHeightMap(const char* c_pszFileName)
{
	...
	ConsoleLog("LoadHeightMap::CalculateNormal {0} ms", ELTimer_GetMSec() - dwStart);
	return true;
}
...
bool CTerrain::isAttrOn(uint16_t wCoordX, uint16_t wCoordY, uint8_t byAttrFlag)
{
	...
	{
		ConsoleLog("CTerrain::isAttrOn Coordiante Error! Return false... Input Coord - X : {0}, Y : {1} ( Limit X : {2}, Y : {3})", wCoordX, wCoordY, ATTRMAP_XSIZE, ATTRMAP_YSIZE);
		return false;
	}
...
uint8_t CTerrain::GetAttr(uint16_t wCoordX, uint16_t wCoordY)
{
	if (wCoordX >= ATTRMAP_XSIZE || wCoordY >= ATTRMAP_YSIZE)
	{
		ConsoleLog("CTerrain::GetAttr Coordiante Error! Return 0... Input Coord - X : {0}, Y : {1} ( Limit X : {2}, Y : {3})", wCoordX, wCoordY, ATTRMAP_XSIZE, ATTRMAP_YSIZE);
		return 0;
	}
...
void CTerrain::GetWaterHeight(uint8_t byWaterNum, int32_t* plWaterHeight)
{
	...
	{
		ConsoleLog("CTerrain::GetWaterHeight WaterNum {0}(Total Num {1}) ERROR!", byWaterNum, m_byNumWater);
		return;
	}
...
bool CTerrain::GetWaterHeight(uint16_t wCoordX, uint16_t wCoordY, int32_t* plWaterHeight)
{
	...
	if (byWaterNum > m_byNumWater)
	{
		ConsoleLog("CTerrain::GetWaterHeight (X {0}, Y {1}) ERROR!", wCoordX, wCoordY, m_byNumWater);
		return false;
	}
				...
				if (rSplat.Active)   // We already have an alpha map which needs to be updated
				{
					if (m_lpAlphaTexture[i])
					{
						...
						do
						{
							ulRef = m_lpAlphaTexture[i]->Release();
							if (ulRef > 0)
								SysLog(" CTerrain::RAW_GenerateSplat - TileCount > 0 : Alpha Texture Release({0}) ERROR", ulRef);
						} while (ulRef > 0);
					}
					rSplat.pd3dTexture = m_lpAlphaTexture[i] = nullptr;
				}
				...
				for (int32_t y = 0; y < SPLATALPHA_RAW_YSIZE; ++y)
				{
					for (int32_t x = 0; x < SPLATALPHA_RAW_XSIZE; ++x)
					{
						...
						else if (byTileNum > i)
						{
							...
							if (byTileTL == i || byTileTR == i || byTileBL == i || byTileBR == i || byTileT == i || byTileB == i || byTileL == i || byTileR == i)
								*aptr = 0xFF;
							else
								*aptr = 0x00;
						}
				...
				if (rSplat.Active)
				{
					if (m_lpAlphaTexture[i])
					{
						...
						do
						{
							ulRef = m_lpAlphaTexture[i]->Release();
							if (ulRef > 0)
								SysLog(" CTerrain::RAW_GenerateSplat - TileDount 0 : Alpha Texture Release({0}) ERROR", ulRef);
						} while (ulRef > 0);
					}
...
LPDIRECT3DTEXTURE9 CTerrain::AddTexture32(uint8_t byImageNum, uint8_t* pbyImage, int32_t lTextureWidth, int32_t lTextureHeight)
{
	...
	hr = ms_lpd3dDevice->CreateTexture(uiNewWidth, uiNewHeight, 5, 0, format, D3DPOOL_MANAGED, &pkTex, 0);

	if (FAILED(hr))
	{
		SysLog("CTerrain::AddTexture32 - CreateTexture Error");
		return nullptr;
	}
	...
	uint8_t abResizeImage[256 * 256]{};
	{
		...
		for (uint32_t y = 0; y < 256; ++y, abCurLine += 258)
		{
			for (uint32_t x = 0; x < 256; ++x)
			{
				...
				*pbDstPixel++ = (((pbSrcPixel[0] + pbSrcPixel[1] + pbSrcPixel[2] +
