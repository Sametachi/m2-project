...
class IMobProto : public Singleton<IMobProto>
{
	public:
		IMobProto() {}
		virtual ~IMobProto() {}
...
class CActorInstance : public IActorInstance, public IFlyTargetableObject
{
	...
	public:
		...
		void AttachWeapon(uint32_t dwItemIndex, uint32_t dwParentPartIndex = PART_MAIN);
		void AttachWeapon(uint32_t dwParentPartIndex, uint32_t dwPartIndex, CItemData* pItemData);
		void AttachAcce(uint32_t dwItemIndex);
...
public:
	...
	protected:
		...
		bool InterceptMotion(EMotionPushType iMotionType, uint16_t wMotion, float fBlendTime = 0.1f, UINT uSkill=0, float fSpeedRatio=1.0f);
	...
	protected:
		...
		// Part
		uint32_t						m_adwPartItemID[PART_MAX_NUM];
