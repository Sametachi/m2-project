...
bool Environment_Load(SEnvironmentData& envData, const char* envFileName)
{
	...
	{
		textLoader.GetTokenBoolean("btexturerendermode", &envData.bSkyBoxTextureRenderMode);
		...
		if (textLoader.GetTokenVector("cloudcolor", &pTokenVectorCloudColor))
			...
			{
				envData.CloudGradientColor.m_FirstColor.r = std::stof(pTokenVectorCloudColor->at(0).c_str());
				envData.CloudGradientColor.m_FirstColor.g = std::stof(pTokenVectorCloudColor->at(1).c_str());
				envData.CloudGradientColor.m_FirstColor.b = std::stof(pTokenVectorCloudColor->at(2).c_str());
				envData.CloudGradientColor.m_FirstColor.a = std::stof(pTokenVectorCloudColor->at(3).c_str());

				envData.CloudGradientColor.m_SecondColor.r = std::stof(pTokenVectorCloudColor->at(4).c_str());
				envData.CloudGradientColor.m_SecondColor.g = std::stof(pTokenVectorCloudColor->at(5).c_str());
				envData.CloudGradientColor.m_SecondColor.b = std::stof(pTokenVectorCloudColor->at(6).c_str());
				envData.CloudGradientColor.m_SecondColor.a = std::stof(pTokenVectorCloudColor->at(7).c_str());
			}
		...
		if (textLoader.GetTokenVector("gradient", &pTokenVector))
			if (0 == pTokenVector->size() % 8)
				if (byGradientCount == pTokenVector->size() / 8)
				{
					...
					{
						envData.SkyBoxGradientColorVector[i].m_FirstColor.r = std::stof(pTokenVector->at(i * 8 + 0).c_str());
						envData.SkyBoxGradientColorVector[i].m_FirstColor.g = std::stof(pTokenVector->at(i * 8 + 1).c_str());
						envData.SkyBoxGradientColorVector[i].m_FirstColor.b = std::stof(pTokenVector->at(i * 8 + 2).c_str());
						envData.SkyBoxGradientColorVector[i].m_FirstColor.a = std::stof(pTokenVector->at(i * 8 + 3).c_str());

						envData.SkyBoxGradientColorVector[i].m_SecondColor.r = std::stof(pTokenVector->at(i * 8 + 4).c_str());
						envData.SkyBoxGradientColorVector[i].m_SecondColor.g = std::stof(pTokenVector->at(i * 8 + 5).c_str());
						envData.SkyBoxGradientColorVector[i].m_SecondColor.b = std::stof(pTokenVector->at(i * 8 + 6).c_str());
						envData.SkyBoxGradientColorVector[i].m_SecondColor.a = std::stof(pTokenVector->at(i * 8 + 7).c_str());
					}
				}
