...
CFlyTrace::CFlyTrace()
{
	...
	/*
	// Code for texture
	CGraphicImage * pImage = (CGraphicImage *)CResourceManager::GetInstance()->GetResourcePointer("d:/ray.jpg");
...
{
	m_TimePositionDeque.push_front(TTimePosition(CTimer::GetInstance()->GetCurrentSecond(),v3Position));
	//Tracenf("%f %f",m_TimePositionDeque.back().first, CTimer::GetInstance()->GetCurrentSecond());
	while(!m_TimePositionDeque.empty() && m_TimePositionDeque.back().first+m_fTailLength<CTimer::GetInstance()->GetCurrentSecond())
	{
...
void CFlyTrace::Render()
{
	...
	for(;it2!=m_TimePositionDeque.end();++it2,++it1)
	{
		...
		float rate1 = (1-(CTimer::GetInstance()->GetCurrentSecond()-it1->first)/m_fTailLength);
		float rate2 = (1-(CTimer::GetInstance()->GetCurrentSecond()-it2->first)/m_fTailLength);
		float size1 = m_fSize;
		float size2 = m_fSize;
		if (!m_bRectShape)
		{
