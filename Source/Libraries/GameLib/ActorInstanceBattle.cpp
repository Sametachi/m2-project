...
void CActorInstance::ComboProcess()
{
	...
	if (0 != m_dwcurComboIndex)
	{
		...
		{
			TraceLog("Attacking motion data is NULL! : {}\n", m_dwcurComboIndex);
			__ClearCombo();
			return;
		}
...
void CActorInstance::__RunNextCombo()
{
	...
	if (wComboIndex==0)
	{
		TraceLog("CActorInstance::__RunNextCombo(wComboType={}, wComboIndex={})", wComboType, wComboIndex);
		return;
	}
	...
	{
		TraceLog("CActorInstance::__RunNextCombo(wComboType={}, wComboIndex={}) - m_pkCurRaceData->GetComboDataPointer(m_wcurMotionMode={}, &pComboData) == NULL",
	...
	{
		TraceLog("CActorInstance::__RunNextCombo(wComboType={}, wComboIndex={}) - (dwComboArrayIndex={}) >= (pComboData->ComboIndexVector.size()={})",
...
{
	if (rkActorDst.IsBuilding() || rkActorDst.IsDoor() || rkActorDst.IsStone())
		return false;

	if (rkActorDst.IsNPC())
		return false;

	if (CRaceManager::GetInstance()->IsHugeRace(rkActorDst.GetRace()))
		return false;
...
void CActorInstance::__ProcessDataAttackSuccess(const NRaceData::TAttackData & c_rAttackData, CActorInstance & rVictim, const D3DXVECTOR3 & c_rv3Position, UINT uiSkill, BOOL isSendPacket)
{
	...
	{
		if (uiSkill)
			rVictim.m_fInvisibleTime = CTimer::GetInstance()->GetCurrentSecond() + c_rAttackData.fInvisibleTime;
	...
	else
	{
		...
		{
			TraceLog("ProcessSucceedingAttacking: Unknown AttackingData.iHittingType {}", c_rAttackData.iHittingType);
		}
	}
...
void CActorInstance::__HitGreate(CActorInstance& rVictim)
{
	...
	else
	{
		if (fScalar < 0.0f)
		{
			...
			{
				rVictim.PushOnceMotion(MOTION_STAND_UP);
				rVictim.PushLoopMotion(MOTION_WAIT);
			}
		}
		else
		{
			...
			{
				rVictim.PushOnceMotion(MOTION_STAND_UP_BACK);
				rVictim.PushLoopMotion(MOTION_WAIT);
			}
		}
	}
...
bool CActorInstance::__isInvisible()
{
	...
	if (CTimer::GetInstance()->GetCurrentSecond() >= m_fInvisibleTime)
		return false;
