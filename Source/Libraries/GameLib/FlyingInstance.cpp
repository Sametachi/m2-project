...
void CFlyingInstance::UpdateAttachInstance()
{
	...
	D3DXQuaternionRotationYawPitchRoll(&q,
		D3DXToRadian(m_pData->m_v3AngVel.y)*CTimer::GetInstance()->GetElapsedSecond(),
		D3DXToRadian(m_pData->m_v3AngVel.x)*CTimer::GetInstance()->GetElapsedSecond(),
		D3DXToRadian(m_pData->m_v3AngVel.z)*CTimer::GetInstance()->GetElapsedSecond());
	...
	for(it = m_vecAttachEffectInstance.begin();it!=m_vecAttachEffectInstance.end();++it)
	{
		...
		switch(rfad.iFlyType)
		{
			...
			case CFlyingData::FLY_ATTACH_TYPE_SINE:
				{
					//Tracenf("%f",CTimer::GetInstance()->GetCurrentSecond());
					float angle = (CTimer::GetInstance()->GetCurrentSecond() - m_fStartTime)*2*3.1415926535897931f/rfad.fPeriod;
					D3DXVECTOR3 p(
			...
			case CFlyingData::FLY_ATTACH_TYPE_EXP:
				{
					float dt = CTimer::GetInstance()->GetCurrentSecond() - m_fStartTime;
...
bool CFlyingInstance::Update()
{
	...
	if (m_pData->m_bIsHoming &&
		m_pData->m_fHomingStartTime + m_fStartTime < CTimer::GetInstance()->GetCurrentSecond())
	{
	...
	m_v3Velocity += m_v3Accel*CTimer::GetInstance()->GetElapsedSecond();
	m_v3Velocity.z+=m_pData->m_fGravity * CTimer::GetInstance()->GetElapsedSecond();
	D3DXVECTOR3 v3Movement = m_v3Velocity * CTimer::GetInstance()->GetElapsedSecond();
	...
	if (m_pData->m_bHitOnBackground)
	{
		...
		if (CFlyingManager::GetInstance()->GetMapManagerPtr())
		{
			float fGroundHeight = CFlyingManager::GetInstance()->GetMapManagerPtr()->GetTerrainHeight(m_v3Position.x,-m_v3Position.y);
...
void CFlyingInstance::__SetDataPointer(CFlyingData * pData, const D3DXVECTOR3 & v3StartPosition)
{
	...
	m_fStartTime = CTimer::GetInstance()->GetCurrentSecond();
