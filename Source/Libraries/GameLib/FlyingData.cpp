...
void CFlyingData::SetBombEffect(const char* szEffectName)
{
	...
	if (CEffectManager::GetInstance()->RegisterEffect(m_strBombEffectName.c_str()),true)
	{
...
//		m_fCollisionSphereRadius = 0.0f;
	...
	else if (!m_strBombEffectName.empty())
	{
		...
		CEffectManager::GetInstance()->RegisterEffect2(m_strBombEffectName.c_str(),&m_dwBombEffectID);
	}
	...
	for(i=0;i<TextFileLoader.GetChildNodeCount();i++)
	{
		...
		if (strNodeName=="attachdata")
		{
			...
			else if (!fad.strFilename.empty())
			{
				...
				CEffectManager::GetInstance()->RegisterEffect(fad.strFilename.c_str());
			}
		...
		{
			TraceLog("CFlyingData::LoadScriptFile Wrong Data : {}", m_strFilename.c_str());
		}
	}
	return true;
}
