...
{
	m_isWalking = TRUE;
	if (MOTION_RUN == GET_MOTION_INDEX(m_kCurMotNode.dwMotionKey))
		SetLoopMotion(MOTION_WALK, 0.15f, m_fMovSpd);
}
...
{
	m_isWalking = FALSE;
	if (MOTION_WALK == GET_MOTION_INDEX(m_kCurMotNode.dwMotionKey))
		SetLoopMotion(MOTION_RUN, 0.15f, m_fMovSpd);
}
...
void CActorInstance::__DestroyTree()
{
	...
	CSpeedTreeForestDirectX9::GetInstance()->DeleteInstance(m_pkTree);
}
