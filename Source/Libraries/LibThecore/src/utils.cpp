...
void trim_and_lower(const char* src, char* dest, size_t dest_size)
{
   ...
   // get \0
    ...
    {
		*(dest++) = LOWER(*tmp); // LOWER is a macro, so ++ must not be used
		++tmp;
		++len;
    }
...
/* Returns 1 when arg1 is the same as the previous end of arg2. */
...
/*	In a string consisting of "item: value" such as "Name: leaf"
    Copy the item to token and the value to value and return it. */
...
int number_ex(int from, int to, const char* file, int line)
{
    if (from > to)
    {
		...
		WarnLog("number(): first argument is bigger than second argument {} -> {}, {} {}", from, to, file, line);
...
void thecore_sleep(struct timeval* timeout)
{
    if (select(0, (fd_set*) 0, (fd_set*) 0, (fd_set*) 0, timeout) < 0)
    {
		if (errno != EINTR)
		{
			WarnLog("select sleep {}", strerror(errno));
			return;
		}
    }
}
...
{
	uint64_t x;
	__asm__ volatile (".uint8_t 0x0f, 0x31" : "=A" (x));
	return x;
}
*/
