...
/*
  ...
  c). For the best performance 56-bit encryption key 
      have to be extendended to 128-uint8_t array, i.e. for each 
  ...
  d). We can use XOR, SHIFT, AND operations to swap 
      ...
      First, shift uint16_t A to get bites 1, 3 on the "right place"

      uint16_t TMP = (uint16_t A) >> 1      = 1   2   3   -

      TMP      = TMP ^ B            = 1^4 2^5 3^6 7

      we don't want to change bits 5 and 7 in the B uint16_t, so 

      TMP = TMP & MASK = TMP & 1010 = 1^4  -  3^6 -  

      now we can easy get the uint16_t B:

      B = B ^ TMP = (4 5 6 7) ^ (1^4  -  3^6 -) = 1 5 3 7 

      if we shift our "masking" TMP uint16_t reverse - we get 
      a mask for A uint16_t:

      TMP = TMP << 1 =  - 1^4  -  3^6

      now we can easy get the uint16_t A:
