...
/*
  ...
  * Edited by Hanju Kim aka. Fertilizer, Cronan
  *
  ...
  * Used to encrypt/decrypt 8 bytes. key must be 16 bytes.
  * sz and sy are assigned in the reverse order of 8 bytes.
  *
  ...
  * Used when encrypting/decrypting more than 8 bytes at once. If size is
  * Encrypt by "increasing" the size by a multiple of 8 if it is not a multiple of 8.
  *
  * ex. tea_code(pdwSrc[1], pdwSrc[0], pdwKey, pdwDest);
  * tea_decrypt(pdwDest, pdwSrc, pdwKey, nSize);
  */

#define TEA_ROUND		32			// 32 is recommended, and the higher the value, the more difficult the result.
