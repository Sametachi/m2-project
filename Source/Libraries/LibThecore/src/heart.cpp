...
int heart_idle(LPHEART ht)
{
	...
	// I missed pulse because I was busy, but I don't have time to sleep...
	// Pulse fps is wrong anyway, it doesn't matter if you get it right.
	if (missed_pulse > 0)
	{
		gettimeofday(&ht->last_time, (struct timezone*) 0);
	}
    ...
    if (missed_pulse <= 0)
    {
		WarnLog("missed_pulse is not positive! ({})", missed_pulse);
		missed_pulse = 1;
    }

    if (missed_pulse > (30 * ht->passes_per_sec))
    {
		WarnLog("losing {} seconds. (lag occured)", missed_pulse / ht->passes_per_sec);
