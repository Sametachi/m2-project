...
{
	_SQLResult() : pSQLResult(nullptr), uiNumRows(0), uiAffectedRows(0), uiInsertID(0)
	{
	}
...
typedef struct _SQLMsg
{
	...
	void Store()
	{
		do
		{
			...
			vec_pkResult.emplace_back(pRes);
...
public:
	...
	bool Setup(const char* c_pszHost, const char* c_pszUser, const char* c_pszPassword, const char* c_pszDB, const char* c_pszLocale, bool bNoThread = false, int32_t iPort = 0);
