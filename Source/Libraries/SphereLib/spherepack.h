...
public:
	...
	SpherePackFactory(int32_t maxspheres, float rootsize, float leafsize, float gravy);
	virtual ~SpherePackFactory(void);
	void Process(void);

	SpherePack* AddSphere_(const Vector3d& pos, float radius, void* userdata, bool isSphere, int32_t flags = SPF_LEAF_TREE);
