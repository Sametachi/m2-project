...
IDirect3DVertexBuffer9* CGraphicBase::m_largePdt2DVertexBuffer;
...
bool CGraphicBase::SetPDTStream(SPDTVertexRaw* pSrcVertices, UINT uVtxCount)
{
	...
	SPDTVertexRaw* dst = nullptr;
	if (FAILED(vb->Lock(0, bytes, (VOID**)&dst, D3DLOCK_DISCARD)))
	{
		D3DPERF_SetMarker(D3DCOLOR_ARGB(251, 50, 50, 0), L" SetPDTStream fail");
	...
	vb->Unlock();
	D3DPERF_SetMarker(D3DCOLOR_ARGB(251, 50, 50, 0), L" SetPDTStream");

	STATEMANAGER->SetStreamSource(0, vb, sizeof(TPDTVertex));
	return true;
}

bool CGraphicBase::Set2DPDTStream(SPDTVertex2D* pVertices, UINT uVtxCount)
{
	return Set2DPDTStream((SPDTVertex2DRaw*)pVertices, uVtxCount);
}

bool CGraphicBase::Set2DPDTStream(SPDTVertex2DRaw* pSrcVertices, UINT uVtxCount)
{
	assert(uVtxCount <= kLargePdtVertexBufferSize);
	...
	IDirect3DVertexBuffer9* vb = GetLargePdt2DVertexBuffer();

	const auto bytes = sizeof(TPDTVertex) * uVtxCount;

	SPDTVertex2DRaw* dst = nullptr;
	if (FAILED(vb->Lock(0, bytes, (VOID**)&dst, D3DLOCK_DISCARD)))
	{
	...
	std::memcpy(dst, pSrcVertices, bytes);
	vb->Unlock();
	D3DPERF_SetMarker(D3DCOLOR_ARGB(251, 50, 50, 0), L" Set2DPDTStream");

	STATEMANAGER->SetStreamSource(0, vb, sizeof(TPDTVertex2D));
	return true;
}
...
//	CCameraManager::GetInstance()->SetCurrentCamera(CCameraManager::DEFAULT_PERSPECTIVE_CAMERA);
	CCameraManager::GetInstance()->GetCurrentCamera()->SetViewParams(vectorEye, vectorCenter, vectorUp);
	UpdateViewMatrix();
}
...
void CGraphicBase::SetPositionCamera(float fx, float fy, float fz, float distance, float pitch, float roll)
{
	...
	if (ms_dwWavingEndTime > CTimer::GetInstance()->GetCurrentMillisecond())
	{
...
{
	//CCameraManager::GetInstance()->SetCurrentCamera(CCameraManager::DEFAULT_ORTHO_CAMERA);
...
{
	CCamera* pkCamera=CCameraManager::GetInstance()->GetCurrentCamera();
	if (!pkCamera)
		return;
...
{
	*px = CCameraManager::GetInstance()->GetCurrentCamera()->GetTarget().x;
	*py = CCameraManager::GetInstance()->GetCurrentCamera()->GetTarget().y;
	*pz = CCameraManager::GetInstance()->GetCurrentCamera()->GetTarget().z;
}

void CGraphicBase::GetCameraPosition(float * px, float * py, float * pz)
{
	*px = CCameraManager::GetInstance()->GetCurrentCamera()->GetEye().x;
	*py = CCameraManager::GetInstance()->GetCurrentCamera()->GetEye().y;
	*pz = CCameraManager::GetInstance()->GetCurrentCamera()->GetEye().z;
}
...
{
	ms_dwWavingEndTime = CTimer::GetInstance()->GetCurrentMillisecond() + int32_t(fDuringTime * 1000.0f);
	ms_iWavingPower = iPower;
}

void CGraphicBase::SetScreenEffectFlashing(float fDuringTime, const D3DXCOLOR & c_rColor)
{
	ms_dwFlashingEndTime = CTimer::GetInstance()->GetCurrentMillisecond() + int32_t(fDuringTime * 1000.0f);
