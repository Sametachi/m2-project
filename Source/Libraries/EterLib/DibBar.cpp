...
{
	RECT rect = { 0, 0, (int32_t)m_dwWidth,(int32_t)m_dwHeight };
...
CBlockTexture* CDibBar::__BuildTextureBlock(uint32_t dwxPos, uint32_t dwyPos, uint32_t dwImageWidth, uint32_t dwImageHeight, uint32_t dwTextureWidth, uint32_t dwTextureHeight)
{
	...
	RECT posRect = {
		static_cast<int32_t>(dwxPos),
		static_cast<int32_t>(dwyPos),
		static_cast<int32_t>(dwxPos + dwImageWidth),
		static_cast<int32_t>(dwyPos + dwImageHeight)
	};
...
bool CDibBar::Create(HDC hdc, uint32_t dwWidth, uint32_t dwHeight)
{
	...
	{
		ConsoleLog("Failed to create CDibBar\n");
		return false;
	}
