...
void CGraphicObjectInstance::Clear()
{
	if (m_CullingHandle)
	{
		CCullingManager::GetInstance()->Unregister(m_CullingHandle);
	...
	m_v3ScaleAttachment.x = m_v3ScaleAttachment.y = m_v3ScaleAttachment.z = 0.0f;

	D3DXMatrixIdentity(&m_worldMatrix);
	D3DXMatrixIdentity(&m_mRotation);
	D3DXMatrixIdentity(&m_matAttachedWorldTransform);
	D3DXMatrixIdentity(&m_matScaleTransformation);
...
{
	D3DXMATRIX mat;
	D3DXMatrixMultiply(&mat, &m_matScale, &m_mRotation);
	m_worldMatrix = mat;
...
const D3DXVECTOR3& CGraphicObjectInstance::GetAttachmentScale() const
{
...
void CGraphicObjectInstance::SetAttachmentScale(const D3DXVECTOR3& scale)
{
...
void CGraphicObjectInstance::RegisterBoundingSphere()
{
	...
	m_CullingHandle = CCullingManager::GetInstance()->Register(this);
}
...
void CGraphicObjectInstance::SetPortal(uint32_t dwIndex, int32_t iID)
{
	...
	m_abyPortalID[dwIndex] = (uint8_t)iID;
}
