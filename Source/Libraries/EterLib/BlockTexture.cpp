...
void CBlockTexture::InvalidateRect(const RECT& c_rsrcRect)
{
	...
	{
		ConsoleLog("InvalidateRect() - Strange rect");
		return;
	}

	// DIBBAR_LONGSIZE_BUGFIX
	const RECT clipRect = {
		std::max<int32_t>(c_rsrcRect.left - dstRect.left, 0l),
		std::max<int32_t>(c_rsrcRect.top - dstRect.top, 0l),
		std::min<int32_t>(c_rsrcRect.right - dstRect.left, dstRect.right - dstRect.left),
		std::min<int32_t>(c_rsrcRect.bottom - dstRect.top, dstRect.bottom - dstRect.top)
	};
	...
	{
		ConsoleLog("InvalidateRect() - Failed to LockRect");
		return;
	}
...
bool CBlockTexture::Create(CGraphicDib* pDIB, const RECT& c_rRect, uint32_t dwWidth, uint32_t dwHeight)
{
	...
	{
		ConsoleLog("Failed to create block texture {0}, {1}\n", dwWidth, dwHeight);
		return false;
	}
