	...
	m_fScaleZ(1.0f)
{
	...
	m_dwlastTime = CTimer::GetInstance()->GetCurrentMillisecond();
...
CGraphicImageInstance * CSkyObject::GenerateTexture(const char * szfilename)
{
	...
	CResource * pResource = CResourceManager::GetInstance()->LoadResource<CResource>(szfilename);
...
{
	if (iFaceIndex < 0 || iFaceIndex >= SKYBOX_FACE_MAX_NUM)
		return;
...
//	CGraphicImage * pImage = (CGraphicImage *) CResourceManager::GetInstance()->GetResourcePointer("D:\\Ymir Work\\special/cloudalpha.tga");
...
void CSkyBox::Refresh()
{
	...
	if( m_ucRenderMode == CSkyObject::SKY_RENDER_MODE_DEFAULT ||  m_ucRenderMode == CSkyObject::SKY_RENDER_MODE_DIFFUSE )
	{
		...
		SetSkyObjectQuadVertical(&m_Faces[SKYBOX_FACE_FRONT].m_SkyObjectQuadVector, v2QuadPoints);
		m_Faces[SKYBOX_FACE_FRONT].m_strfacename = "front";

		//// Face 1: BACK
		v2QuadPoints[0] = D3DXVECTOR2(-1.0f, 1.0f);
		v2QuadPoints[1] = D3DXVECTOR2(1.0f, 1.0f);
		SetSkyObjectQuadVertical(&m_Faces[SKYBOX_FACE_BACK].m_SkyObjectQuadVector, v2QuadPoints);
		m_Faces[SKYBOX_FACE_BACK].m_strfacename = "back";

		//// Face 2: LEFT
		v2QuadPoints[0] = D3DXVECTOR2(-1.0f, -1.0f);
		v2QuadPoints[1] = D3DXVECTOR2(-1.0f, 1.0f);
		SetSkyObjectQuadVertical(&m_Faces[SKYBOX_FACE_LEFT].m_SkyObjectQuadVector, v2QuadPoints);
		m_Faces[SKYBOX_FACE_LEFT].m_strfacename = "left";

		//// Face 3: RIGHT
		v2QuadPoints[0] = D3DXVECTOR2(1.0f, 1.0f);
		v2QuadPoints[1] = D3DXVECTOR2(1.0f, -1.0f);
		SetSkyObjectQuadVertical(&m_Faces[SKYBOX_FACE_RIGHT].m_SkyObjectQuadVector, v2QuadPoints);
		m_Faces[SKYBOX_FACE_RIGHT].m_strfacename = "right";
		...
		SetSkyObjectQuadHorizon(&m_Faces[SKYBOX_FACE_TOP].m_SkyObjectQuadVector, v3QuadPoints);
		m_Faces[SKYBOX_FACE_TOP].m_strfacename = "top";
		...
		v3QuadPoints[2] = D3DXVECTOR3(-1.0f, -1.0f, -1.0f);
		v3QuadPoints[3] = D3DXVECTOR3(1.0f, -1.0f, -1.0f);
		SetSkyObjectQuadHorizon(&m_Faces[SKYBOX_FACE_BOTTOM].m_SkyObjectQuadVector, v3QuadPoints);
		m_Faces[SKYBOX_FACE_BOTTOM].m_strfacename = "bottom";
	...
	else if( m_ucRenderMode == CSkyObject::SKY_RENDER_MODE_TEXTURE )
	{
		...
		SetSkyObjectQuadHorizon(&m_Faces[SKYBOX_FACE_FRONT].m_SkyObjectQuadVector, v3QuadPoints);
		m_Faces[SKYBOX_FACE_FRONT].m_strfacename = "front";
		...
		SetSkyObjectQuadHorizon(&m_Faces[SKYBOX_FACE_BACK].m_SkyObjectQuadVector, v3QuadPoints);
		m_Faces[SKYBOX_FACE_BACK].m_strfacename = "back";
		...
		SetSkyObjectQuadHorizon(&m_Faces[SKYBOX_FACE_LEFT].m_SkyObjectQuadVector, v3QuadPoints);
		m_Faces[SKYBOX_FACE_LEFT].m_strfacename = "left";
		...
		SetSkyObjectQuadHorizon(&m_Faces[SKYBOX_FACE_RIGHT].m_SkyObjectQuadVector, v3QuadPoints);
		m_Faces[SKYBOX_FACE_RIGHT].m_strfacename = "right";
...
void CSkyBox::SetSkyColor(const TVectorGradientColor & c_rColorVector, const TVectorGradientColor & c_rNextColorVector, int32_t lTransitionTime)
{
	...
	TSkyObjectFace & aFaceTop = m_Faces[SKYBOX_FACE_TOP];
	ulVectorGradientColornum = 0;
	...
	TSkyObjectFace & aFaceBottom = m_Faces[SKYBOX_FACE_BOTTOM];
...
void CSkyBox::StartTransition()
{
	...
	for (uint8_t ucj = 0; ucj < SKYBOX_FACE_MAX_NUM; ++ucj)
...
void CSkyBox::Update()
{
	...
	bool bResult = false;
	for (uint8_t uci = 0; uci < SKYBOX_FACE_MAX_NUM; ++uci)
...
void CSkyBox::Render()
{
	...
	if( m_ucRenderMode == CSkyObject::SKY_RENDER_MODE_TEXTURE )
	{
		...
		for (uint32_t i = 0; i < SKYBOX_FACE_MAX_NUM; ++i)
		{
