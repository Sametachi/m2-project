...
void CGraphicMarkInstance::Load()
{
	...
	CResource* pResource = CResourceManager::GetInstance()->LoadResource<CResource>(GetImageFileName().c_str());

	if (!pResource)
	{
		SysLog("CGraphicMarkinstance::Load - [{0}] NOT EXIST", GetImageFileName().c_str());
		return;
	}
