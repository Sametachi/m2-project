...
void CScreen::RenderCircle3d(float fx, float fy, float fz, float fRadius, int32_t iStep)
{
	...
	const D3DXMATRIX & c_rmatInvView = CCameraManager::GetInstance()->GetCurrentCamera()->GetBillboardMatrix();
...
{
	const D3DXVECTOR3& c_rv3Eye=CCameraManager::GetInstance()->GetCurrentCamera()->GetEye();
	const D3DXVECTOR3& c_rv3View=CCameraManager::GetInstance()->GetCurrentCamera()->GetView();
