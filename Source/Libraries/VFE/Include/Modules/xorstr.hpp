...
namespace NXorStringFuncs
{
    ...
    {
        return static_cast<unsigned>(1013904223ULL + 1664525ULL * (Rounds > 0 ? LinearCongruentGenerator(static_cast<unsigned long long>(Rounds) - 1) : Seed));
    }
...
{
public:
    constexpr CXString(std::size_t uiSize, const uint16_t* c_pwEncrypted) :
    ...
    {
        uint16_t wSize = (m_uiSize & 0xFFFF) ^ (m_uiSize >> 16);
