...
{
	CSpeedTreeForestDirectX9::GetInstance()->UpdateSystem(ELTimer_GetMSec() / 1000.0f);
	m_pSpeedTree->SetLodLevel(1.0f);
	CSpeedTreeForestDirectX9::GetInstance()->UpdateCompundMatrix(CCameraManager::GetInstance()->GetCurrentCamera()->GetEye(), ms_matView, ms_matProj);
...
bool CSpeedTreeWrapper::LoadTree(const char * pszSptFile, const uint8_t * c_pbBlock, uint32_t uiBlockSize, uint32_t nSeed, float fSize, float fSizeVariance)
{
	...
	if (!m_pSpeedTree->LoadTree(c_pbBlock, uiBlockSize))
	{
		...
		{
			SysLog("SpeedTreeRT Error: {0}", CSpeedTreeRT::GetCurrentError());
			return false;
		}
	}
	...
	if (m_pSpeedTree->Compute(nullptr, nSeed))
	{
		...
		std::string aa = std::string(pszSptFile);
		std::string bb = std::string(m_pTextureInfo->m_pBranchTextureFilename);
		LoadTexture((CFileNameHelper::GetPath(aa) + CFileNameHelper::NoExtension(bb) + ".dds").c_str(), m_BranchImageInstance);

#ifdef WRAPPER_RENDER_SELF_SHADOWS
		if (m_pTextureInfo->m_pSelfShadowFilename != NULL)
		{
			std::string aaa = std::string(m_pTextureInfo->m_pSelfShadowFilename);
			LoadTexture((CFileNameHelper::GetPath(aa) + CFileNameHelper::NoExtension(aaa) + ".dds").c_str(), m_ShadowImageInstance);
	}
#endif
		if (m_pTextureInfo->m_pCompositeFilename)
		{
			std::string bbb = std::string(m_pTextureInfo->m_pCompositeFilename);
			LoadTexture((CFileNameHelper::GetPath(aa) + CFileNameHelper::NoExtension(bbb) + ".dds").c_str(), m_CompositeImageInstance);
		}
		SetupBuffers();
		bSuccess = true;
	}
	...
	{
		ConsoleLog("\nFatal Error, cannot compute tree [{0}]\n\n", CSpeedTreeRT::GetCurrentError());
	}
...
void CSpeedTreeWrapper::SetupBranchBuffers(void)
{
	...
	if (m_unBranchVertexCount > 1)
	{
		...
		std::memcpy(pIndexBuffer, pBranches->m_pStrips[0], pBranches->m_pStripLengths[0] * sizeof(uint16_t));
...
void CSpeedTreeWrapper::SetupFrondBuffers(void)
{
	...
	if (m_unFrondVertexCount > 1)
	{
		...
		std::memcpy(pIndexBuffer, pFronds->m_pStrips[0], pFronds->m_pStripLengths[0] * sizeof(uint16_t));
	...
	for (uint32_t unLod = 0; unLod < m_usNumLeafLods; ++unLod)
	{
		...
		for (uint32_t unLeaf = 0; unLeaf < usLeafCount; ++unLeaf)
		{
			...
			for (uint32_t unVert = 0; unVert < 6; ++unVert)  
			{
				...
				std::memcpy(pVertex->m_vPosition, &(pLeaf->m_pCenterCoords[unLeaf * 3]), 3 * sizeof(float));
...
#endif
				std::memcpy(pVertex->m_fTexCoords, &(pLeaf->m_pLeafMapTexCoords[unLeaf][anVertexIndices[unVert] * 2]), 2 * sizeof(float));
#ifdef WRAPPER_USE_GPU_WIND
...
CSpeedTreeWrapper * CSpeedTreeWrapper::MakeInstance()
{
	...
	if (pInstance->m_pSpeedTree)
    {
		...
		std::memcpy(pInstance->m_afPos, m_afPos, 3 * sizeof(float));
		std::memcpy(pInstance->m_afBoundingBox, m_afBoundingBox, 6 * sizeof(float));
		pInstance->m_pInstanceOf = this;
		m_vInstances.emplace_back(pInstance);
    }
    else
	{
		ConsoleLog("SpeedTreeRT Error: [{0}]\n", m_pSpeedTree->GetCurrentError());
        delete pInstance;
        pInstance = nullptr;
	}
...
void CSpeedTreeWrapper::RenderLeaves(void) const
{
	...
	{
		const CSpeedTreeRT::SGeometry::SLeaf* pLeaf = (unLeafLevel == 0) ? &m_pGeometryCache->m_sLeaves0 : &m_pGeometryCache->m_sLeaves1;
...
{
	CResource* pResource = CResourceManager::GetInstance()->LoadResource<CResource>(pFilename);
