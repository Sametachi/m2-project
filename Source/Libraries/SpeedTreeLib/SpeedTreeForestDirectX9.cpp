...
CSpeedTreeForestDirectX9::CSpeedTreeForestDirectX9() : m_dwBranchVertexShader(nullptr), m_dwLeafVertexShader(nullptr)
{
}
...
bool CSpeedTreeForestDirectX9::SetRenderingDevice()
{
...
{
	UpdateSystem(CTimer::GetInstance()->GetCurrentSecond());

	if (m_pMainTreeMap.empty())
		return;

	if (!(ulRenderBitVector & Forest_RenderToShadow) && !(ulRenderBitVector & Forest_RenderToMiniMap))
		UpdateCompundMatrix(CCameraManager::GetInstance()->GetCurrentCamera()->GetEye(), ms_matView, ms_matProj);
...
void CSpeedTreeForestDirectX9::RenderShadows(uint32_t ulRenderBitVector)
{
	UpdateSystem(CTimer::GetInstance()->GetCurrentSecond());
