...
#define POLY_CEIL POLY_MAXVALUE + 29
...
struct CSymTable 
{
	CSymTable(int32_t aTok, const std::string& aStr);
	virtual ~CSymTable() = default;
...
public:
	...
	int32_t Analyze(const char* pszStr = NULL);
	double Eval() const;
...
protected:
	int32_t my_irandom(double start, double end) const;
	double my_frandom(double start, double end) const;

	void init();
	int32_t emplace(const std::string& s, int32_t tok);
