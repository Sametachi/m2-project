...
bool CGrannyModelInstance::Intersect(const D3DXMATRIX* c_pMatrix,
	float* /*pu*/, float* /*pv*/, float* pt, D3DXVECTOR3* pScaleMatrix)
{
	if (!m_modelInstance)
		return false;
...
void CGrannyModelInstance::GetBoundBox(D3DXVECTOR3* vtMin, D3DXVECTOR3* vtMax, D3DXVECTOR3* scale)
{
	if (!m_modelInstance)
		return;
	...
	for (int32_t m = 0; m < meshCount; ++m)
	{
		...
		for (int32_t b = 0; b < pgrnMesh->BoneBindingCount; ++b)
		{
			...
			D3DXMATRIX mat(GrannyGetWorldPose4x4(__GetWorldPosePtr(), boneIndices[b]));
			if (scale) 
			{
				D3DXMATRIX matScale;
				D3DXMATRIX* s = D3DXMatrixScaling(&matScale, scale->x, scale->y, scale->z);
				mat *= matScale;
			}

			MakeBoundBox(&BoundBox, (float*)&mat, rgrnBoneBinding.OBBMin, rgrnBoneBinding.OBBMax, vtMin, vtMax);
		}
	}
}
