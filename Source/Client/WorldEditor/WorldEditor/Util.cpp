...
void CreateBitmapButton(CButton * pButton, int32_t iBitmapID, CBitmap & rBitmap)
{
...
void RepositioningWindow(CPageCtrl & rPageCtrl, int32_t ixTemporarySpace, int32_t iyTemporarySpace)
{
	CRect Rect;
...
void CreateHighColorImageList(int32_t iBitmapID, CImageList * pImageList)
{
	CBitmap Bitmap;
...
void SetDialogFloatText(HWND hWnd, int32_t iID, float fData)
{
	char szString[64 + 1];
...
void SetDialogIntegerText(HWND hWnd, int32_t iID, int32_t iData)
{
	char szString[64 + 1];
...
float GetDialogFloatText(HWND hWnd, int32_t iID)
{
	char szString[64 + 1];
...
int32_t GetDialogIntegerText(HWND hWnd, int32_t iID)
{
	char szString[64 + 1];
	GetDlgItemText(hWnd, iID, szString, 64);
...
const char * GetProgramExcutingPath(const char * c_szFullFileName)
{
	...
	for (uint32_t i = 0; i < strlen(g_szProgramPath); ++i)
	{
		char src, dst;
...
void RenderBackGroundCharacter(float fx, float fy, float fz)
{
	...
	if (!g_isCharacterData)
	{
		...
		if (pCharacter->IsEmpty())
		{
			...
			LogBoxf("%s character does not exist", pCharacter->GetFileName());
			return;
		}
		...
		{
			g_isCharacterFile = FALSE;
			LogBoxf("%s motion does not exist", pMotion->GetFileName());
			return;
		}
...
void RenderBackGroundTile()
{
	...
	if (!g_isTileData)
	{
		...
		{
			LogBox("background_stone.gr2 does not exist");
			return;
		}
