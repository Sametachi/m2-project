...
#ifdef ROTATION_LEFT2RIGHT
	if(m_ClickedRightButton)
#else
...
//		if (RotationMode == m_Mode)
		if (false == bLockRotation)
		{
			...
			fPitch = max( - pCurrentCamera->GetPitch() - 89.9f, min( 89.9f - pCurrentCamera->GetPitch(),  - (m_ClickedRightButtonPoint.y - Mouse.y) * 0.3f));
...
void CViewportManager::SetScreenStatus(int32_t iWidth, int32_t iHeight)
{
	m_iWidth = iWidth;
	m_iHeight = iHeight;
}

void CViewportManager::SetCursorPosition(int32_t ix, int32_t iy)
{
	m_ixCursor = ix;
	m_iyCursor = iy;
}
