...
int32_t iStartUpPage = ID_VIEW_MAP;
//int32_t iStartUpPage = ID_VIEW_OBJECT;
//int32_t iStartUpPage = ID_VIEW_EFFECT;
...
int32_t CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
BOOL CMainFrame::CreateStatusBar()
{
	...
	m_wndStatusBar.GetPaneInfo(WE_FRAME_INFO, uiID, uiStyle, iWidth);
	m_wndStatusBar.SetPaneInfo(WE_FRAME_INFO, uiID, uiStyle, 350);
	m_wndStatusBar.SetPaneText(WE_FRAME_INFO, "N/A");

	m_wndStatusBar.GetPaneInfo(WE_FRAME_PIXEL, uiID, uiStyle, iWidth);
	m_wndStatusBar.SetPaneInfo(WE_FRAME_PIXEL, uiID, uiStyle, 150);

	m_wndStatusBar.GetPaneInfo(WE_FRAME_AREA, uiID, uiStyle, iWidth);
	m_wndStatusBar.SetPaneInfo(WE_FRAME_AREA, uiID, uiStyle, 100);
	return TRUE;
}
...
void CMainFrame::UpdateStatusBar(float fx, float fy, float fz)
{
	...
	pStatusBar->SetPaneText(WE_FRAME_PIXEL, strPixelPosition, TRUE);
	pStatusBar->SetPaneText(WE_FRAME_AREA, strAreaPosition, TRUE);
}

void CMainFrame::SetStatusInfo(CString strMapAutoSave)
{
	CStatusBar* pStatusBar = &m_wndStatusBar;

	CString strMapAutoSave_Timestamped;

	time_t currentTime; struct tm* localTime;
	time(&currentTime); localTime = localtime(&currentTime);
	strMapAutoSave_Timestamped.Format("[%02d:%02d:%02d] %s", localTime->tm_hour, localTime->tm_min, localTime->tm_sec, strMapAutoSave);

	pStatusBar->SetPaneText(WE_FRAME_INFO, strMapAutoSave_Timestamped, TRUE);
}

/////////////////////////////////////////////////////////////////////////////
// CMainFrame message handlers

BOOL CMainFrame::OnCmdMsg(UINT nID, int32_t nCode, void* pExtra, AFX_CMDHANDLERINFO* pHandlerInfo)
{
...
void CMainFrame::UpdateMapControlBar()
{
	...
	int32_t iMode = m_wndMapCtrlBar.UpdatePage();
