...
int32_t CWorldEditorView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	if (CView::OnCreate(lpCreateStruct) == -1)
		return -1;

	m_pDlgPerspective = std::make_unique<CDlgPerspective>();
	m_pDlgPerspective->Create(IDD_DIALOG_PERSPECTIVE, this);
	m_pDlgPerspective->SetWindowPos(NULL, 0, 0, 0, 0, TRUE);
	m_pDlgPerspective->SetFOV(globals::dft::WINDOW_FOV_SIZE);
...
{
	CResource * pResource = CResourceManager::Instance().GetTypeResourcePointer("Tahoma:15.fnt");
...
/////////////////////////////////////////////////////////////////////////////////////////
// Camera Start
void CWorldEditorView::UpdateCursorPosition(int32_t ix, int32_t iy)
{
	CRect Rect;
	GetWindowRect(&Rect);
...
*/
void CWorldEditorView::RunKeyDown(int32_t iChar)
{
	CWorldEditorDoc * pDoc = GetDocument();
	CSceneBase * pScene = pDoc->GetActiveScene();
	pScene->KeyDown(iChar);
}
void CWorldEditorView::RunKeyUp(int32_t iChar)
{
	CWorldEditorDoc * pDoc = GetDocument();
	CSceneBase * pScene = pDoc->GetActiveScene();
	pScene->KeyUp(iChar);
}
void CWorldEditorView::RunMouseMove(int32_t x, int32_t y)
{
	UpdateFrustum();
...
BOOL CWorldEditorView::RunMouseWheel(int16_t zDelta)
{
	UpdateFrustum();
...
void CWorldEditorView::OnSize(UINT nType, int32_t cx, int32_t cy)
{
	if (!m_bInitialized)
	{
...
void CWorldEditorView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	switch (nChar)
	{
		...
		case VK_F5:
			{
				CPythonLauncher::RunFile("WorldEditorRemix.py");
			}
			break;
		case VK_F4:
			{
...
BOOL CWorldEditorView::OnMouseWheel(UINT nFlags, int16_t zDelta, CPoint pt)
{
...
void CWorldEditorView::Process()
{
	...
	//////////////////////////////////////////////////////////////////////////////
	static int32_t lastTime = timeGetTime();
	int32_t lElapsedTime = timeGetTime()-lastTime;
	if (lElapsedTime < 16)
	{
