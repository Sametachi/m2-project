...
void CObjectModelCollisionSubPage::UpdateUI()
{
	...
	switch (pCollisionData->iCollisionType)
	{
		...
		case NRaceData::COLLISION_TYPE_ATTACKING:
			assert(!"Collision attack type has been removed");
			break;
...
void CObjectModelCollisionSubPage::UpdateSphereData()
{
	...
	m_ctrlSphereSize.SetPos(int32_t(pSphereData->fRadius));
	SetDialogIntegerText(GetSafeHwnd(), IDC_COLLISION_PAGE_SIZE_PRINT, int32_t(pSphereData->fRadius));
...
void CObjectModelCollisionSubPage::SetSphereSize(uint32_t dwAttachingIndex, uint32_t dwSphereIndex, int32_t iSize)
{
	TSphereData * pSphereData;
...
void CObjectModelCollisionSubPage::SetSphereSizeType(uint32_t dwAttachingIndex, uint32_t dwSphereIndex, uint32_t dwSizeType)
{
	...
	const int32_t c_iBodyCollisionSizeType[5] =
	{
		40, 60, 80, 100, 120
	};
	const int32_t c_iDefendingCollisionSizeType[5] =
	{
		50, 70, 90, 110, 130
	};
	...
	switch (pCollisionData->iCollisionType)
	{
		...
		default:
			assert(!"CObjectModelCollisionSubPage::SetSphereSizeType - Wrong Type");
			break;
	}
...
{
	int32_t iSize = m_ctrlSphereSize.GetPos();
	SetSphereSize(m_dwAttachingIndex, m_dwSphereIndex, iSize);
