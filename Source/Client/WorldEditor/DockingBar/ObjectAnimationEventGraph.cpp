...
{
	CGraphicText * pText = (CGraphicText *)CResourceManager::Instance().GetTypeResourcePointer("Tahoma:11.fnt");
...
void CObjectAnimationEventGraph::SetCurrentHitDataIndex(int32_t iIndex)
{
...
{
	int32_t iMousePosY = point.y;
	if (m_isFrameStep)
	{
		const float c_fFrameTime = 1.0f / 30.0f;
		int32_t iStepSize = int32_t(float(m_WindowRect.Height()) * (c_fFrameTime/m_fDuration));
	...
	// Combo
	if (m_pMotionData->IsComboInputTimeData())
	for (int32_t i = COMBO_NODE_MAX_NUM-1; i >= 0; --i)
	{
