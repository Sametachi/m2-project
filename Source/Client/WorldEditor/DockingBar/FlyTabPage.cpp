...
void CFlyTabPage::CallBack()
{
	...
	int32_t iIndex = ms_pThis->m_ctrlList.GetCurSel();
	COLORREF Color;
...
void CFlyTabPage::RebuildAttachData()
{
	...
	for(int32_t i=0;i<pData->GetAttachDataCount();i++)
	{
		char szBuffer[1024];
...
void CFlyTabPage::SelectAttachIndex(int32_t iIndex)
{
	// Setting UI
	...
	else
	{
		...
		m_ctrlTailColor.SetColor(RGB((uint8_t)(rfad.dwTailColor>>16),(uint8_t)(rfad.dwTailColor>>8),(uint8_t)(rfad.dwTailColor)));
		...
		switch(rfad.iFlyType)
		{
			...
			case CFlyingData::FLY_ATTACH_TYPE_MULTI_LINE:
				...
				GetDlgItem(IDC_FLY_ARG1)->SetWindowText("axis of rotation");
...
void CFlyTabPage::OnFlyTabNewEffect()
{
	...
	int32_t idx = (pData->AttachFlyEffect(CFlyingData::FLY_ATTACH_TYPE_LINE, "", 0.0f, 0.0f, 0.0f));
	char szBuffer[1024];
...
{
	int32_t iIndex = m_ctrlList.GetCurSel();
	if (iIndex == -1)
		return;
...
void CFlyTabPage::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	if (IDC_FLY_TAB_TAIL_ALPHA == m_iSliderIndex)
	{
		...
		rfad.dwTailColor |= (uint8_t)(m_ctrlTailAlpha.GetPos()*255/100)<<24;
...
{
	uint32_t dwFlag = OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT; // @fixme131 OFN_ALLOWMULTISELECT wrong flag
