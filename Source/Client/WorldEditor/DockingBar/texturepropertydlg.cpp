...
#define PostUpdate(iEditControl) \
	...
	{ \
		...
		int32_t len = pEdit->GetWindowTextLength(); \
...
BEGIN_MESSAGE_MAP(CTexturePropertyDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON_CHANGE_TEX, OnBtnChangeTexture)
	ON_BN_CLICKED(IDC_BUTTON_REMOVE_TEX, OnBtnRemoveTexture)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CTexturePropertyDlg::OnBtnChangeTexture()
{
	CWorldEditorApp * pApplication = (CWorldEditorApp *)AfxGetApp();
	CMapManagerAccessor * pMapManagerAccessor = pApplication->GetMapManagerAccessor();
	...
	{
		c_rTerrainTexture.stFilename = FileOpener.GetPathName().GetString();
		pTextureSet->SetTexture(m_lTexNum,
							 c_rTerrainTexture.stFilename.c_str(),
							 c_rTerrainTexture.UScale,
							 c_rTerrainTexture.VScale,
							 c_rTerrainTexture.UOffset,
							 c_rTerrainTexture.VOffset,
							 c_rTerrainTexture.bSplat,
							 c_rTerrainTexture.Begin,
							 c_rTerrainTexture.End,
...
void CTexturePropertyDlg::OnBtnRemoveTexture()
{
	CWorldEditorApp * pApplication = (CWorldEditorApp *)AfxGetApp();
	CMapManagerAccessor * pMapManagerAccessor = pApplication->GetMapManagerAccessor();
	if (!pMapManagerAccessor)
	...
	{
		if (!pMapManagerAccessor->RemoveTerrainTexture(m_lTexNum))
			LogBoxf("Failed to delete %s from the list.", c_rTerrainTexture.stFilename.c_str());
		// reload textures p1
		ResetTextures();
		// workaround to close the dialog
		CDialog::EndDialog(IDCANCEL); // CDialog::OnCancel();
	}
}
...
BOOL CTexturePropertyDlg::OnInitDialog()
{
	...
	m_SliderUScale.SetPos((int32_t) m_fUScale * 255);
	m_SliderVScale.SetPos((int32_t) m_fVScale * 255);
	m_SliderUOffset.SetPos((int32_t) m_fVOffset * 65535);
	m_SliderVOffset.SetPos((int32_t) m_fVOffset * 65535);
...
// SetXX
//
void CTexturePropertyDlg::SetUScale(int32_t iUScale)
{
	Prolog();
...
void CTexturePropertyDlg::SetVScale(int32_t iVScale)
{
	Prolog();
...
void CTexturePropertyDlg::SetUOffset(int32_t iUOffset)
{
	Prolog();
...
void CTexturePropertyDlg::SetVOffset(int32_t iVOffset)
{
	Prolog();
...
void CTexturePropertyDlg::SetBegin(int32_t iBegin)
{
	Prolog();
...
void CTexturePropertyDlg::SetEnd(int32_t iEnd)
{
	Prolog();
...
void CTexturePropertyDlg::OnUpdateEditBegin()
{
	...
	UpdateData(FALSE);
	c_rTerrainTexture.Begin = (uint16_t) (m_fEditBegin / m_fHeightScale * 100);
...
void CTexturePropertyDlg::OnUpdateEditEnd()
{
	...
	UpdateData(FALSE);
	c_rTerrainTexture.End = (uint16_t) (m_fEditEnd / m_fHeightScale * 100);
...
void CTexturePropertyDlg::OnUpdateEditUOffset()
{
	...
	m_SliderUOffset.SetPos((int32_t) (m_fEditUOffset * 65535));
...
void CTexturePropertyDlg::OnUpdateEditUScale()
{
	...
	m_SliderUScale.SetPos((int32_t)(m_fEditUScale * 256));
...
void CTexturePropertyDlg::OnUpdateEditVOffset()
{
	...
	m_SliderVOffset.SetPos((int32_t) (m_fEditVOffset * 65535));
...
void CTexturePropertyDlg::OnUpdateEditVScale()
{
	...
	m_SliderVScale.SetPos((int32_t) (m_fEditVScale * 256));
