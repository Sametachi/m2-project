...
void CObjectAnimationPage::__UpdateHitDataList()
{
	...
	if (c_pHitData)
	{
		...
		m_ctrlWeaponLength.SetPos(int32_t(c_pHitData->fWeaponLength));
...
void CObjectAnimationPage::UpdateLoopCount()
{
	...
	int32_t iLoopCount = pObjectData->GetMotionDataPointer()->GetLoopCount();
	if (0 == iLoopCount)
	{
...
void CObjectAnimationPage::RecvLocalTime(float fDuration, float fLocalTime)
{
	...
	int32_t iPosition = int32_t((fLocalTime / fDuration) * float(iMax - iMin)) + iMin;
...
void CObjectAnimationPage::OnLoadAnimation()
{
	...
	CHECK_RETURN(!pObjectData->isModelThing(), "First, load your model");
	...
	{
		CHECK_RETURN(!CResourceManager::Instance().IsFileExist(FileOpener.GetPathName()), "There are no files");
...
void CObjectAnimationPage::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	int32_t iWeaponLength = m_ctrlWeaponLength.GetPos();
	...
	int32_t iHitLimitCount = m_ctrlHitLimit.GetPos();
...
void CObjectAnimationPage::OnSaveAnimationScript()
{
	...
	else
	{
		...
		int32_t pos = stScriptFileName.find_last_of(".", stScriptFileName.length());
...
void CObjectAnimationPage::OnCheckLimitLoopEnable()
{
	...
	{
		int32_t iLoopCount = GetDialogIntegerText(GetSafeHwnd(), IDC_OBJECT_ANIMATION_LOOP_COUNT);
...
void CObjectAnimationPage::OnToggleFrameStep()
{
	if (IsDlgButtonChecked(IDC_OBJECT_ANIMATION_FRAME_STEP))
	{
		...
		int32_t iStep = fDuration / c_fFrameTime;
		...
		{
			int32_t iPos = int32_t(float(i) / float(iStep) * float(iMax - iMin)) + iMin;
