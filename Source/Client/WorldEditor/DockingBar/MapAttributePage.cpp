...
BOOL CMapAttributePage::Create(CWnd * pParent)
{
	...
	CheckDlgButton(IDC_CHECK_SHOW_ALLMONSTERAREAINFO, globals::dft::VIEW_MONSTER_AREA_INFO); // @warme103 incredible lag if set to 1
	...
	if (globals::dft::ATTR_SLIDER_REMOVAL)
	{
		m_SliderAttrNum.EnableWindow(FALSE);
		m_SliderAttrNum.ShowWindow(SW_HIDE);
		SetDlgItemText(IDC_ATTR_NUM, "");
	}
...
void CMapAttributePage::Initialize()
{
	...
	uint32_t dwMonsterAreaInfoCount = pMapManagerAccessor->GetMonsterAreaInfoCount();
	...
	uint32_t dwNonPlayerCount = rNonPlayerCharacterInfo.GetNonPlayerCount();
	for (uint32_t dwNonPlayerMapIndex = 0; dwNonPlayerMapIndex < dwNonPlayerCount; ++dwNonPlayerMapIndex)
	{
		int32_t dwLevel = rNonPlayerCharacterInfo.GetLevelByMapIndex(dwNonPlayerMapIndex);
		uint8_t byMonsterRank = rNonPlayerCharacterInfo.GetRankByMapIndex(dwNonPlayerMapIndex);
		std::string strMonsterName = rNonPlayerCharacterInfo.GetNameByMapIndex(dwNonPlayerMapIndex);
		uint32_t dwMonsterVID = rNonPlayerCharacterInfo.GetVIDByMapIndex(dwNonPlayerMapIndex);

		_snprintf(szMonsterName, 128, "%d:%s(%02d):%d", byMonsterRank,	strMonsterName.c_str(), dwLevel, dwMonsterVID);
		m_ComboMonsterName.AddString(szMonsterName);
		m_ComboSelectedMonsterName.AddString(szMonsterName);
	}

	uint32_t dwNPCGroupMapCount = rNonPlayerCharacterInfo.GetNPCGroupCount();
	for (uint32_t dwNPCGroupMapIndex = 0; dwNPCGroupMapIndex < dwNPCGroupMapCount; ++dwNPCGroupMapIndex)
	{
		std::string strNPCGroupName = rNonPlayerCharacterInfo.GetNPCGroupNameByMapIndex(dwNPCGroupMapIndex);
		uint32_t dwNPCGroupID = rNonPlayerCharacterInfo.GetNPCGroupIDByMapIndex(dwNPCGroupMapIndex);
		std::string strNPCGroupLeaderName = rNonPlayerCharacterInfo.GetNPCGroupLeaderNameByMapIndex(dwNPCGroupMapIndex);
		uint32_t dwNPCGroupFollowerCount = rNonPlayerCharacterInfo.GetNPCGroupFollowerCountByMapIndex(dwNPCGroupMapIndex);
	...
	{
		m_ComboMonsterName.AddString("No Monsters");
		m_ComboMonsterName.SelectString(0, "No Monsters");
	}
}
...
void CMapAttributePage::UpdateBrushMode()
{
	...
	doc->SetEditingMapAttribute(TRUE == IsDlgButtonChecked(IDC_CHECK_CELLEEDIT));
...
uint8_t CMapAttributePage::GetAttributeFlag()
{
	int32_t iAttrNum = m_SliderAttrNum.GetPos();
	...
	int32_t aiChecked[8] =
	{
		...
		(globals::dft::ATTR_SLIDER_REMOVAL) ? 0 : (iAttrNum % 2),
		(globals::dft::ATTR_SLIDER_REMOVAL) ? 0 : ((iAttrNum / 2) % 2),
		(globals::dft::ATTR_SLIDER_REMOVAL) ? 0 : ((iAttrNum / 4) % 2),
		(globals::dft::ATTR_SLIDER_REMOVAL) ? 0 : ((iAttrNum / 8) % 2)
	};
...
void CMapAttributePage::UpdateNewMonsterSelect()
{
	...
	int32_t iMonsterNameEndPos = strMonsterNameWithTypeAndRankAndLevelRange.find_first_of("(", 0);
	int32_t iMonsterVIDBeginPos = strMonsterNameWithTypeAndRankAndLevelRange.find_first_of(":", iMonsterNameEndPos) + 1;
	...
	uint32_t dwMonsterVID = (uint32_t) atoi(strMonsterVID.c_str());
...
void CMapAttributePage::UpdateSelectedMonsterSelect()
{
	...
	std::string strMonsterNameWithTypeAndRankAndLevelRange = strSelectedMonsterName.GetBuffer(128);
	int32_t iMonsterNameBeginPos = strMonsterNameWithTypeAndRankAndLevelRange.find_first_of(":", 0) + 1;
	int32_t iMonsterNameEndPos = strMonsterNameWithTypeAndRankAndLevelRange.find_first_of("(", 0);
	int32_t iMonsterVIDBeginPos = strMonsterNameWithTypeAndRankAndLevelRange.find_first_of(":", iMonsterNameEndPos) + 1;
	std::string strMonsterType = strMonsterNameWithTypeAndRankAndLevelRange.substr(0, 1);
	...
	{
		//G:%s(%s,%d):%d
		int32_t iMonsterGroupLeaderNameBeginPos = strMonsterNameWithTypeAndRankAndLevelRange.find_first_of("(", 0) + 1;
		int32_t iMonsterGroupLeaderNameEndPos = strMonsterNameWithTypeAndRankAndLevelRange.find_first_of(",", 0);
		...
		int32_t iMonsterGroupFollowerCountBeginPos = strMonsterNameWithTypeAndRankAndLevelRange.find_first_of(",", 0) + 1;
		int32_t iMonsterGroupFollowerCountEndPos = strMonsterNameWithTypeAndRankAndLevelRange.find_first_of(")", iMonsterGroupFollowerCountBeginPos);
		...
		uint32_t dwMonsterGroupFollowerCount = (uint32_t) atoi(strMonsterGroupFollowerCount.c_str());
...
void CMapAttributePage::UpdateSelectedMonsterAreainfo()
{
	...
	case CMonsterAreaInfo::MONSTERAREAINFOTYPE_MONSTER:
		{
			...
			{
				uint32_t dwMonsterVID = m_pSelectedMonsterAreaInfo->GetMonsterVID();
				if (dwMonsterVID != 0)
				{
			...
			uint32_t dwVID = m_pSelectedMonsterAreaInfo->GetMonsterVID();
			int32_t dwLevel = rNonPlayerCharacterInfo.GetLevelByVID(dwVID);

			_snprintf(szMonsterName, 128, "%d:%s(%02d):%d",
	...
	case CMonsterAreaInfo::MONSTERAREAINFOTYPE_GROUP:
		{
			...
			if (0 == strMonsterName.compare("Noname"))
			{
				uint32_t dwMonsterGroupID = m_pSelectedMonsterAreaInfo->GetMonsterGroupID();
			...
			uint32_t dwMonsterGroupFollowerCount = rNonPlayerCharacterInfo.GetNPCGroupFollowerCountByGroupID(dwMonsterGroupID);
	...
	uint32_t dwMonsterCount = m_pSelectedMonsterAreaInfo->GetMonsterCount();
...
void CMapAttributePage::OnChangeEditMonsterareainfoMonstercount()
{
	...
	uint32_t dwNewMonsterCount = (uint32_t) atoi(m_strNewMonsterCount);
...
void CMapAttributePage::OnChangeEditSelectedmonsterareainfoMonstercount()
{
	...
	UpdateData();
	uint32_t dwSelctedMonsterCount = (uint32_t) atoi(m_strSelectedMonsterCount);
