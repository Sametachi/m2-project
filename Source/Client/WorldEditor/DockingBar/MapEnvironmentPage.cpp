...
BEGIN_MESSAGE_MAP(CMapEnvironmentPage, CPageCtrl)
	...
	// new stuff
	ON_BN_CLICKED(IDC_BUTTON_RECALCULATESHADOW, OnCheckRecalculateShadow)
	ON_BN_CLICKED(IDC_MAP_ENVIRONMENT_LOAD_BACKGROUND_MUSIC, OnCheckBackGroundPlayMusic)
	ON_BN_CLICKED(IDC_MAP_ENVIRONMENT_LOAD_BACKGROUND_MUSIC2, OnCheckBackGroundPlaySound)
	ON_BN_CLICKED(IDC_MAP_ENVIRONMENT_LOAD_BACKGROUND_MUSIC3, OnCheckBackGroundPlayFadeIn)
	ON_BN_CLICKED(IDC_MAP_ENVIRONMENT_LOAD_BACKGROUND_MUSIC4, OnCheckBackGroundPlayFadeOut)
	ON_BN_CLICKED(IDC_MAP_ENVIRONMENT_LOAD_BACKGROUND_MUSIC5, OnCheckBackGroundStopMusic3D)
	ON_BN_CLICKED(IDC_MAP_ENVIRONMENT_LOAD_BACKGROUND_MUSIC6, OnCheckBackGroundStopFadeOut)
#ifdef ENABLE_WIND_OPTION
	ON_BN_CLICKED(IDC_MAP_ENVIRONMENT_WIND_STRENGTH_ENABLE, OnEnableWind)
#endif
END_MESSAGE_MAP()
...
void CMapEnvironmentPage::UpdateUI()
{
	...
	COLORREF MaterialDiffuseColor = RGB(uint16_t(c_pEnvironmentData->Material.Diffuse.r*255.0f),
									 uint16_t(c_pEnvironmentData->Material.Diffuse.g*255.0f),
									 uint16_t(c_pEnvironmentData->Material.Diffuse.b*255.0f));
	COLORREF MaterialAmbientColor = RGB(uint16_t(c_pEnvironmentData->Material.Ambient.r*255.0f),
									 uint16_t(c_pEnvironmentData->Material.Ambient.g*255.0f),
									 uint16_t(c_pEnvironmentData->Material.Ambient.b*255.0f));
	COLORREF MaterialEmissiveColor = RGB(uint16_t(c_pEnvironmentData->Material.Emissive.r*255.0f),
									 uint16_t(c_pEnvironmentData->Material.Emissive.g*255.0f),
									 uint16_t(c_pEnvironmentData->Material.Emissive.b*255.0f));
	COLORREF LightDiffuseColor = RGB(uint16_t(c_pEnvironmentData->DirLights[ENV_DIRLIGHT_BACKGROUND].Diffuse.r*255.0f),
									 uint16_t(c_pEnvironmentData->DirLights[ENV_DIRLIGHT_BACKGROUND].Diffuse.g*255.0f),
									 uint16_t(c_pEnvironmentData->DirLights[ENV_DIRLIGHT_BACKGROUND].Diffuse.b*255.0f));
	COLORREF LightAmbientColor = RGB(uint16_t(c_pEnvironmentData->DirLights[ENV_DIRLIGHT_BACKGROUND].Ambient.r*255.0f),
									 uint16_t(c_pEnvironmentData->DirLights[ENV_DIRLIGHT_BACKGROUND].Ambient.g*255.0f),
									 uint16_t(c_pEnvironmentData->DirLights[ENV_DIRLIGHT_BACKGROUND].Ambient.b*255.0f));
	COLORREF FogColor		   = RGB(uint16_t(c_pEnvironmentData->FogColor.r*255.0f),
									 uint16_t(c_pEnvironmentData->FogColor.g*255.0f),
									 uint16_t(c_pEnvironmentData->FogColor.b*255.0f));
	COLORREF FilteringColor    = RGB(uint16_t(c_pEnvironmentData->FilteringColor.r*255.0f),
									 uint16_t(c_pEnvironmentData->FilteringColor.g*255.0f),
									 uint16_t(c_pEnvironmentData->FilteringColor.b*255.0f));
	...
	m_ctrlWindStrength.SetPos(int32_t(c_pEnvironmentData->fWindStrength * 100.0f)); // @fixme116 (missing *100.0f)
	m_ctrlWindRandom.SetPos(int32_t(c_pEnvironmentData->fWindRandom * 100.0f)); // @fixme116 (missing *100.0f)
	m_ctrlFilteringAlpha.SetPos(int32_t(c_pEnvironmentData->FilteringColor.a * 100.0f));
#ifdef ENABLE_WIND_OPTION
	m_ctrlWindStrength.EnableWindow(c_pEnvironmentData->bWindEnable);
	m_ctrlWindRandom.EnableWindow(c_pEnvironmentData->bWindEnable);
	CheckDlgButton(IDC_MAP_ENVIRONMENT_WIND_STRENGTH_ENABLE, c_pEnvironmentData->bWindEnable);
#endif

	m_ctrlFogNearDistance.SetLimitText(16);
	m_ctrlFogFarDistance.SetLimitText(16);

#ifdef CWE_INFO_ENVIRONMENT_FIX
	CMapOutdoor& rMapOutdoor = pMapManagerAccessor->GetMapOutdoorRef();
	SetDlgItemText(IDC_MAP_ENVIRONMENT_SCRIPT_NAME_PRINT, (rMapOutdoor.GetEnvironmentDataName().empty()) ? "N/A" : rMapOutdoor.GetEnvironmentDataName().c_str());

	static char chVersion[32 + 1];
	snprintf(chVersion, sizeof(chVersion), "%.2f", c_pEnvironmentData->fVersion);
	SetDlgItemText(IDC_MAP_ENVIRONMENT_SCRIPT_VERSION_PRINT, (rMapOutdoor.GetEnvironmentDataName().empty()) ? "N/A" : chVersion);
#endif
	...
	SetDlgItemText(IDC_MAP_ENVIRONMENT_CLOUD_TEXTURE_FILE_NAME2, c_pEnvironmentData->strSkyBoxFaceFileName[SKYBOX_FACE_FRONT].c_str()); // front
	SetDlgItemText(IDC_MAP_ENVIRONMENT_CLOUD_TEXTURE_FILE_NAME4, c_pEnvironmentData->strSkyBoxFaceFileName[SKYBOX_FACE_BACK].c_str()); // back
	SetDlgItemText(IDC_MAP_ENVIRONMENT_CLOUD_TEXTURE_FILE_NAME5, c_pEnvironmentData->strSkyBoxFaceFileName[SKYBOX_FACE_LEFT].c_str()); // left
	SetDlgItemText(IDC_MAP_ENVIRONMENT_CLOUD_TEXTURE_FILE_NAME6, c_pEnvironmentData->strSkyBoxFaceFileName[SKYBOX_FACE_RIGHT].c_str()); // right
	SetDlgItemText(IDC_MAP_ENVIRONMENT_CLOUD_TEXTURE_FILE_NAME3, c_pEnvironmentData->strSkyBoxFaceFileName[SKYBOX_FACE_TOP].c_str()); // top
	SetDlgItemText(IDC_MAP_ENVIRONMENT_CLOUD_TEXTURE_FILE_NAME8, c_pEnvironmentData->strSkyBoxFaceFileName[SKYBOX_FACE_BOTTOM].c_str()); // bottom
	...
	m_ctrlLensFlareMaxBrightness.SetPos(int32_t(c_pEnvironmentData->fLensFlareMaxBrightness*100.0f));
	...
	m_ctrlMainFlareSize.SetPos(int32_t(c_pEnvironmentData->fMainFlareSize*100.0f));

	COLORREF BrightnessColor = RGB(uint16_t(c_pEnvironmentData->LensFlareBrightnessColor.r*255.0f),
									uint16_t(c_pEnvironmentData->LensFlareBrightnessColor.g*255.0f),
									uint16_t(c_pEnvironmentData->LensFlareBrightnessColor.b*255.0f));
...
void CMapEnvironmentPage::RebuildGradientList()
{
	...
	for (uint32_t i = 0; i < c_pEnvironmentData->bySkyBoxGradientLevelUpper; ++i)
	{
		_snprintf(szGradientName, 32, "Upper %02d", i);
		m_ctrlSkyBoxGradientList.InsertString(dwControlIndex++, szGradientName);
	}
	for (uint32_t j = 0; j < c_pEnvironmentData->bySkyBoxGradientLevelLower; ++j)
	{
...
void CMapEnvironmentPage::SelectGradient(uint32_t dwIndex)
{
	...
	COLORREF FirstColor = RGB(uint16_t(pGradientColor->m_FirstColor.r*255.0f),
								uint16_t(pGradientColor->m_FirstColor.g*255.0f),
								uint16_t(pGradientColor->m_FirstColor.b*255.0f));
	COLORREF SecondColor = RGB(uint16_t(pGradientColor->m_SecondColor.r*255.0f),
								uint16_t(pGradientColor->m_SecondColor.g*255.0f),
								uint16_t(pGradientColor->m_SecondColor.b*255.0f));
...
#ifdef ENABLE_WIND_OPTION
void CMapEnvironmentPage::OnEnableWind()
{
	CWorldEditorApp * pApplication = (CWorldEditorApp *)AfxGetApp();
	CMapManagerAccessor * pMapManagerAccessor = pApplication->GetMapManagerAccessor();

	pMapManagerAccessor->EnableWind(IsDlgButtonChecked(IDC_MAP_ENVIRONMENT_WIND_STRENGTH_ENABLE));
	UpdateUI();
}
#endif
...
//				m_ctrlFogNearDistance.GetLine(0, szDistance);
				...
				Tracef("fNear value changed to : %s -> %f\n", szDistance, fNear);
...
//				m_ctrlFogFarDistance.GetLine(0, szDistance);
				...
				Tracef("fFar value read from : %s -> %f\n", szDistance, fFar);
...
//				m_ctrlFogFarDistance.GetLine(0, szDistance);
				...
				Tracef("fFar value changed to : %s -> %f\n", szDistance, fFar);
...
//				m_ctrlFogNearDistance.GetLine(0, szDistance);
				...
				Tracef("fNear value read from : %s -> %f\n", szDistance, fNear);
...
void CMapEnvironmentPage::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	switch(m_iSliderIndex)
	{
		...
		case IDC_MAP_ENVIRONMENT_WIND_RANDOM:
			pMapManagerAccessor->SetWindRandom(float(m_ctrlWindRandom.GetPos()) / 100.0f); // @fixme115 (SetWindStrength -> SetWindRandom)
			break;
...
#ifdef CWE_INFO_ENVIRONMENT_FIX
		SetDlgItemText(IDC_MAP_ENVIRONMENT_SCRIPT_NAME_PRINT, (strEnvironmentName.empty()) ? "N/A" : strEnvironmentName.c_str());

		static char chVersion[32 + 1];
		const TEnvironmentData* c_pEnvironmentData;
		pMapManagerAccessor->GetEnvironmentData(&c_pEnvironmentData);
		snprintf(chVersion, sizeof(chVersion), "%.2f", c_pEnvironmentData->fVersion);
		SetDlgItemText(IDC_MAP_ENVIRONMENT_SCRIPT_VERSION_PRINT, (strEnvironmentName.empty()) ? "N/A" : chVersion);
#endif
...
void CMapEnvironmentPage::SetSkyBoxTexture( int32_t iFaceIndex )
{
	CMapManagerAccessor* pAccessor = NULL;
	GetMapManagerAccessor( &pAccessor );
...
// new stuff
void CMapEnvironmentPage::OnCheckRecalculateShadow()
{
	CMainFrame *pFrame = (CMainFrame*)AfxGetMainWnd();
	CWorldEditorDoc * pDocument = (CWorldEditorDoc *)pFrame->GetActiveDocument();
	CSceneMap* pScene = pDocument->GetSceneMap();
	pScene->SetObjectShadowRendering(true);
	Tracef("Shadows recalculated\n");
	// pScene->SetTerrainRendering(globals::dft::VIEW_SHADOW_OUTPUT_BY_DEFAULT);
}

void CMapEnvironmentPage::OnCheckBackGroundPlayMusic()
{
	uint32_t dwFlag = OFN_FILEMUSTEXIST | OFN_OVERWRITEPROMPT | OFN_NOCHANGEDIR;
	const char * c_szFilter = "MP3 Files (*.mp3)|*.mp3|All Files (*.*)|*.*|";
	...
	if (TRUE == FileOpener.DoModal())
	{
		...
		rkSndMgr.PlayMusic(FileOpener.GetPathName());
		Tracef("PlayMusic %s\n", FileOpener.GetPathName());
	}
}

void CMapEnvironmentPage::OnCheckBackGroundPlaySound()
{
	uint32_t dwFlag = OFN_FILEMUSTEXIST | OFN_OVERWRITEPROMPT | OFN_NOCHANGEDIR;
	const char * c_szFilter = "MP3 Files (*.mp3)|*.mp3|All Files (*.*)|*.*|";
	...
	{
		CSoundManager& rkSndMgr=CSoundManager::Instance();
		rkSndMgr.PlaySound2D(FileOpener.GetPathName());
		Tracef("PlaySound2D %s\n", FileOpener.GetPathName());
	}
}

void CMapEnvironmentPage::OnCheckBackGroundPlayFadeIn()
{
	uint32_t dwFlag = OFN_FILEMUSTEXIST | OFN_OVERWRITEPROMPT | OFN_NOCHANGEDIR;
	const char * c_szFilter = "MP3 Files (*.mp3)|*.mp3|All Files (*.*)|*.*|";
	...
	{
		CSoundManager& rkSndMgr=CSoundManager::Instance();
		rkSndMgr.FadeInMusic(FileOpener.GetPathName());
		Tracef("FadeInMusic %s\n", FileOpener.GetPathName());
	}
}

void CMapEnvironmentPage::OnCheckBackGroundPlayFadeOut()
{
	uint32_t dwFlag = OFN_FILEMUSTEXIST | OFN_OVERWRITEPROMPT | OFN_NOCHANGEDIR;
	const char * c_szFilter = "MP3 Files (*.mp3)|*.mp3|All Files (*.*)|*.*|";
	...
	{
		CSoundManager& rkSndMgr=CSoundManager::Instance();
		rkSndMgr.FadeOutMusic(FileOpener.GetPathName());
		Tracef("FadeOutMusic %s\n", FileOpener.GetPathName());
	}
}

void CMapEnvironmentPage::OnCheckBackGroundStopMusic3D()
{

	CSoundManager& rkSndMgr=CSoundManager::Instance();
	rkSndMgr.StopAllSound3D();
	Tracef("StopAllSound3D\n");
}

void CMapEnvironmentPage::OnCheckBackGroundStopFadeOut()
{

	CSoundManager& rkSndMgr=CSoundManager::Instance();
	rkSndMgr.FadeOutAllMusic();
	Tracef("FadeOutAllMusic\n");
}
