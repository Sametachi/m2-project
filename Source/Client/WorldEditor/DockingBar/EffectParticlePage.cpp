...
void CEffectParticlePage::SetData(uint32_t dwIndex)
{
	...
	// TexAniDelay
	m_ctrlTexAniDelay.SetPos(int32_t(pAccessor->GetTexAniDelayReference() * 1000.0f));
...
void CEffectParticlePage::SelectEmitterShape(int32_t iShape)
{
	CWorldEditorApp * pApplication = (CWorldEditorApp *)AfxGetApp();
	CEffectAccessor * pEffectAccessor = pApplication->GetEffectAccessor();
...
void CEffectParticlePage::SelectEmitterTimeEvent(int32_t iEvent)
{
	CWorldEditorApp * pApplication = (CWorldEditorApp *)AfxGetApp();
	CEffectAccessor * pEffectAccessor = pApplication->GetEffectAccessor();
...
void CEffectParticlePage::SelectParticleTimeEvent(int32_t iEvent)
{
	CWorldEditorApp * pApplication = (CWorldEditorApp *)AfxGetApp();
	CEffectAccessor * pEffectAccessor = pApplication->GetEffectAccessor();
...
BOOL CEffectParticlePage::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	switch (LOWORD(wParam))
	{
		case IDC_EFFECT_PARTICLE_TEXTURE_LIST:
			...
			{
				CGraphicImage * pImage;
				int32_t iIndex = m_ctrlTextureList.GetCurSel();
				if (iIndex == CB_ERR)
					m_ctrlPreview.UpdatePreview(NULL);
				else
				{
