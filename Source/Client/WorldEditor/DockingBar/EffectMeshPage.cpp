...
void CEffectMeshPage::SetData(uint32_t dwEffectIndex)
{
	...
	m_ctrlMeshAnimationDelay.SetPos(int32_t(fMeshDelay * 1000.0f));
...
void CEffectMeshPage::SetElement(uint32_t dwEffectIndex, uint32_t dwElementIndex)
{
	...
	// Blending
	int32_t iSrcBlendingType = pMesh->GetBlendingSrcType(m_dwMeshIndex);
	int32_t iDestBlendingType = pMesh->GetBlendingDestType(m_dwMeshIndex);
	...
	m_ctrlTextureAnimationDelay.SetPos(int32_t(fTextureDelay * 1000.0f));
	...
	{
		COLORREF Color = RGB(uint16_t(ColorFactor.r*255.0f),
							 uint16_t(ColorFactor.g*255.0f),
							 uint16_t(ColorFactor.b*255.0f));
...
void CEffectMeshPage::SetBillboardType(int32_t iType)
{
	CWorldEditorApp * pApplication = (CWorldEditorApp *)AfxGetApp();
	CEffectAccessor * pEffectAccessor = pApplication->GetEffectAccessor();
...
void CEffectMeshPage::SetBlendType(int32_t iSrcType, int32_t iDestType)
{
	CWorldEditorApp * pApplication = (CWorldEditorApp *)AfxGetApp();
	CEffectAccessor * pEffectAccessor = pApplication->GetEffectAccessor();
...
void CEffectMeshPage::OnAcceptAllBillboard()
{
	...
	int32_t iType = MESH_BILLBOARD_TYPE_NONE;
...
void CEffectMeshPage::OnAcceptAllAlpha()
{
	...
	int32_t iSrcType = m_ctrlBlendSrcType.GetBlendType();
	int32_t iDestType = m_ctrlBlendDestType.GetBlendType();
	int32_t iOperationType = m_ctrlColorOperation.GetOperationType();
		...
		case IDC_EFFECT_MESH_ANIMATION_LOOP:
			{
				...
				if (GetEffectElement(m_dwElementIndex, &pEffect))
				{
					int32_t isChecked = IsDlgButtonChecked(IDC_EFFECT_MESH_ANIMATION_LOOP);
		...
		case IDC_EFFECT_MESH_TEXTURE_ANIMATION_LOOP:
			{
				...
				if (GetMeshElementData(m_dwElementIndex, m_dwMeshIndex, &pMeshData))
				{
					int32_t isChecked = IsDlgButtonChecked(IDC_EFFECT_MESH_TEXTURE_ANIMATION_LOOP);
