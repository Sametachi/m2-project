...
int32_t CSizingControlBar::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
{
    int32_t nPos = FindSizingBar(this);
    ASSERT(nPos >= 0);
...
CSize CSizingControlBar::CalcFixedLayout(BOOL bStretch, BOOL bHorz)
{
    ...
    for (int32_t i = 0; i < arrSCBars.GetSize(); i++)
    ...
    int32_t nLengthAvail = bHorz ? rc.Width() + 2 : rc.Height() - 2;
...
CSize CSizingControlBar::CalcDynamicLayout(int32_t nLength, uint32_t dwMode)
{
...
UINT CSizingControlBar::GetEdgeHTCode(int32_t nEdge)
{
...
void CSizingControlBar::GetRowInfo(int32_t& nFirst, int32_t& nLast, int32_t& nThis)
{
    ...
    int32_t i, nBars = m_pDockBar->m_arrBars.GetSize();
...
const int32_t CSizingControlBar::FindSizingBar(CControlBar* pBar) const
{
    for (int32_t nPos = 0; nPos < m_arrBars.GetSize(); nPos++)
...
BOOL CSizingControlBar::NegociateSpace(int32_t nLengthAvail, BOOL bHorz)
{
    ...
    int32_t nActualLength = 0;
    int32_t nMinLength = 2;
    int32_t nWidth = 0;
    for (int32_t iBar = 0; iBar < arrSCBars.GetSize(); iBar++)
    {
        pBar = arrSCBars[iBar];
    ...
    for (int32_t jBar = 0; jBar < arrSCBars.GetSize(); jBar++)
	{
        if (bHorz)
    ...
    int32_t nDelta = nLengthAvail - nActualLength;

    while (nDelta != 0)
    {
        int32_t nDeltaOld = nDelta;
        for (int32_t i = 0; i < arrSCBars.GetSize(); i++)
        {
            pBar = arrSCBars[i];
            int32_t nLMin = bHorz ? pBar->m_szMin.cx : pBar->m_szMin.cy;
            int32_t nL = bHorz ? pBar->m_szHorz.cx : pBar->m_szVert.cy;
            ...
            // sign of nDelta
            int32_t nDelta2 = (nDelta == 0) ? 0 : ((nDelta < 0) ? -1 : 1);
...
void CSizingControlBar::LoadState(LPCTSTR lpszProfileName)
{
    ...
    m_szHorz.cx = max(m_szMin.cx, (int32_t) pApp->GetProfileInt(szSection,
        _T("sizeHorzCX"), m_szHorz.cx));
    m_szHorz.cy = max(m_szMin.cy, (int32_t) pApp->GetProfileInt(szSection,
        _T("sizeHorzCY"), m_szHorz.cy));

    m_szVert.cx = max(m_szMin.cx, (int32_t) pApp->GetProfileInt(szSection,
        _T("sizeVertCX"), m_szVert.cx));
    m_szVert.cy = max(m_szMin.cy, (int32_t) pApp->GetProfileInt(szSection,
        _T("sizeVertCY"), m_szVert.cy));

    m_szFloat.cx = max(m_szMin.cx, (int32_t) pApp->GetProfileInt(szSection,
        _T("sizeFloatCX"), m_szFloat.cx));
    m_szFloat.cy = max(m_szMin.cy, (int32_t) pApp->GetProfileInt(szSection,
...
{
    for (int32_t i = 0; i < m_arrBars.GetSize(); i++)
...
void CSCBButton::Paint(CDC* pDC)
{
    ...
    int32_t nPrevBkMode = pDC->SetBkMode(TRANSPARENT);
    CFont font;
    int32_t ppi = pDC->GetDeviceCaps(LOGPIXELSX);
    int32_t pointsize = MulDiv(60, 96, ppi); // 6 points at 96 ppi
