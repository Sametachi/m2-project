...
void CEffectTabPage::UpdateUI()
{
	...
	for (uint32_t i = 0; i < iCount; ++i)
	{
		...
		if (pElement->bVisible)
			_snprintf(szName, 32, "[%02d] Show %s", i+1, pElement->strName); // @fixme126 shortened
		else
			_snprintf(szName, 32, "[%02d] Hide %s", i+1, pElement->strName); // @fixme126 shortened
...
int32_t CEffectTabPage::GetCurrentTab()
{
	CWorldEditorApp * pApplication = (CWorldEditorApp *)AfxGetApp();
	CEffectAccessor * pEffectAccessor = pApplication->GetEffectAccessor();
...
int32_t CEffectTabPage::GetSelectedIndex()
{
	static int32_t lastSelect=-2;
...
void CEffectTabPage::OnSaveScript(const char * c_szFileName)
{
	...
	{
		std::string strBackupFileName = c_szFileName;
		strBackupFileName += ".bak";
		if (!CopyFile(c_szFileName, strBackupFileName.c_str(), FALSE))
		{
			LogBox("Failed to save the backup file", "Error");
		}
	}
...
int32_t CEffectTabPage::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void CEffectTabPage::OnCreateParticle()
{
	...
	int32_t iIndex = pEffectAccessor->GetElementCount();
...
void CEffectTabPage::OnShowAll()
{
	...
	for (int32_t i = 0; i < pEffectAccessor->GetElementCount(); ++i)
