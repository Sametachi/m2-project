...
class CCursorRenderer : public CScreen
{
	...
	public:
		...
		bool GetSettingCursorPosition(uint32_t dwIndex, int32_t * px, int32_t * py);

		// Normal
		void SetObjectBrushType(int32_t iType);
		void SetObjectAreaSize(int32_t iSize);
		void SetObjectAreaDensity(int32_t iDensity);
		void SetObjectAreaRandom(int32_t iRandom);
		...
		void RenderCursorSquare(float fxStart, float fyStart, float fxEnd, float fyEnd, int32_t iStep = 50);
		void RenderCursorCircle(float fx, float fy, float fz, float fRadius, int32_t iStep = 50);

	protected:
		void __Initialize();

		void RenderPieceLine(float fxStart, float fyStart, float fxEnd, float fyEnd, int32_t iStep);

	protected:
		int32_t								m_iCursorType;
		...
		int32_t								m_iObjectBrushType;
		int32_t								m_iObjectAreaHalfSize;
		int32_t								m_iObjectAreaDensity;
		int32_t								m_iObjectAreaRandom;
		BOOL							m_isGridMode;
