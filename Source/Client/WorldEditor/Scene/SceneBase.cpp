...
int32_t CSceneBase::ms_iPureRenderingTime;
...
{
	int32_t iStartTime = timeGetTime();
...
void CSceneBase::KeyDown(int32_t iChar)
{
	OnKeyDown(iChar);
}

void CSceneBase::KeyUp(int32_t iChar)
{
	if (VK_Y==iChar)
	{
		D3DXVECTOR3 pv3Position = ms_Camera->GetTarget();
		CCameraManager::Instance().GetCurrentCamera()->Move(D3DXVECTOR3(-pv3Position.x+0.0f, -pv3Position.y+0.0f, -pv3Position.z+-163.94f));
	}
	OnKeyUp(iChar);
}

void CSceneBase::MouseMove(int32_t x, int32_t y)
{
	OnMouseMove(x, y);
}
...
BOOL CSceneBase::MouseWheel(int16_t zDelta)
{
...
int32_t CSceneBase::GetRenderingMode()
{
