...
void CCursorRenderer::RenderPieceLine(float fxStart, float fyStart, float fxEnd, float fyEnd, int32_t iStep)
{
...
void CCursorRenderer::RenderCursorSquare(float fxStart, float fyStart, float fxEnd, float fyEnd, int32_t iStep)
{
	SetDiffuseOperation();
...
void CCursorRenderer::RenderCursorCircle(float fx, float fy, float fz, float fRadius, int32_t iStep)
{
	SetDiffuseOperation();
	SetDiffuseColor(globals::dft::RENDER_CURSOR_COLOR_R,globals::dft::RENDER_CURSOR_COLOR_G,globals::dft::RENDER_CURSOR_COLOR_B);
 	STATEMANAGER.SetTransform(D3DTS_WORLD, &GetIdentityMatrix());
...
{
	SetDiffuseColor(globals::dft::RENDER_CURSOR_COLOR_R,globals::dft::RENDER_CURSOR_COLOR_G,globals::dft::RENDER_CURSOR_COLOR_B, 1.0f);
...
void CCursorRenderer::SetCursor(uint32_t dwCRC)
{
	...
	int32_t iLength = strlen(pszPropertyType) + 1;
...
bool CCursorRenderer::GetSettingCursorPosition(uint32_t dwIndex, int32_t * px, int32_t * py)
{
...
void CCursorRenderer::SetObjectBrushType(int32_t iType)
{
	m_iObjectBrushType = iType;
}

void CCursorRenderer::SetObjectAreaSize(int32_t iSize)
{
	m_iObjectAreaHalfSize = iSize;
}

void CCursorRenderer::SetObjectAreaDensity(int32_t iDensity)
{
	m_iObjectAreaDensity = iDensity;
}

void CCursorRenderer::SetObjectAreaRandom(int32_t iRandom)
{
...
void CCursorRenderer::SetObjectHeight(float fHeight)
{
	...
	m_fObjectHeight = fMAX(m_fObjectHeight, -globals::dft::OBJECT_HEIGHT_MAX);
	m_fObjectHeight = fMIN(m_fObjectHeight, +globals::dft::OBJECT_HEIGHT_MAX);
}
