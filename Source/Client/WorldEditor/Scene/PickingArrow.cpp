...
int32_t CPickingArrows::m_dwIndex = 0;
const int32_t c_iArrowBoxesCount = 3;
...
void CPickingArrows::SetArrowSets(int32_t dwIndex)
{
	m_dwIndex = 3*dwIndex;
}
...
int32_t CPickingArrows::Picking()
{
	m_iHoldingDirection = -1;

	//float fu, fv, ft;
	for (int32_t i = 0; i < c_iArrowBoxesCount; ++i)
	{
...
void CPickingArrows::Render()
{
	for (uint32_t j = 0; j < c_iArrowBoxesCount; ++j)
	{
		...
		STATEMANAGER.SaveRenderState(D3DRS_TEXTUREFACTOR,((uint32_t)D3DXCOLOR(c_fArrowBoxes[i][3], c_fArrowBoxes[i][4], c_fArrowBoxes[i][5],1.0f)));
		RenderLine3d(m_v3Position.x, m_v3Position.y, m_v3Position.z,
