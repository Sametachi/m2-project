...
void CSceneObject::OnUpdate()
{
	...
	static int32_t s_iLastTime = timeGetTime();
	int32_t iCurrentTime = timeGetTime();
...
int32_t GetOperation(int32_t iOperation)
{
	return iOperation + 1;
}
...
void CSceneObject::OnKeyDown(int32_t iChar)
{
	if (VK_SPACE == iChar) // Space 32
	{
		if (!isPlay())
		{
			Play(false);
...
void CSceneObject::OnKeyUp(int32_t iChar)
{
}

void CSceneObject::OnMouseMove(int32_t x, int32_t y)
{
	CWorldEditorApp * pApplication = (CWorldEditorApp *)AfxGetApp();
	CObjectData * pObjectData = pApplication->GetObjectData();
...
void CSceneObject::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	CPickingArrows pa;
	pa.SetCenterPosition(m_v3Target);
	int32_t iPickingDirection = pa.Picking();
...
BOOL CSceneObject::OnMouseWheel(int16_t zDelat)
{
	return FALSE;
}
...
void CSceneObject::BuildTimeHitPosition(NRaceData::TMotionAttackData * pMotionAttackData)
{
	...
	for (; itor != pMotionAttackData->HitDataContainer.end(); ++itor)
	{
		...
		const char * c_szFileName = "D:\\Ymir Work\\item/weapon/00010.gr2";
		if (!CResourceManager::Instance().IsFileExist(c_szFileName)) // @fixme119 animation atk bone crash if the previous file is missing
		{
			LogBoxf("The file [%s] is not a model or doesn't exist", c_szFileName);
			return;
		}
		...
		for(float fTime = rHitData.fAttackStartTime; fTime < rHitData.fAttackEndTime; fTime += 1.0f/120)
		{
			...
			if (!pCompositeBoneMat) // @fixme119 animation atk bone crash
				break;
