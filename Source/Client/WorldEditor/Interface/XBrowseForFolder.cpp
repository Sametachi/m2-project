...
public:
	CRect() { }
	CRect(int32_t l, int32_t t, int32_t r, int32_t b) { left = l;
										top = t;
										right = r;
										bottom = b; }
	...
	static void SwapLeftRight(LPRECT lpRect) { int32_t temp = lpRect->left;
...
static int32_t CALLBACK BrowseCallbackProc(HWND hwnd,		// Window handle to the browse dialog box
														...
														// BROWSEINFO structure
{
	switch (uMsg)
	{
		case BFFM_INITIALIZED:		// sent when the browse dialog box has finished initializing.
		{
			...
			int32_t lStyle = ::GetWindowLong(hwnd, GWL_STYLE);
			...
			if (hwndTree)
			{
				...
				int32_t h = rectCancel.Height();
				int32_t w = rectCancel.Width();
