...
/*
	...
	static int32_t	HSV2RGB (double hue,double sat,double value,double *red,double *green,double *blue);
	static int32_t	RGB2HSV(double red,double green,double blue,double *hue,double *sat,double *value);
...
#define	HSV_SAT_INIT_0()	coef3 = coef1,	coef3_adv = (int32_t) ((val - coef3) / HSV_LOOP_STEPS (j))

// advance for HSV colorspace in SAT mode, for HUE between 0 and 1 (0 and 60 deg)
#define	HSV_SAT_ADV_0()	    coef3 += coef3_adv

#define	HSV_SAT_INIT_1()	coef2 = val, coef2_adv = (int32_t) ((val * (1.0 - sat) - coef2) / HSV_LOOP_STEPS (j))
...
public:
	...
	// Draw panel border.
	virtual void DrawPanel(HDC hdc,int32_t left,int32_t top,int32_t right,int32_t bottom,int32_t flags);

	// Draw panel border.
	virtual void DrawPanel (HDC hdc,LPRECT rc,int32_t flags)
	{
		DrawPanel(hdc, rc->left, rc->top, rc->right, rc->bottom, flags);
	};

	// Draw triangle.
	virtual void DrawTriangle(HDC hdc,int32_t x,int32_t y,enum types type,int32_t size,COLORREF line_color,COLORREF fill_color);
	...
	// Get rgb value.
	void GetRGB(uint32_t *buffer,int32_t samples,COLORREF start,COLORREF end);

	// hsv from hue.
	void HSV_HUE(uint32_t *buffer,int32_t samples,double sat,double val);

	// hsv from sat.
	void HSV_SAT(uint32_t *buffer,int32_t samples,double hue,double val);

	// hsv value.
	void HSV_VAL(uint32_t *buffer,int32_t samples,double hue,double sat);

	// Set
	void set(uint32_t *buffer,uint32_t value,size_t count);

	//
	void set(uint32_t **buffer,uint32_t value,size_t count);

	//
	void copy(uint32_t *target,const uint32_t *source,size_t count);

	//
	void copy(uint32_t **target,const uint32_t *source,size_t count);

	//
	void copy_reverse(uint32_t *target,const uint32_t *source,size_t count);

	//
	void reverse(uint32_t *buffer,size_t count);

};

inline int32_t scaled_red (COLORREF c)
{
	return (GetRValue (c) << int_extend);
}

inline int32_t scaled_green (COLORREF c)
{
	return (GetGValue (c) << int_extend);
}

inline int32_t scaled_blue (COLORREF c)
{
