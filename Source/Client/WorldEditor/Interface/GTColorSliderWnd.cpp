...
BOOL GTColorSliderWnd::Create(uint32_t dwStyle,CRect rcPos, CWnd* pParent,UINT nID,int32_t nBulletType,BOOL bPopup)
{
	nBulletType;
	bPopup;
...
void GTColorSliderWnd::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	new_pos = static_cast <int32_t>((m_nEndValue - m_nStartValue) * d +
...
void GTColorSliderWnd::OnPaint()
{
	...
	if (hdc != NULL)
	{
		...
		if (m_nCurLayout & layout_horizontal)
		{
			...
			x =(int32_t)((m_rcPaint.right - m_rcPaint.left) * d);
		...
		else // if (m_nCurLayout & layout_vertical)
		{
			...
			y =(int32_t)((m_rcPaint.bottom - m_rcPaint.top) * d);
...
void GTColorSliderWnd::OnSize(UINT nType, int32_t cx, int32_t cy)
{
...
int32_t GTColorSliderWnd::OnSelectBulletOK(UINT wParam, int32_t lParam)
{
	wParam;
	lParam;
...
void GTColorSliderWnd::SetLayoutValue(IN int32_t value)
{
	m_nCurLayout = value;
	Invalidate(FALSE);
}

void GTColorSliderWnd::SetDataValue(IN int32_t value)
{
...
void GTColorSliderWnd::SetStartValue(int32_t value)
{
...
void GTColorSliderWnd::SetEndValue(int32_t value)
{
...
void GTColorSliderWnd::SetPosition(int32_t value)
{
...
void GTColorSliderWnd::SetControlValue(int32_t index,int32_t value)
{
	if (index < 0 || index > 1)
		// invalid index
		return;
...
void GTColorSliderWnd::DoDrawRGB(void)
{
	...
	c1 =(uint8_t)((int32_t) additional_components [0] * 255 / max_rgb);
	c2 =(uint8_t)((int32_t) additional_components [1] * 255 / max_rgb);
