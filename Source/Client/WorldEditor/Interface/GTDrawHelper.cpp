...
void GTDrawHelper::DrawPanel(HDC hdc,int32_t left,int32_t top,int32_t right,int32_t bottom,int32_t flags)
{
...
void GTDrawHelper::DrawTriangle(HDC hdc,int32_t x,int32_t y,enum types type,int32_t size,COLORREF line_color,COLORREF fill_color)
{
...
int32_t GTDrawHelper::HSV2RGB(double hue,double sat,double value,double *red,double *green,double *blue)
{
...
int32_t GTDrawHelper::RGB2HSV(double red,double green,double blue,double *hue,double *sat,double *value)
{
...
void GTDrawHelper::GetRGB(uint32_t *buffer,int32_t samples,COLORREF start,COLORREF end)
{
	int32_t	red, green, blue;
	int32_t	red_adv, green_adv, blue_adv;
...
void GTDrawHelper::HSV_HUE(uint32_t *buffer,int32_t samples,double sat,double val_fp)
{
	...
	// constants
	val =(int32_t)(val_fp * 255) << int_extend;
	coef1 =(int32_t)(val *(1 - sat));
...
void GTDrawHelper::HSV_SAT(uint32_t *buffer,int32_t samples,double hue,double val_fp)
{
	...
	// coefficients and advances
	signed int32_t		coef1, coef2, coef3;
	signed int32_t		coef1_adv, coef2_adv, coef3_adv;
	...
	coef2 = val;
	coef2_adv =(int32_t)((1 - frac) * val - coef2) / samples;
	coef3 = val;
	coef3_adv =(int32_t)(frac * val - coef3) / samples;
...
void GTDrawHelper::HSV_VAL(uint32_t *buffer,int32_t samples,double hue,double sat)
{
	// loop counter
	int32_t		j;
	...
	coef1 = 0;
	coef1_adv =(int32_t)(val_max *(1 - sat)) / samples;
	coef2 = 0;
	coef2_adv =(int32_t)(val_max *(1 - sat * frac)) / samples;
	coef3 = 0;
	coef3_adv =(int32_t)(val_max *(1 - sat *(1 - frac))) / samples;
	val = 0;
...
void GTDrawHelper::set(uint32_t *buffer,uint32_t value,size_t count)
{
	while (count--)
		*buffer++ = value;
}
...
void GTDrawHelper::copy(uint32_t *target,const uint32_t *source,size_t count)
{
	while (count--)
		*target++ = *source++;
}
...
void GTDrawHelper::copy_reverse(uint32_t *target,const uint32_t *source,size_t count)
{
	source +=(count - 1);
	while (count--)
		*target++ = *source--;
...
void GTDrawHelper::reverse(uint32_t *buffer,size_t count)
{
	uint32_t	*p = buffer + count - 1;
