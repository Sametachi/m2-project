...
void CGTColorDialogDlg::ColorChanged()
{
	...
	if (!m_bChangingHSV)
	{
		...
		sMsg.Format("%d",int32_t(sat*100));
		m_edt_HSV_S.SetWindowText(sMsg);

		sMsg.Format("%d",int32_t(val*100));
...
CString	CGTColorDialogDlg::ColorToString(COLORREF cr)
{
    ...
    int32_t Rc = GetRValue(cr);
    int32_t Gc = GetGValue(cr);
    int32_t Bc = GetBValue(cr);
...
void CGTColorDialogDlg::HandleRGBEdit()
{
	...
	int32_t nR = atoi((LPCTSTR)sVal);

	m_edt_RGB_G.GetWindowText(sVal);
	int32_t nG = atoi((LPCTSTR)sVal);

	m_edt_RGB_B.GetWindowText(sVal);
	int32_t nB = atoi((LPCTSTR)sVal);
...
int32_t	CGTColorDialogDlg::hex(char* szhex)
{
...
COLORREF CGTColorDialogDlg::html2rgb(char* szHTMLColorValue)
{
	...
	int32_t red = hex(szHTMLColorValue + 1);
	int32_t green = hex(szHTMLColorValue + 3);
	int32_t blue = hex(szHTMLColorValue + 5);
