...
{
    int32_t result = (int32_t)GotoURL(m_strURL, SW_SHOW);
...
int32_t CFOHyperLink::GetRegKey(HKEY key, LPCTSTR subkey, LPTSTR retdata)
{
    HKEY hkey;
    int32_t retval = RegOpenKeyEx(key, subkey, 0, KEY_QUERY_VALUE, &hkey);

    if (retval == ERROR_SUCCESS) {
        int32_t datasize = MAX_PATH;
        TCHAR data[MAX_PATH];
...
void CFOHyperLink::ReportError(int32_t nError)
{
    CString str;
    switch (nError) {
...
HINSTANCE CFOHyperLink::GotoURL( LPCTSTR url, int32_t showcmd )
{
