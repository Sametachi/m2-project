...
BOOL GTColorSelectorWnd::Create(uint32_t dwStyle,CRect rcPos, CWnd* pParent,UINT nID,int32_t nBulletType,BOOL bPopup)
{
	nBulletType;
	bPopup;
...
void GTColorSelectorWnd::SetDataValue(IN int32_t value)
{
	if (m_nCurMode == value)
		return;
...
void GTColorSelectorWnd::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if (m_bTracking)
	{
		...
		point.x =(int16_t) ceil((point.x - m_rcPaint.left) *(double) max_x /(m_rcPaint.right - m_rcPaint.left));
		point.y =(int16_t) ceil((m_rcPaint.bottom - point.y) *(double) max_y /(m_rcPaint.bottom - m_rcPaint.top));
...
void GTColorSelectorWnd::OnSize(UINT nType, int32_t cx, int32_t cy)
{
...
int32_t GTColorSelectorWnd::OnSelectBulletOK(UINT wParam, int32_t lParam)
{
	wParam;
	lParam;
...
void GTColorSelectorWnd::SetXPos(int32_t value)
{
	if (x_pos == value)
		return;
	x_pos = value;
...
void GTColorSelectorWnd::SetYPos(int32_t value)
{
	if (y_pos == value)
		return;
	y_pos = value;
...
{
	uint8_t	c =(uint8_t)((int32_t) additional_component * 255 / max_rgb);
...
void GTColorSelectorWnd::DoDrawRGB(void)
{
	...
	int32_t		nSkip;	// number of pixels to skip after current row and before next one

	uint32_t	*p;		// pointer to first pixel from the blend & a loop pointer

	int32_t		width = m_rcFrame.right - m_rcFrame.left;
	int32_t		height = m_rcFrame.bottom - m_rcFrame.top;
	int32_t		blend_width = m_rcPaint.right - m_rcPaint.left + 1;
	int32_t		blend_height = m_rcPaint.bottom - m_rcPaint.top + 1;

	// left side components, extended with int_extend bits
	int32_t		left_red, left_green, left_blue;
	// left side component advance, extended with int_extend bits
	int32_t		left_red_adv, left_green_adv, left_blue_adv;
	int32_t		right_red, right_green, right_blue;
	int32_t		right_red_adv, right_green_adv, right_blue_adv;
	...
	case	modes_hsv_hue:
		{
			...
			m_Panel.RGB2HSV(
				...
				&hue, &sat, &val);
			x =(int32_t) ceil(sat * scale_hsv_sat);
			y =(int32_t) ceil(val * scale_hsv_value);
			comp =(int32_t) ceil(hue * scale_hsv_hue);
		}
		break;
	...
	case	modes_hsv_sat:
		{
			...
			m_Panel.RGB2HSV(
				...
				blue /(double) max_rgb_blue,
				&hue, &sat, &val);
			x =(int32_t) ceil(hue * scale_hsv_hue);
			y =(int32_t) ceil(val * scale_hsv_value);
			comp =(int32_t) ceil(sat * scale_hsv_sat);
		}
		break;
	...
	case	modes_hsv_value:
		{
			...
			m_Panel.RGB2HSV(
				...
				&hue, &sat, &val);
			x =(int32_t) ceil(hue * scale_hsv_hue);
			y =(int32_t) ceil(sat * scale_hsv_sat);
			comp =(int32_t) ceil(val * scale_hsv_value);
		}
		break;
...
void GTColorSelectorWnd::SetControlValue(int32_t value)
{
