...
const int32_t CONTRAST_MARKER = 1;
const int32_t MARKER_SIZE = 5;

const int32_t	max_picker [6][3] =
{
...
class GTColorSelectorWnd : public CWnd
{
	...
	public:
		...
		// get/set visual mode
		inline int32_t GetDataValue(void) const { return (m_nCurMode); };
		void SetDataValue(int32_t value);

		// get/set additional component's value
		inline int32_t GetControlValue(void) const { return (additional_component); };
		void SetControlValue(int32_t value);
		...
		inline int32_t GetXPos(void) const { return (x_pos); };
		void SetXPos(int32_t value);

		// get/set current y-axis position
		inline int32_t GetYPos(void) const { return (y_pos); };
