...
void CToolBarFile::ShowButtonText(BOOL bShow)
{
	if (bShow)
	{
		SetSizes(CSize(43, 34), CSize(16, 15));
		LoadBitmap(IDR_MAINFRAME);
		m_nBitmapID = IDR_MAINFRAME;

		SetButtonText(CommandToIndex(ID_FILE_NEW), "New");
		...
		SetButtonText(CommandToIndex(ID_VIEW_OBJECT), "View Object");
		SetButtonText(CommandToIndex(ID_VIEW_ENVIRONMENT), "View Environment");

		SetButtonText(CommandToIndex(ID_APP_ABOUT), "About");
	}
	else
	{
		SetSizes(CSize(43, 34), CSize(36, 28));
