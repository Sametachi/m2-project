...
void CEffectTimeEventGraph::Initialize(int32_t iTimerID)
{
...
void CEffectTimeEventGraph::Resizing(int32_t iWidth, int32_t iHeight)
{
...
void CEffectTimeEventGraph::RenderGrid()
{
	...
	for (int32_t x = 0; x < m_ixGridCount+1; ++x)
	{
	...
	for (int32_t y = 0; y < m_iyGridCount+1; ++y)
	{
...
void CEffectTimeEventGraph::TimeToScreen(float fTime, int32_t * px)
{
	*px = (fTime / m_fMaxTime) * float(m_iWidth) + m_ixTemporary;
}

void CEffectTimeEventGraph::ScreenToTime(int32_t ix, float * pfTime)
{
	*pfTime = (float(ix - m_ixTemporary) / float(m_iWidth)) * m_fMaxTime;
}

void CEffectTimeEventGraph::ValueToScreen(float fValue, int32_t * piy)
{
	*piy = m_iHeight - ((fValue - m_fStartValue) / m_fMaxValue) * float(m_iHeight) + m_iyTemporary;
}

void CEffectTimeEventGraph::ScreenToValue(int32_t iy, float * pfValue)
{
