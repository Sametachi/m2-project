...
void CFloatEditDialog::OnSize(UINT nType, int32_t cx, int32_t cy)
{
...
void CFloatEditDialog::OnEndlabeleditFloatEditListTime(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	*pResult = 0;
	...
	/*for(int32_t i=0;i<m_pAccessor->GetValueCount();i++)
	{
		m_pAccessor->GetTime(i,&time);
		if (time>newtime)
		{
...
void CFloatEditDialog::OnClickFloatEditListTime(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	if (GetAsyncKeyState(VK_CONTROL) & 0x8000)
	{
		...
		{
			int32_t nItem = m_ctrlListTime.GetNextSelectedItem(pos);
			if (nItem)
			{
				int32_t nPrevItem = nItem - 1;
				float time1, time2;
				float value1, value2;
...
void CFloatEditDialog::OnClickFloatEditListValue(NMHDR* pNMHDR, LRESULT* pResult)
{
	if (GetAsyncKeyState(VK_CONTROL) & 0x8000)
	{
		...
		{
			int32_t nItem = m_ctrlListValue.GetNextSelectedItem(pos);
			if (nItem)
			{
				int32_t nPrevItem = nItem - 1;
				float time1, time2;
