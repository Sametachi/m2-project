...
void CMapObjectPropertyPageTree::OnUpdateUI(CProperty * pProperty)
{
	...
	int32_t iTreeSize = atoi(c_szTreeSize);
	int32_t iTreeSizeVariance = atoi(c_szTreeSizeVariance);
...
void CMapObjectPropertyPageTree::OnUpdatePropertyData(const char * c_szPropertyName)
{
	...
	int32_t iTreeSize = m_ctrlTreeSize.GetPos();
	int32_t iTreeVariance = m_ctrlTreeSizeVariance.GetPos();
...
bool CMapObjectPropertyPageTree::OnSave(const char * c_szPathName, CProperty * pProperty)
{
	...
	if (!pProperty)
	{
		...
		{
			LogBox("The property exists with the same name.", "Error", GetSafeHwnd());
			return false;
		}
