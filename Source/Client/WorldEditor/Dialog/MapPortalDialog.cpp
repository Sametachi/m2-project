...
void CMapPortalDialog::__RefreshPortalNumberList()
{
	...
	std::set<int32_t> kSet_iPortalNumber;
	...
	std::set<int32_t>::iterator itor = kSet_iPortalNumber.begin();
	for (; itor != kSet_iPortalNumber.end(); ++itor)
	{
		...
		ListView_SetItemState(m_ctrlPortalList.GetSafeHwnd(), iIndex, UINT((int32_t(bCheck) + 1) << 12), LVIS_STATEIMAGEMASK);
...
void CMapPortalDialog::__ShowAll()
{
	...
	{
		int32_t iID = m_kVec_iPortalID[i];
...
{
	m_pAccessor->ClearPortal();
	std::set<int32_t>::iterator itor=m_kSet_iShowingPortalID.begin();
...
void CMapPortalDialog::OnItemchangedPortalList(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	*pResult = 0;
	...
	int32_t iListIndex = pNMListView->iItem;
	if (iListIndex >= int32_t(m_kVec_iPortalID.size()))
		return;

	int32_t iID = m_kVec_iPortalID[iListIndex];
...
void CMapPortalDialog::OnPortalShowAll()
{
	...
	for (uint32_t i = 0; i < m_kVec_iPortalID.size(); ++i)
	{
		ListView_SetItemState(m_ctrlPortalList.GetSafeHwnd(), i, UINT((int32_t(TRUE) + 1) << 12), LVIS_STATEIMAGEMASK);
	}
...
void CMapPortalDialog::OnPortalHideAll()
{
	...
	for (uint32_t i = 0; i < m_kVec_iPortalID.size(); ++i)
	{
		ListView_SetItemState(m_ctrlPortalList.GetSafeHwnd(), i, UINT((int32_t(FALSE) + 1) << 12), LVIS_STATEIMAGEMASK);
	}
