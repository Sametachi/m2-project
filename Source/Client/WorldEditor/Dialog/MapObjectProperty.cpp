...
void CMapObjectProperty::DoDataExchange(CDataExchange* pDX)
{
	...
	m_ctrlPropertyTypeList.InsertString(prt::PROPERTY_TYPE_DUNGEON_BLOCK, "Dungeon Block");
...
int32_t CMapObjectProperty::GetPropertyType()
{
...
void CMapObjectProperty::SetupProperty()
{
	...
	int32_t iPropertyType = prt::GetPropertyType(c_szPropertyType);
...
void CMapObjectProperty::OnOk()
{
	if (!m_pActivePage)
	{
		LogBox("Type must be chosen", "Error", GetSafeHwnd());
		return;
	}
...
void CMapObjectProperty::OnChangePropertyType()
{
	...
	for (int32_t i = 0; i < prt::PROPERTY_TYPE_MAX_NUM; ++i)
	{
