...
void CMapManagerAccessor::EnableWind(BOOL bFlag)
{
	m_EnvironmentData.bWindEnable = bFlag;
	if (!bFlag)
	{
		SetWindRandom(Environment_GetDefaultWindRandom());
		SetWindStrength(Environment_GetDefaultWindStrength());
	}
}
#endif
...
void CMapManagerAccessor::SetFilteringAlphaSrc(uint8_t byAlphaSrc)
{
	m_EnvironmentData.byFilteringAlphaSrc = byAlphaSrc;
}

void CMapManagerAccessor::SetFilteringAlphaDest(uint8_t byAlphaDest)
{
...
void CMapManagerAccessor::SetSkyBoxFaceTexture( const char* pFileName, int32_t iFaceIndex )
{
...
std::string CMapManagerAccessor::GetSkyBoxFaceTextre( int32_t iFaceIndex )
{
	if( iFaceIndex < 0 || iFaceIndex > 5 )
		return "";
...
uint8_t & CMapManagerAccessor::GetSkyBoxGradientUpperReference()
{
	return m_EnvironmentData.bySkyBoxGradientLevelUpper;
}
uint8_t & CMapManagerAccessor::GetSkyBoxGradientLowerReference()
{
	return m_EnvironmentData.bySkyBoxGradientLevelLower;
}
BOOL CMapManagerAccessor::GetSkyBoxGradientColorPointer(uint32_t dwIndex, TGradientColor ** ppGradientColor)
{
...
void CMapManagerAccessor::InsertGradientUpper()
{
	...
	std::vector<TGradientColor> TempGradientVector = m_EnvironmentData.SkyBoxGradientColorVector;
	...
	for (uint32_t i = 0; i < TempGradientVector.size(); ++i)
	{
	...
	for (uint32_t j = 0; j < m_EnvironmentData.bySkyBoxGradientLevelLower; ++j)
	{
...
void CMapManagerAccessor::InsertGradientLower()
{
	...
	for (uint32_t j = 0; j < m_EnvironmentData.bySkyBoxGradientLevelLower; ++j)
	{
		m_EnvironmentData.SkyBoxGradientColorVector[m_EnvironmentData.bySkyBoxGradientLevelUpper+j-1] = TempGradientVector[m_EnvironmentData.bySkyBoxGradientLevelUpper+j-1]; // @fixme128 (missing -1)
	}
...
void CMapManagerAccessor::DeleteGradient(uint32_t dwIndex)
{
	if (dwIndex >= m_EnvironmentData.SkyBoxGradientColorVector.size())
		return;
...
void CMapManagerAccessor::SaveEnvironmentScript(const char * c_szFileName)
{
	...
	const float c_fEnvironment_Script_Version = m_EnvironmentData.fVersion ? m_EnvironmentData.fVersion : 1.0f;
#else
	...
	fprintf(File, "ScriptType         EnvironmentData\n"); // @fixme117 (typo EnvrionmentData to EnvironmentData)
...
#ifdef ENABLE_WIND_OPTION
	if (m_EnvironmentData.bWindEnable)
	{
		PrintfTabs(File, 0, "Group Wind\n");
		PrintfTabs(File, 0, "{\n");
		PrintfTabs(File, 1, "Enable    %d\n", m_EnvironmentData.bWindEnable);
		PrintfTabs(File, 1, "Strength  %f\n", m_EnvironmentData.fWindStrength);
		PrintfTabs(File, 1, "Random    %f\n", m_EnvironmentData.fWindRandom);
		PrintfTabs(File, 0, "}\n");
		PrintfTabs(File, 0, "\n");
	}
#endif
	...
	PrintfTabs(File, 1, "FrontFaceFileName	   \"%s\"\n", m_EnvironmentData.strSkyBoxFaceFileName[SKYBOX_FACE_FRONT].c_str());
	PrintfTabs(File, 1, "BackFaceFileName	   \"%s\"\n", m_EnvironmentData.strSkyBoxFaceFileName[SKYBOX_FACE_BACK].c_str());
	PrintfTabs(File, 1, "LeftFaceFileName	   \"%s\"\n", m_EnvironmentData.strSkyBoxFaceFileName[SKYBOX_FACE_LEFT].c_str());
	PrintfTabs(File, 1, "RightFaceFileName	   \"%s\"\n", m_EnvironmentData.strSkyBoxFaceFileName[SKYBOX_FACE_RIGHT].c_str());
	PrintfTabs(File, 1, "TopFaceFileName	   \"%s\"\n", m_EnvironmentData.strSkyBoxFaceFileName[SKYBOX_FACE_TOP].c_str());
	PrintfTabs(File, 1, "BottomFaceFileName	   \"%s\"\n", m_EnvironmentData.strSkyBoxFaceFileName[SKYBOX_FACE_BOTTOM].c_str());
	PrintfTabs(File, 1, "\n");
	...
	if (!m_EnvironmentData.SkyBoxGradientColorVector.empty())
	{
		...
		PrintfTabs(File, 1, "{\n");
		for (uint32_t k = 0; k < m_EnvironmentData.SkyBoxGradientColorVector.size(); ++k)
		{
...
void CMapManagerAccessor::InitializeEnvironmentData()
{
	...
	m_EnvironmentData.fWindStrength = Environment_GetDefaultWindStrength();
	m_EnvironmentData.fWindRandom = Environment_GetDefaultWindRandom();
#ifdef ENABLE_WIND_OPTION
	m_EnvironmentData.bWindEnable = FALSE;
#endif
	...
	// @fixme118 BEGIN
	for (auto & skybox : m_EnvironmentData.strSkyBoxFaceFileName)
		skybox = "";
	// @fixme118 END
...
#ifdef CWE_INFO_ENVIRONMENT_FIX
	m_EnvironmentData.fVersion = 1.0f;
#endif
}
