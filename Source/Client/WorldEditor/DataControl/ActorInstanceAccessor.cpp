...
void CActorInstanceAccessor::AttachObject(const char * c_szFileName, const char * c_szBoneName)
{
	...
	ModelInstance.pModelInstance->SetMainModelPointer(ModelInstance.pThing->GetModelPointer(0), NULL); // @fixme125 (null dvb)
...
BOOL CActorInstanceAccessor::GetBoneMatrix(uint32_t dwBoneIndex, D3DXMATRIX ** ppMatrix)
{
	return CGraphicThingInstance::GetBoneMatrix(0, dwBoneIndex, ppMatrix);
}

BOOL CActorInstanceAccessor::GetBoneIndexByName(const char * c_szBoneName, int32_t * pBoneIndex) const
{
...
BOOL CActorInstanceAccessor::SetAccessorModel(CGraphicThing * pThing)
{
	...
	{
		LogBoxf("The file [%s] is not a model or doesn't exist", pThing->GetFileName());
		pThing->Release();
		return FALSE;
	}
	...
	if (!pModel)
	{
		LogBoxf("The file [%s] is not a normal model", pThing->GetFileName());
		pThing->Release();
		return FALSE;
	}
...
BOOL CActorInstanceAccessor::SetAccessorMotion(CGraphicThing * pThing)
{
	...
	{
		LogBox("The file [%s] is not a motion", pThing->GetFileName());
		pThing->Release();
		return FALSE;
	}
	...
	if (!pMotion)
	{
		LogBox("The file [%s] is not a normal motion", pThing->GetFileName());
		pThing->Release();
		return FALSE;
	}
