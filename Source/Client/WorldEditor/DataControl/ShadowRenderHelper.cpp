...
void CShadowRenderHelper::SetIntermediateShadowMapPower(uint8_t byShadowMapPower)
{
...
void CShadowRenderHelper::SetShadowMapPower(uint8_t byShadowMapPower)
{
...
bool CShadowRenderHelper::StartRenderingPhase(uint8_t byPhase)
{
	CTerrain * pTerrain;
	const TOutdoorMapCoordinate & rOutdoorMapCoordinate = m_pMapOutdoorAccessor->GetCurCoordinate();
	const uint8_t byNumTerrain = (m_wCurCoordY - rOutdoorMapCoordinate.m_sTerrainCoordY + 1) * 3 + (m_wCurCoordX - rOutdoorMapCoordinate.m_sTerrainCoordX + 1);
...
void CShadowRenderHelper::EndRenderingPhase(uint8_t byPhase)
{
	switch (byPhase)
	{
		case 1:
