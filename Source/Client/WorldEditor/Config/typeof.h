...
namespace msvc_typeof_impl {
	...
	*/
	...
	template<int32_t ID, typename T = msvc_extract_type_default_param> struct msvc_extract_type;

	template<int32_t ID> struct msvc_extract_type<ID, msvc_extract_type_default_param>
	{
	...
	template<int32_t ID, typename T> struct msvc_extract_type : msvc_extract_type<ID, msvc_extract_type_default_param>
	{
	...
	template<int32_t N> class CCounter;

	// TUnused is required to force compiler to recompile CCountOf class
	template<typename TUnused, int32_t NTested = 0> struct CCountOf
	{
		enum
		{
	...
	template<class TTypeReg, class TUnused, int32_t NValue> struct CProvideCounterValue { enum { value = NValue }; };
	...
	template<int32_t NSize>	class sized { char m_pad[NSize]; };
