...
bool CGuildMarkImage::Build(const char * c_szFileName)
{
	...
	{
		SysLog("CGuildMarkImage: cannot initialize image");
		return false;
	}
...
bool CGuildMarkImage::Load(const char * c_szFileName) 
{
	...
	if (!ilLoad(IL_TYPE_UNKNOWN, (const ILstring) c_szFileName))
	{
		...
		{
			SysLog("CGuildMarkImage: cannot open file for writing {}", c_szFileName);
			return false;
		}
	}
	...
	{
		SysLog("CGuildMarkImage: {} width must be {}", c_szFileName, WIDTH);
		return false;
	}

	if (ilGetInteger(IL_IMAGE_HEIGHT) != HEIGHT)
	{
		SysLog("CGuildMarkImage: {} height must be {}", c_szFileName, HEIGHT);
		return false;
	}
...
bool CGuildMarkImage::SaveMark(uint32_t posMark, uint8_t * pbImage)
{
	...
	{
		SysLog("CGuildMarkImage::CopyMarkFromData: Invalid mark position {}", posMark);
		return false;
	}
...
bool CGuildMarkImage::SaveBlockFromCompressedData(uint32_t posBlock, const uint8_t * pbComp, uint32_t dwCompSize)
{
	...
	if (LZO_E_OK != lzo1x_decompress_safe(pbComp, dwCompSize, reinterpret_cast<uint8_t *> (apxBuf), reinterpret_cast<lzo_uint*>(&sizeBuf), CLZO::GetInstance()->GetWorkMemory()))
	{
		SysLog("CGuildMarkImage::CopyBlockFromCompressedData: cannot decompress, compressed size = {}", dwCompSize);
		return false;
	}

	if (sizeBuf != sizeof(apxBuf))
	{
		SysLog("CGuildMarkImage::CopyBlockFromCompressedData: image corrupted, decompressed size = {}", sizeBuf);
		return false;
	}
...
{
	Pixel apxBuf[SGuildMarkBlock::SIZE];
	TraceLog("CGuildMarkImage::BuildAllBlocks");
...
void SGuildMarkBlock::Compress(const Pixel * pxBuf)
{
	...
	if (LZO_E_OK !=
		lzo1x_999_compress(reinterpret_cast<const uint8_t*>(pxBuf), sizeof(Pixel) * SIZE, m_abCompBuf,
			reinterpret_cast<lzo_uint*>(&m_sizeCompBuf), CLZO::GetInstance()->GetWorkMemory()))
	{
