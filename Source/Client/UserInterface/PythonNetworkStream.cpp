...
#include "../../Libraries/eterLib/NetPacketHeaderMap.h"
...
class CMainPacketHeaderMap : public CNetworkPacketHeaderMap
{
	...
	public:
		CMainPacketHeaderMap()
		{
			...
			Set(HEADER_GC_SAFEBOX_DEL,	CNetworkPacketHeaderMap::TPacketType(sizeof(TPacketGCSafeboxItemDel), STATIC_SIZE_PACKET));
...
UINT CPythonNetworkStream::UploadMark(const char * c_szImageFileName)
{
	...
	if (CGuildMarkManager::INVALID_MARK_ID == CGuildMarkManager::GetInstance()->GetMarkID(m_dwGuildID))
...
void CPythonNetworkStream::SetPhaseWindow(UINT ePhaseWnd, pybind11::handle poPhaseWnd)
{
...
void CPythonNetworkStream::ClearPhaseWindow(UINT ePhaseWnd, pybind11::handle poPhaseWnd)
{
	if (ePhaseWnd>=PHASE_WINDOW_NUM)
		return;

	if (!poPhaseWnd.is(m_apoPhaseWnd[ePhaseWnd]))
		return;

	m_apoPhaseWnd[ePhaseWnd] = pybind11::handle();;
}

void CPythonNetworkStream::SetServerCommandParserWindow(pybind11::handle poWnd)
{
...
bool CPythonNetworkStream::CheckPacket(TPacketHeader* pRetHeader)
{
	*pRetHeader = 0;
	...
	{
		TraceLog("Unknown packet header: {}, last: {} {}", header, g_iLastPacket[0], g_iLastPacket[1]);
		ClearRecvBuffer();
	...
	if (PacketType.isDynamicSizePacket)
	{
		...
		{
			TraceLog("CPythonNetworkStream::CheckPacket - Not enough dynamic packet size: header {} packet size: {}\n",
...
{
	TraceLog("Phase {} does not handle this header (header: {}, last: {}, {})",
...
{
	TraceLog("recv ping packet. (securitymode {})\n", IsSecurityMode());
...
//		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "StartGame");
	}
...
void CPythonNetworkStream::SetOffLinePhase()
{
	...
	TraceLog("## Network - OffLine Phase ##");
...
// Handler
void CPythonNetworkStream::SetHandler(pybind11::handle poHandler)
{
...
CPythonNetworkStream::CPythonNetworkStream()
{
	...
	m_strPhase = "OffLine";
	AbstractResources::GetInstance()->SetLoadingState(false);
	...
	m_poSerCommandParserWnd = pybind11::handle();
...
CPythonNetworkStream::~CPythonNetworkStream()
{
	TraceLog("PythonNetworkMainStream Clear");
}
