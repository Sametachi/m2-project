...
{
	PyCallClassMemberFunc(m_ppyGameWindow, "ClearAffects");
}

void CPythonPlayer::SetAffect(UINT uAffect)
{
	PyCallClassMemberFunc(m_ppyGameWindow, "SetAffect", uAffect);
...
void CPythonPlayer::ResetAffect(UINT uAffect)
{
	...
	PyCallClassMemberFunc(m_ppyGameWindow, "ResetAffect", uAffect);
...
void CPythonPlayer::ChangeCurrentSkillNumberOnly(uint32_t dwSlotIndex)
{
	...
	if (!__IsRightButtonSkillMode())
	{
		if (!__IsTarget())
		{
			PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotUseSkill", GetMainCharacterIndex(), "NEED_TARGET");
			return;
		}
	...
	else
	{
		...
		PyCallClassMemberFunc(m_ppyGameWindow, "ChangeCurrentSkill", dwSlotIndex);
	}
}
	...
	if (pSkillData->IsStandingSkill())
	{
		if (pSkillData->IsToggleSkill())
		{
			if (IsSkillActive(dwSlotIndex))
			{
				...
				CPythonNetworkStream::GetInstance()->SendUseSkillPacket(rkSkillInst.dwIndex);
				return;
			}
		}
...
bool CPythonPlayer::__CheckSkillUsable(uint32_t dwSlotIndex)
{
	...
	if (pkInstMain->IsMountingHorse())
	{
		...
		{
			PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotUseSkill", GetMainCharacterIndex(), "NOT_HORSE_SKILL");
			return false;
		}
	}
	...
	if (pSkillData->IsHorseSkill())
	{
		if (!pkInstMain->IsMountingHorse())
		{
			PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotUseSkill", GetMainCharacterIndex(), "HAVE_TO_RIDE");
			return false;
		}
	}
	...
	if (pSkillData->IsAttackSkill())
	{
		...
		{
			PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotUseSkill", GetMainCharacterIndex(), "IN_SAFE");
...
//		return false;
	...
	if (pSkillData->IsNeedEmptyBottle())
	{
		if (!__HasItem(27995))
		{
			PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotUseSkill", GetMainCharacterIndex(), "NEED_EMPTY_BOTTLE");
			return false;
		}
	}
	...
	if (pSkillData->IsNeedPoisonBottle())
	{
		if (!__HasItem(27996))
		{
			PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotUseSkill", GetMainCharacterIndex(), "NEED_POISON_BOTTLE");
			return false;
		}
	}
	...
	{
		PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotUseSkill", GetMainCharacterIndex(), "REMOVE_FISHING_ROD");
		return false;
	}
	...
	if (m_sysIsLevelLimit)
	{
		...
		{
			PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotUseSkill", GetMainCharacterIndex(), "NOT_YET_LEARN");
			return false;
		}
	}
	...
	{
		PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotUseSkill", GetMainCharacterIndex(), "NOT_MATCHABLE_WEAPON");
		return false;
	}
	...
	if (__CheckDashAffect(*pkInstMain))
	{
		if (!pSkillData->IsChargeSkill())
		{
			...
			{
				PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotUseSkill", GetMainCharacterIndex(), "WAIT_COOLTIME");
				return false;
			}
		}
	}
	else
	{
		...
		{
			PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotUseSkill", GetMainCharacterIndex(), "NOT_ENOUGH_HP");
			return false;
		}

		if (__CheckShortMana(rkSkillInst, *pSkillData))
		{
			PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotUseSkill", GetMainCharacterIndex(), "NOT_ENOUGH_SP");
			return false;
		}
	}
...
bool CPythonPlayer::__CheckShortMana(TSkillInstance& rkSkillInst, CPythonSkill::TSkillData& rkSkillData)
{
	...
	if (!rkSkillData.IsToggleSkill())
	{
		if (iNeedSP == 0)
		{
			CPythonChat::GetInstance()->AppendChat(CHAT_TYPE_INFO, "!!! Find strange game data. Please reinstall metin2.");
			return true;
		}
	}
...
bool CPythonPlayer::__CheckRestSkillCoolTime(uint32_t dwSlotIndex)
{
	...
	float fElapsedTime = CTimer::GetInstance()->GetCurrentSecond() - m_playerStatus.aSkill[dwSlotIndex].fLastUsedTime;
...
bool CPythonPlayer::__ProcessEnemySkillTargetRange(CInstanceBase& rkInstMain, CInstanceBase& rkInstTarget, CPythonSkill::TSkillData& rkSkillData, uint32_t dwSkillSlotIndex)
{
	...
	if (CRaceManager::GetInstance()->IsHugeRace(rkInstTarget.GetRace()))
	{
	...
	auto rkBG= GetBackgroundInstance();
	if (rkBG->IsBlock(kPPosTarget.x, kPPosTarget.y))
	{
		PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotUseSkill", GetMainCharacterIndex(), "CANNOT_APPROACH");
		return false;
	}
...
bool CPythonPlayer::__UseSkill(uint32_t dwSlotIndex)
{
	...
	if (dwSlotIndex >= SKILL_MAX_NUM)
	{
		TraceLog("CPythonPlayer::__UseSkill(dwSlotIndex={}) It's not available skill slot number", dwSlotIndex);
		return false;
	}
	...
	if (!CPythonSkill::GetInstance()->GetSkillData(rkSkillInst.dwIndex, &pSkillData))
	{
		TraceLog("CPythonPlayer::__UseSkill(dwSlotIndex={}) There is no skill data", dwSlotIndex);
		return false;
	}
	...
	if (pSkillData->IsToggleSkill())
	{
		if (IsSkillActive(dwSlotIndex))
		{
			CPythonNetworkStream::GetInstance()->SendUseSkillPacket(rkSkillInst.dwIndex, dwSlotIndex);
			return false;
		}
	}
	...
	if (!pkInstMain)
	{
		TraceLog("CPythonPlayer::__UseSkill(dwSlotIndex={}) There is no main player", dwSlotIndex);
		return false;
	}
		...
		pSkillData->IsAutoSearchTarget())
	{
		...
		if (pkInstTarget)
		{
			if (pSkillData->IsOnlyForAlliance())
			{
				if (pkInstMain == pkInstTarget)
				{
					...
					{
						PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotUseSkill", GetMainCharacterIndex(), "CANNOT_USE_SELF");
						return false;
					}
				}
				...
				else
				{
					if (pSkillData->CanUseForMe())
					{
						...
						TraceLog(" [ALERT] 동료에게 사용하는 기술임에도 적에게 타겟팅 되어있어서 자신에게로 재설정\n");
					}
					else
					{
						PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotUseSkill",GetMainCharacterIndex(), "ONLY_FOR_ALLIANCE");
						return false;
					}
				}
			}
			else
			{
				...
				{
					PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotUseSkill",  GetMainCharacterIndex(), "CANNOT_ATTACK_ENEMY_IN_SAFE_AREA");
					return false;
				}
				...
				{
					PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotUseSkill",  GetMainCharacterIndex(), "CANNOT_ATTACK");
					return false;
				}
			}
		...
		else
		{
			...
			if (pSkillData->CanUseForMe())
			{
				...
				TraceLog(" [ALERT] 타겟이 없어서 플레이어에게 사용합니다\n");
			}
			else if (pSkillData->IsNeedCorpse())
			{
				PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotUseSkill", GetMainCharacterIndex(), "ONLY_FOR_CORPSE");
				return false;
			}
			else
			{
	...
	if (pSkillData->CanChangeDirection())
	{
		...
		{
			TraceLog("CPythonPlayer::__UseSkill({})", dwSlotIndex);
		}
	}
	...
	if (dwTargetMaxCount>0 && pkInstTarget)
	{
			...
			if (pkInstMain->NEW_GetInstanceVectorInFanRange(float(dwRange), *pkInstTarget, &kVct_pkInstTarget))
			{
				...
				for (i=kVct_pkInstTarget.begin(); i!=kVct_pkInstTarget.end(); ++i)
				{
					...
					if (pkInstTarget!=pkInstEach && !pkInstEach->IsDead())
					{
						...
						CPythonNetworkStream::GetInstance()->SendAddFlyTargetingPacket(pkInstEach->GetVirtualID(), pkInstEach->GetGraphicThingInstanceRef().OnGetFlyTargetPosition());
		...
		if (dwTargetCount<dwTargetMaxCount)
		{
			while (dwTargetCount<dwTargetMaxCount)
			{
				...
				CPythonNetworkStream::GetInstance()->SendAddFlyTargetingPacket(0, kPPosDst);
	...
	if (!pSkillData->IsNoMotion())
	{
		...
		{
			TraceLog("CPythonPlayer::__UseSkill({}) - pkInstMain->NEW_UseSkill - ERROR", dwSlotIndex);
			return false;
		}
	}
...
uint8_t CPythonPlayer::__GetSkillType(uint32_t dwSkillSlotIndex)
{
	...
	auto rkPythonSkill = CPythonSkill::GetInstance();
...
void CPythonPlayer::__RunCoolTime(uint32_t dwSkillSlotIndex)
{
	...
	CPythonSkill::TSkillData * pkSkillData;
	if (!CPythonSkill::GetInstance()->GetSkillData(rkSkillInst.dwIndex, &pkSkillData))
	{
		TraceLog("CPythonPlayer::__SendUseSkill(dwSkillSlotIndex={}) - NOT CHECK", dwSkillSlotIndex);
		return;
	}
	...
	rkSkillInst.fLastUsedTime = CTimer::GetInstance()->GetCurrentSecond();
	...
	PyCallClassMemberFunc(m_ppyGameWindow, "RunUseSkillEvent", dwSkillSlotIndex, rkSkillInst.fCoolTime);
}

bool CPythonPlayer::__HasEnoughArrow()
{
	CItemData * pItemData;
	if (CItemManager::GetInstance()->GetItemDataPointer(GetItemIndex(TItemPos(INVENTORY, c_Equipment_Arrow)), &pItemData))
	if (ITEM::TYPE_WEAPON == pItemData->GetType())
	if (ITEM::WEAPON_ARROW == pItemData->GetSubType())
	{
		return true;
	}
...
void CPythonPlayer::UseGuildSkill(uint32_t dwSkillSlotIndex)
{
	...
	{
		TraceLog("CPythonPlayer::UseGuildSkill(dwSkillSlotIndex={}) It's not available skill slot number", dwSkillSlotIndex);
		return;
	}
	...
	{
		if (!CPythonGuild::GetInstance()->IsDoingGuildWar())
		{
			PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotUseSkill", GetMainCharacterIndex(), "ONLY_FOR_GUILD_WAR");
			return;
		}
	}
	...
	{
		TraceLog("CPythonPlayer::UseGuildSkill({}) - pkInstMain->NEW_UseSkill - ERROR", dwSkillIndex);
	}

	///////////////////////////////////////////////////////////////////////////////////////////////

	CPythonNetworkStream::GetInstance()->SendGuildUseSkillPacket(dwSkillIndex, 0);
	__RunCoolTime(dwSkillSlotIndex);
}
...
void CPythonPlayer::SetComboSkillFlag(BOOL bFlag)
{
	...
	{
		TraceLog("CPythonPlayer::SetComboSkillFlag(killIndex={}) - Can't Find Slot Index\n", c_iSkillIndex_Combo);
		return;
	}
	...
	if (iLevel <= 0)
	{
		TraceLog("CPythonPlayer::SetComboSkillFlag(skillIndex={}, skillLevel={}) - Invalid Combo Skill Level\n", c_iSkillIndex_Combo, iLevel);
		return;
	}
...
bool CPythonPlayer::__CheckSpecialSkill(uint32_t dwSkillIndex)
{
	...
	if (c_iSkillIndex_Fishing == dwSkillIndex)
	{
		...
		{
			PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotUseSkill", GetMainCharacterIndex(), "EQUIP_FISHING_ROD");
		}
		return true;
	}
	...
	else if (c_iSkillIndex_Combo == dwSkillIndex)
	{
		...
		if (iLevel > 0)
		{
			CPythonNetworkStream::GetInstance()->SendUseSkillPacket(dwSkillIndex);
		}
		else
		{
			PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotUseSkill", GetMainCharacterIndex(), "NOT_YET_LEARN");
