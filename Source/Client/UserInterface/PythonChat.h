...
class CWhisper
{
	public:
		typedef struct SChatLine
		{
			...
			void				Render(RECT* pRect);

			void				SetPosition(int32_t iX, int32_t iY);
...
class CPythonChat : public Singleton<CPythonChat>
{
	public:
		enum EWhisperType
		{
		...
		typedef struct SChatLine
		{
			...
			static CDynamicPool<SChatLine> ms_kPool;
			int32_t					m_iType;
			float					m_fAppendedTime;
			D3DXCOLOR				m_aColor[CHAT_LINE_COLOR_ARRAY_MAX_NUM];
			CGraphicTextInstance	m_Instance;
			int32_t					m_iX, m_iY;
		} TChatLine;
	...
	public:
		...
		void InitWhisper(pybind11::handle ppyObject);
