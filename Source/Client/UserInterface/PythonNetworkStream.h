...
class CPythonNetworkStream : public CNetworkStream, public Singleton<CPythonNetworkStream>
{
	public:
		enum
		{
	...
	public:
		...
		bool SendSpecial(int32_t nLen, void * pvBuf);
		...
		void SetPhaseWindow(UINT ePhaseWnd, pybind11::handle poPhaseWnd);
		void ClearPhaseWindow(UINT ePhaseWnd, pybind11::handle poPhaseWnd);
		void SetServerCommandParserWindow(pybind11::handle poPhaseWnd);

		bool SendSyncPositionElementPacket(uint32_t dwVictimVID, uint32_t dwVictimX, uint32_t dwVictimY);

		bool SendAttackPacket(UINT uMotAttack, uint32_t dwVIDVictim);
		...
		void SendDataToPython(const char* funcname, Args&& ...args)
		{
			PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_LOGIN], funcname, std::forward<Args>(args)...);
		}
	...
	protected:
		...
		pybind11::handle							m_apoPhaseWnd[PHASE_WINDOW_NUM];
		pybind11::handle							m_poSerCommandParserWnd;
