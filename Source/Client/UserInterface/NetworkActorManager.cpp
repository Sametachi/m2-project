...
void CNetworkActorManager::__OLD_Update()
{
	...
	auto rkChrMgr=__GetCharacterManager();
...
{
	return CPythonCharacterManager::GetInstance();
}
...
{
	TraceLog("CNetworkActorManager::__AppendCharacterManagerActor({}, {})", rkNetActorData.m_dwVID, rkNetActorData.m_stName.c_str());
...
void CNetworkActorManager::AppendActor(const SNetworkActorData& c_rkNetActorData)
{
	if (__IsMainActorVID(c_rkNetActorData.m_dwVID))
	{
		...
		CInstanceBase* pMainInstance = CPythonCharacterManager::GetInstance()->GetInstancePtr(c_rkNetActorData.m_dwVID);
		if( pMainInstance )
		{
...
void CNetworkActorManager::RemoveActor(uint32_t dwVID)
{
	...
	{	
		ConsoleLog("CNetworkActorManager::RemoveActor(dwVID={}) - NOT EXIST VID", dwVID);
		return;
	}
...
void CNetworkActorManager::UpdateActor(const SNetworkUpdateActorData& c_rkNetUpdateActorData)
{
	...
	if (m_kNetActorDict.end()==f)
	{
		ConsoleLog("CNetworkActorManager::UpdateActor(dwVID={}) - NOT EXIST VID", c_rkNetUpdateActorData.m_dwVID);
		return;
	}
...
void CNetworkActorManager::MoveActor(const SNetworkMoveActorData& c_rkNetMoveActorData)
{
	...
	if (m_kNetActorDict.end()==f)
	{
		ConsoleLog("CNetworkActorManager::MoveActor(dwVID={}) - NOT EXIST VID", c_rkNetMoveActorData.m_dwVID);
		return;
	}
...
void CNetworkActorManager::SyncActor(uint32_t dwVID, int32_t lPosX, int32_t lPosY)
{
	...
	if (m_kNetActorDict.end()==f)
	{
		ConsoleLog("CNetworkActorManager::SyncActor(dwVID={}) - NOT EXIST VID", dwVID);
		return;
	}
...
void CNetworkActorManager::SetActorOwner(uint32_t dwOwnerVID, uint32_t dwVictimVID)
{
	...
	if (m_kNetActorDict.end()==f)
	{
		ConsoleLog("CNetworkActorManager::SetActorOwner(dwOwnerVID={}, dwVictimVID={}) - NOT EXIST VID", dwOwnerVID, dwVictimVID);
		return;
	}
