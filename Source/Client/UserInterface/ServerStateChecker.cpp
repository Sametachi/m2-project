...
void CServerStateChecker::Create(pybind11::handle poWnd)
{
	m_poWnd = poWnd;
}
...
void CServerStateChecker::Request()
{
	...
	if (!m_kStream.Connect(m_lstChannel.begin()->c_szAddr, m_lstChannel.begin()->uPort))
	{
		for (std::list<TChannel>::const_iterator it = m_lstChannel.begin(); it != m_lstChannel.end(); ++it)
			PyCallClassMemberFunc(m_poWnd, "NotifyChannelState", it->uServerIndex, 0);
		return;
	}
...
void CServerStateChecker::Update()
{
	...
	for (int32_t i = 0; i < nSize; i++)
	{
		...
		for (std::list<TChannel>::const_iterator it = m_lstChannel.begin(); it != m_lstChannel.end(); ++it)
		{
			if (std::cmp_equal(channelStatus.nPort, it->uPort))
			{
				PyCallClassMemberFunc(m_poWnd, "NotifyChannelState", it->uServerIndex, channelStatus.bStatus);
				break;
			}
		}
	}
