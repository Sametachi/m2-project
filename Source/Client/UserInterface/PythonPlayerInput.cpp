...
void CPythonPlayer::__ClearTarget()
{
	...
	CPythonNetworkStream::GetInstance()->SendTargetPacket(0);
}
...
void CPythonPlayer::SetTarget(uint32_t dwVID, BOOL bForceChange)
{
	...
	CInstanceBase * pkInstTarget = CPythonCharacterManager::GetInstance()->GetInstancePtr(dwVID);
	if (pkInstTarget)
	{
		if (pkInstMain->IsTargetableInstance(*pkInstTarget))
		{
			...
			CPythonNetworkStream::GetInstance()->SendTargetPacket(dwVID);
...
{
	uint32_t dwVID;
	if (!CPythonCharacterManager::GetInstance()->OLD_GetPickedInstanceVID(&dwVID))
		return false;
...
{
	CInstanceBase * pkInstMain = CPythonPlayer::GetInstance()->NEW_GetMainActorPtr();
	if (!pkInstMain)
		return;

	CInstanceBase * pkInstTarget = CPythonCharacterManager::GetInstance()->GetInstancePtr(dwVictimActorID);
	if (!pkInstTarget)
		return;
...
//		return;
	...
	PyCallClassMemberFunc(m_ppyGameWindow, "SetPCTargetBoard", pkInstTarget->GetVirtualID(), pkInstTarget->GetNameString());
}
...
void CPythonPlayer::__OnPressItem(CInstanceBase& rkInstMain, uint32_t dwPickedItemID)
{
	...
	{
		TraceLog("CPythonPlayer::__OnPressItem - ALREADY PICKED ITEM");
		return;
	}
...
bool CPythonPlayer::NEW_MoveToDirection(float fDirRot)
{
	...
	if (pkCmrCur)
	{
		...
		if (m_isCmrRot)
		{
			...
			float fElapsedTime = CPythonApplication::GetInstance()->GetGlobalElapsedTime();
...
bool CPythonPlayer::NEW_CancelFishing()
{
	...
	if (pkInstMain->IsFishing())
	{
		...
		if (CTimer::GetInstance()->GetCurrentMillisecond() < s_dwLastCancelTime + 500)
			return false;

		CPythonNetworkStream::GetInstance()->SendFishingPacket(0);
		s_dwLastCancelTime = CTimer::GetInstance()->GetCurrentMillisecond();
		return true;
	}
	...
	else
	{
		if (pkInstMain->CanFishing())
		{
			...
			if (pkInstMain->GetFishingRot(&irot))
				CPythonNetworkStream::GetInstance()->SendFishingPacket(irot);
			else
			{
...
bool CPythonPlayer::__CanShot(CInstanceBase& rkInstMain, CInstanceBase& rkInstTarget)
{
	if (!__HasEnoughArrow())
	{
		PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotShot", GetMainCharacterIndex(), "EMPTY_ARROW");
		return false;
	}

	if (rkInstMain.IsInSafe())
	{
		PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotShot",  GetMainCharacterIndex(), "IN_SAFE");
		return false;
	}

	if (rkInstTarget.IsInSafe())
	{
		PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotShot",GetMainCharacterIndex(), "DEST_IN_SAFE");
		return false;
	}	
...
{
	int32_t lWidth = CWindowManager::GetInstance()->GetScreenWidth();
	int32_t lHeight = CWindowManager::GetInstance()->GetScreenHeight();
...
void CPythonPlayer::__ReserveProcess_ClickActor()
{
	...
	{
		PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotAttack",  GetMainCharacterIndex(), "IN_SAFE");
		pkInstMain->NEW_Stop();
		__ClearReservedAction();
		return;
	}
	...
	{
		PyCallClassMemberFunc(m_ppyGameWindow, "OnCannotAttack", GetMainCharacterIndex(), "DEST_IN_SAFE");
		pkInstMain->NEW_Stop();
		__ClearReservedAction();
		return;
	}
