...
void CPythonApplication::__UpdateCamera()
{
	//////////////////////
	// Camera Setting
	CCamera * pMainCamera = CCameraManager::GetInstance()->GetCurrentCamera();
	if (!pMainCamera)
		return;
...
{
	return CCameraManager::GetInstance()->GetCurrentCamera()->GetRoll();
}

float CPythonApplication::GetPitch()
{
	return CCameraManager::GetInstance()->GetCurrentCamera()->GetPitch();
}
...
{
	if (CCameraManager::DEFAULT_PERSPECTIVE_CAMERA == CCameraManager::GetInstance()->GetCurrentCameraNum())
	{
	...
	CCameraManager::GetInstance()->SetCurrentCamera(EVENT_CAMERA_NUMBER);
	CCamera * pCamera = CCameraManager::GetInstance()->GetCurrentCamera();
	if (!pCamera)
		return;
...
void CPythonApplication::BlendEventCamera(const SCameraSetting & c_rCameraSetting, float fBlendTime)
{
	...
	m_fBlendCameraStartTime = CTimer::GetInstance()->GetCurrentSecond();
...
void CPythonApplication::GetCameraSetting(SCameraSetting * pCameraSetting)
{
	...
	if (CCameraManager::GetInstance()->GetCurrentCamera())
		pCameraSetting->v3CenterPosition.z += CCameraManager::GetInstance()->GetCurrentCamera()->GetTargetHeight();
