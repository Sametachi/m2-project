...
bool CGuildMarkManager::SaveMarkIndex()
{
	...
	if (!fp)
	{
		SysLog("MarkManager::SaveMarkData: cannot open index file.");
		return false;
	}
	...
	fclose(fp);
	TraceLog("MarkManager::SaveMarkData: index count %d", m_mapGID_MarkID.size());
	return true;
}
...
uint32_t CGuildMarkManager::SaveMark(uint32_t guildID, uint8_t * pbMarkImage)
{
	...
	if ((idMark = GetMarkID(guildID)) == INVALID_MARK_ID)
	{
		if ((idMark = __AllocMarkID(guildID)) == INVALID_MARK_ID)
		{
			...
			TraceLog("SaveMark: mark id alloc {}", idMark);
	}
	else
		TraceLog("SaveMark: mark id found {}", idMark);
...
void CGuildMarkManager::SaveSymbol(const char* filename)
{
	...
	if (!fp)
	{
		TraceLog("Cannot open Symbol file (name: {})", filename);
		return;
	}
...
{
	TraceLog("GuildSymbolUpload guildID {} Size {}", guildID, iSize);
