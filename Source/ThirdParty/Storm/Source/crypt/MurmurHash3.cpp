...
BOOST_FORCEINLINE uint64_t rotl64 ( uint64_t x, int8_t r )
{
  return (x << r) | (x >> (64 - r));
}
...
BOOST_FORCEINLINE uint64_t getblock(const uint64_t * p, int i)
{
  return p[i];
}
...
BOOST_FORCEINLINE uint64_t fmix(uint64_t k)
{
  k ^= k >> 33;
