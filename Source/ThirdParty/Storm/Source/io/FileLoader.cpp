...
void FileLoader::Load(const StringRef& filename,
                      bsys::error_code& ec,
                      vstd::size_type alignment,
                      uint32_t flags)
{
	STORM_ASSERT(!m_isLoaded, "Load() can only be called on an empty FileLoader");
	...
	if (!ec) {
		...
		uint8_t* ptr = AllocateArray<uint8_t>(m_allocator, size + 1, flags);
