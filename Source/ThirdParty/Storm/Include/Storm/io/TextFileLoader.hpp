...
#ifndef STORM_IO_TEXTFILELOADER_HPP
#define STORM_IO_TEXTFILELOADER_HPP
...
/// @file io/TextFileLoader.hpp
/// Defines the TextFileLoader class.
///
...
/// A simple class representing a text file loaded into memory.
///
/// The TextFileLoader class loads a complete text file into memory,
/// in such a way, that the text data is guranteed to be NUL-terminated and
/// contiguous.
///
/// @par Example
/// @code
/// boost::system::error_code ec;
/// TextFileLoader loader(m_allocator);
///
/// loader.Load("hello_world.txt", ec);
/// if (loader.IsLoaded()) {
///     printf("File size is %u, content ptr is %p",
///            loader.GetSize(), loader.GetText());
/// }
/// @endcode
///
/// @note The TextFileLoader's buffer is guranteed to be at least as big as
/// the file plus one byte (terminating NUL).
///
/// @note All memory allocated by the TextFileLoader class is guranteed to be
/// flagged as temporary.
class TextFileLoader
{
	public:
		/// Construct a new TextFileLoader
		///
		/// This constructor initializes the TextFileLoader with
		/// @c allocator.
		///
		/// @param allocator The allocator which should be used to allocate
		/// the memory needed for storing the file's data
		TextFileLoader(const Allocator& allocator);

		/// Destruct a TextFileLoader object
		///
		/// This destructor frees all the allocated memory using
		/// the TextFileLoader's allocator, if necessary.
		~TextFileLoader();
		...
		/// allocator.
		///
		/// @warning This function expects an empty TextFileLoader object
		/// (i.e. no file loaded).
...
/**/
BOOST_FORCEINLINE TextFileLoader::TextFileLoader(const Allocator& allocator)
...
BOOST_FORCEINLINE TextFileLoader::~TextFileLoader()
{
	m_allocator.Deallocate(m_data);
}

BOOST_FORCEINLINE char* TextFileLoader::GetText()
{
	return m_data;
}

BOOST_FORCEINLINE const char* TextFileLoader::GetText() const
{
	return m_data;
}

BOOST_FORCEINLINE vstd::size_type TextFileLoader::GetSize() const
{
	return m_size;
}

BOOST_FORCEINLINE bool TextFileLoader::IsLoaded() const
{
	return m_isLoaded;
}
