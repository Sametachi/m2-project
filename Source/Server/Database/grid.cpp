...
CGrid::CGrid(CGrid* pGrid, int32_t w, int32_t h) : m_iWidth(w), m_iHeight(h)
{
	m_pGrid = new char[m_iWidth * m_iHeight];
	int32_t iSize = std::MIN(w * h, pGrid->m_iWidth* pGrid->m_iHeight);
	memcpy(m_pGrid, pGrid->m_pGrid, sizeof(char) * iSize);
}
