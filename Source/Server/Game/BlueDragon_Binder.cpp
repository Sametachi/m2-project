...
{
	lua_State* L = quest::CQuestManager::GetInstance()->GetLuaState();
	...
	for(size_t i=0 ; i < cnt ; ++i)
	{
		...
		if (!key)
		{
			va_end(vl);
			lua_settop(L, stack_top);
			SysLog("BlueDragon: wrong key list");
			return 0;
		}
		...
		if (!lua_istable(L, -1) && i != cnt-1)
		{
			va_end(vl);
			lua_settop(L, stack_top);
			SysLog("BlueDragon: wrong key table {}", key);
			return 0;
		}
	}
	...
	{
		lua_settop(L, stack_top);
		SysLog("BlueDragon: Last key is not a number");
		return 0;
	}
...
uint32_t BlueDragon_GetRangeFactor(const char* key, const int32_t val)
{
	...
	if (!lua_istable(L, -1))
	{
		...
		SysLog("BlueDragon: no required table {}", key);
		return 0;
	}
	...
	for(size_t i=1 ; i <= cnt ; ++i)
	{
		...
		if (!lua_istable(L, -1))
		{
			...
			SysLog("BlueDragon: wrong table index {} {}", key, i);
			return 0;
		}
		...
		if (!lua_isnumber(L, -1))
		{
			...
			SysLog("BlueDragon: no min value set {}", key);
			return 0;
		}
		...
		if (!lua_isnumber(L, -1))
		{
			...
			SysLog("BlueDragon: no max value set {}", key);
			return 0;
		}
		...
		if (min <= val && val <= max)
		{
			...
			if (!lua_isnumber(L, -1))
			{
				...
				SysLog("BlueDragon: no pct value set {}", key);
				return 0;
			}
...
uint32_t BlueDragon_GetIndexFactor(const char* container, const size_t idx, const char* key)
{
	...
	if (!lua_istable(L, -1))
	{
		...
		SysLog("BlueDragon: wrong table index {} {}", key, idx);
		return 0;
	}
