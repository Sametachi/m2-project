...
namespace quest
{
	int32_t marriage_engage_to(lua_State* L)
	{
		...
		LPCHARACTER ch_you = CHARACTER_MANAGER::GetInstance()->Find(vid);
		if (ch_you)
		{
			marriage::CManager::GetInstance()->RequestAdd(ch->GetPlayerID(), ch_you->GetPlayerID(), ch->GetName(), ch_you->GetName());
		}
		return 0;
	}
	...
	{
		LPCHARACTER ch = CQuestManager::GetInstance()->GetCurrentCharacterPtr();
		marriage::TMarriage* pMarriage = marriage::CManager::GetInstance()->Get(ch->GetPlayerID());
		if (!pMarriage)
		{
			SysLog("pid[{}:{}] is not exist couple", ch->GetPlayerID(), ch->GetName());
			return 0;
		}
		marriage::CManager::GetInstance()->RequestRemove(ch->GetPlayerID(), pMarriage->GetOther(ch->GetPlayerID()));
		return 0;
	}
	...
	int32_t marriage_find_married_vid(lua_State* L)
	{
		...
		if (pMarriage)
		{
			LPCHARACTER you = CHARACTER_MANAGER::GetInstance()->FindByPID(pMarriage->GetOther(ch->GetPlayerID()));
			if (you)
				vid = you->GetVID();
		}
	...
	{
		marriage::CManager::GetInstance()->for_each_wedding(FBuildLuaWeddingMapList(L));
		return 1;
	}
	...
	int32_t marriage_join_wedding(lua_State* L)
	{
		...
		{
			SysLog("invalid player id for wedding map");
			return 0;
		}
		...
		marriage::TMarriage* pMarriage = marriage::CManager::GetInstance()->Get(pid1);
		if (!pMarriage)
		{
			SysLog("pid[{}:{}] is not exist couple", ch->GetPlayerID(), ch->GetName());
			return 0;
	...
	int32_t marriage_wedding_dark(lua_State* L)
	{
		...
		{
			SysLog("invalid argument 1 : must be boolean");
			return 0;
		}
		LPCHARACTER ch = CQuestManager::GetInstance()->GetCurrentCharacterPtr();
		marriage::TMarriage* pMarriage = marriage::CManager::GetInstance()->Get(ch->GetPlayerID());
		...
		if (pMarriage->pWeddingInfo)
		{
			marriage::WeddingMap* pWedding = marriage::WeddingManager::GetInstance()->Find(pMarriage->pWeddingInfo->dwMapIndex);
	...
	int32_t marriage_wedding_client_command(lua_State* L)
	{
		if (!lua_isstring(L, 1))
		{
			SysLog("invalid argument 1 : must be string");
			return 0;
		}
	...
	int32_t marriage_wedding_music(lua_State* L)
	{
		...
		if (!lua_isstring(L, 2))
		{
			SysLog("invalid argument 2 : must be string");
			return 0;
		}
	...
	int32_t marriage_get_married_time(lua_State* L)
	{
		...
		if (!pMarriage)
		{
			SysLog("trying to get time for not married character");
			lua_pushnumber(L, 0);
			return 1;
		}
	...
	void RegisterMarriageFunctionTable()
	{
		...
		CQuestManager::GetInstance()->AddLuaFunctionTable("marriage", marriage_functions);
	}
}
