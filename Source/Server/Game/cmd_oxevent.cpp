...
ACMD(do_oxevent_show_quiz)
{
	...
	COXEventManager::GetInstance()->ShowQuizList(ch);
	ch->ChatPacket(CHAT_TYPE_INFO, "===== OX QUIZ LIST END =====");
}

ACMD(do_oxevent_log)
{
	if (COXEventManager::GetInstance()->LogWinner() == false)
	{
...
{
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Number of other participants : %d"), COXEventManager::GetInstance()->GetAttenderCount());
}
