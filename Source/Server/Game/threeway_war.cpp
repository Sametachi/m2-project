...
EVENTFUNC(regen_mob_event)
{
	...
	if (info == nullptr)
	{
		SysLog("regen_mob_event> <Factor> Null pointer");
		return 0;
	}
	...
	int32_t choice = quest::CQuestManager::GetInstance()->GetEventFlag("threeway_war_choice");
	if (0 == choice)
	...
	{
		if (0 != choice)
			if (regen_load_in_file(filename, iMapIndex, pMap->m_setting.iBaseX, pMap->m_setting.iBaseY))
			{
				sprintf (filename, "%sregen00.txt", GetSungziMapPath());
				regen_load_in_file(filename, iMapIndex, pMap->m_setting.iBaseX, pMap->m_setting.iBaseY);
			}
	}
...
int32_t CThreeWayWar::GetKillScore(uint8_t empire) const
{
	...
	{
		SysLog("ThreeWayWar::GetKillScore Wrong Empire variable");
		return 0;
	}
...
void CThreeWayWar::SetKillScore(uint8_t empire, int32_t count)
{
	if(empire <= 0 || empire >= EMPIRE_MAX_NUM)
	{
		SysLog("ThreeWayWar::SetKillScore Wrong Empire variable");
		return;
	}
...
bool CThreeWayWar::LoadSetting(const char* szFileName)
{
	...
	if (!pf)
	{
		SysLog("[INIT_FORKED] Do not open file ({})", szPath);
		return false;
	}
...
{
	const size_t idx = quest::CQuestManager::GetInstance()->GetEventFlag("threeway_war_pass_idx");
...
{
	const size_t idx = quest::CQuestManager::GetInstance()->GetEventFlag("threeway_war_sungzi_idx");
...
void CThreeWayWar::RandomEventMapSet()
{
	...
	quest::CQuestManager::GetInstance()->RequestSetEventFlag("threeway_war_sungzi_idx", sung_idx);
	quest::CQuestManager::GetInstance()->RequestSetEventFlag("threeway_war_pass_idx", pass_idx);
}
...
void CThreeWayWar::onDead(LPCHARACTER pChar, LPCHARACTER pKiller)
{
	if (!pChar->IsPC())
		return;
	...
	if (!IsSungZiMapIndex(pChar->GetMapIndex()))
		return;

	if (!pKiller || true != pKiller->IsPC())
		return;

	if (pChar->GetEmpire() == pKiller->GetEmpire())
		return;

	int32_t nKillScore = GetKillScore(pKiller->GetEmpire());

	if (nKillScore >= 0)
	{
		nKillScore += GetKillValue(pChar->GetLevel());
		SetKillScore(pKiller->GetEmpire(), nKillScore);
	}
	...
	const int32_t nVictoryScore = quest::CQuestManager::GetInstance()->GetEventFlag("threeway_war_kill_count");
	...
	if (0 == GetRegenFlag())
	{
		...
		CHARACTER_MANAGER::GetInstance()->SendScriptToMap(pChar->GetMapIndex(), Script);
		...
		LogManager::GetInstance()->CharLog(0, 0, 0, 0, "THREEWAY", szNotice, NULL);
	...
	else if (1 == GetRegenFlag())
	{
		...
		if (NULL != pSecMap)
		{
			...
			LogManager::GetInstance()->CharLog(0, 0, 0, 0, "THREEWAY", szBuf, NULL);
		}

		for (int32_t n = 0; n < quest::CQuestManager::GetInstance()->GetEventFlag("threeway_war_boss_count");)
		{
			...
			LPCHARACTER ch = CHARACTER_MANAGER::GetInstance()->SpawnMob(
...
void CThreeWayWar::RemoveAllMonstersInThreeWay() const
{
	...
	{
		LPSECTREE_MAP pSecMap = SECTREE_MANAGER::GetInstance()->GetMap(*iter);
...
const char* GetSungziMapPath()
{
	...
	snprintf(s_szMapPath, sizeof(s_szMapPath), "%s/map/%s/",
		   	...
		   	CThreeWayWar::GetInstance()->GetEventSungZiMapInfo().m_stMapName.c_str());
...
const char* GetPassMapPath(uint8_t bEmpire)
{
	if (bEmpire > 0 && bEmpire < EMPIRE_MAX_NUM)
	{
		...
		snprintf(s_szMapPath, sizeof(s_szMapPath), "%s/map/%s/",
				LocaleService_GetBasePath().c_str(),
				CThreeWayWar::GetInstance()->GetEventPassMapInfo().m_stMapName[bEmpire-1].c_str());
...
int32_t GetPassMapIndex(uint8_t bEmpire)
{
	if (bEmpire > 0 && bEmpire < EMPIRE_MAX_NUM)
	{
		return CThreeWayWar::GetInstance()->GetEventPassMapInfo().m_iForkedPass[bEmpire-1];
	}
...
int32_t GetPassStartX(uint8_t bEmpire)
{
	if (bEmpire > 0 && bEmpire < EMPIRE_MAX_NUM)
	{
		return CThreeWayWar::GetInstance()->GetEventPassMapInfo().m_iForkedPassStartPosition[bEmpire-1][0];
	}
...
int32_t GetPassStartY(uint8_t bEmpire) 
{
	if (bEmpire > 0 && bEmpire < EMPIRE_MAX_NUM)
	{
		return CThreeWayWar::GetInstance()->GetEventPassMapInfo().m_iForkedPassStartPosition[bEmpire-1][1];
	}
...
{
	return CThreeWayWar::GetInstance()->GetEventSungZiMapInfo().m_iForkedSung;
}
...
int32_t GetSungziStartX(uint8_t bEmpire)
{
	if (bEmpire > 0 && bEmpire < EMPIRE_MAX_NUM)
	{
		return CThreeWayWar::GetInstance()->GetEventSungZiMapInfo().m_iForkedSungziStartPosition[bEmpire-1][0];
	}
...
int32_t GetSungziStartY(uint8_t bEmpire)
{
	if (bEmpire > 0 && bEmpire < EMPIRE_MAX_NUM)
	{
		return CThreeWayWar::GetInstance()->GetEventSungZiMapInfo().m_iForkedSungziStartPosition[bEmpire-1][1];
	}
