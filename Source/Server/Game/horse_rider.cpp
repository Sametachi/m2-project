...
EVENTFUNC(horse_stamina_consume_event)
{
	...
	if (info == nullptr)
	{
		SysLog("horse_stamina_consume_event> <Factor> Null pointer");
		return 0;
	}
...
EVENTFUNC(horse_stamina_regen_event)
{
	...
	if (info == nullptr)
	{
		SysLog("horse_stamina_regen_event> <Factor> Null pointer");
		return 0;
	}
...
void CHorseRider::StartStaminaConsumeEvent()
{
	...
	PyLog("HORSE STAMINA REGEN EVENT CANCEL");
	event_cancel(&m_eventStaminaRegen);
	...
	PyLog("HORSE STAMINA CONSUME EVENT CREATE");
}
...
void CHorseRider::StartStaminaRegenEvent()
{
	...
	PyLog("HORSE STAMINA CONSUME EVENT CANCEL");
	event_cancel(&m_eventStaminaConsume);
	...
	PyLog("HORSE STAMINA REGEN EVENT CREATE");
}
