	...
	void operator()(LPENTITY ent)
	{
		if (NULL != ent)
		{
			if (ent->IsType(ENTITY_CHARACTER))
			{
				...
				{
					if (NULL != ch->FindAffect(AFFECT_REVIVE_INVISIBLE, ITEM::APPLY_NONE))
						return;

					if ((signed)BlueDragon_GetSkillFactor(2, "Skill0", "damage_area") < DISTANCE_APPROX(pAttacker->GetX()-ch->GetX(), pAttacker->GetY()-ch->GetY()))
					{
						PyLog("BlueDragon: Breath too far ({})", DISTANCE_APPROX(pAttacker->GetX()-ch->GetX(), pAttacker->GetY()-ch->GetY()));
						return;
					}
					...
					PyLog("BlueDragon: Breath to {} pct({}) dam({}) overlap({})", ch->GetName(), pct, dam, overlapDamageCount);
				}
			}
		}
	}
		...
		if (NULL != ent)
		{
			if (ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (ch->IsPC() && !ch->IsDead())
				{
					...
					PyLog("BlueDragon: WeakBreath to {} addPct({}) dam({})", ch->GetName(), addPct, dam);
				}
			}
		}
	}
		...
		if (NULL != ent)
		{
			if (ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (ch->IsPC() && !ch->IsDead())
				{
					...
					SkillAttackAffect(ch, 1000, ITEM::IMMUNE_STUN, AFFECT_STUN, POINT_NONE, 0, AFF_STUN, sec, "BDRAGON_STUN");

					PyLog("BlueDragon: EarthQuake to {} addPct({}) dam({}) sec({})", ch->GetName(), addPct, dam, sec);
					...
					{
						if (SECTREE_MANAGER::GetInstance()->IsMovablePosition(ch->GetMapIndex(), tx, ty))
						{
							break;
						}
