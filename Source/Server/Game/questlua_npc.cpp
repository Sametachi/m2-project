...
namespace quest
{
	...
	int32_t npc_open_shop(lua_State * L)
	{
		...
		if (CQuestManager::GetInstance()->GetCurrentNPCCharacterPtr())
			CShopManager::GetInstance()->StartShopping(CQuestManager::GetInstance()->GetCurrentCharacterPtr(), CQuestManager::GetInstance()->GetCurrentNPCCharacterPtr(), iShopVnum);
		return 0;
	}
	...
	{
		lua_pushnumber(L, CQuestManager::GetInstance()->GetCurrentNPCRace());
		return 1;
	}
	...
	int32_t npc_is_near_vid(lua_State* L)
	{
		...
		auto q = CQuestManager::GetInstance();
		LPCHARACTER ch = CHARACTER_MANAGER::GetInstance()->Find((uint32_t)lua_tonumber(L, 1));
		LPCHARACTER npc = q->GetCurrentNPCCharacterPtr();
	...
	int32_t npc_get_vid_attack_mul(lua_State* L)
	{
		...
		LPCHARACTER targetChar = CHARACTER_MANAGER::GetInstance()->Find(vid);
	...
	void RegisterNPCFunctionTable()
	{
		...
		CQuestManager::GetInstance()->AddLuaFunctionTable("npc", npc_functions);
	}
};
