...
namespace marriage
{
	...
	int32_t TMarriage::GetMarriagePoint()
	{
		if (test_server)
		{
			int32_t value = quest::CQuestManager::GetInstance()->GetEventFlag("lovepoint");
			if (value)
	...
	void TMarriage::Login(LPCHARACTER ch)
	{
		...
		if (is_married)
		{
			...
			if (!d1)
			{
				pCCI = P2P_MANAGER::GetInstance()->FindByPID(m_pid1);

				if (pCCI)
				{
					d1 = pCCI->pDesc;
					d1->SetRelay(pCCI->szName);
				}
			}
			...
			if (!d2)
			{
				pCCI = P2P_MANAGER::GetInstance()->FindByPID(m_pid2);

				if (pCCI)
				{
					d2 = pCCI->pDesc;
					d2->SetRelay(pCCI->szName);
				}
			}
			...
			if (d1 && d2)
			{
				...
				PyLog("lover_login {} {}", m_pid1, m_pid2);
			}
		}
	}
	...
	void TMarriage::NearCheck()
	{
		...
		PyLog("NearCheck {} {} {} {} {}", m_pid1, m_pid2, IsNear(), isLastNear, byLastLovePoint, GetMarriagePoint());
	...
	EVENTFUNC(near_check_event)
	{
		...
		if (info == nullptr)
		{
			SysLog("near_check_event> <Factor> Null pointer");
			return 0;
		}
	...
	void TMarriage::Save()
	{
		PyLog("TMarriage::Save() - RequestUpdate.bSave={}", bSave);
		if (bSave)
		{
			CManager::GetInstance()->RequestUpdate(m_pid1, m_pid2, love_point, is_married);
			bSave = false;
		}
	}
	...
	void TMarriage::Update(uint32_t point)
	{
		...
		if (point > 0 && is_married)
		{
			...
			{
				LPCHARACTER ch;
				ch = CHARACTER_MANAGER::GetInstance()->FindByPID(m_pid1);
				if (ch)
					ch->ChatPacket(CHAT_TYPE_PARTY, "lovepoint bykill %.3g total %d", love_point / 1000000., GetMarriagePoint());
				ch = CHARACTER_MANAGER::GetInstance()->FindByPID(m_pid2);
				if (ch)
					ch->ChatPacket(CHAT_TYPE_PARTY, "lovepoint bykill %.3g total %d", love_point / 1000000., GetMarriagePoint());
			}
		}
	...
	void TMarriage::WarpToWeddingMap(uint32_t dwPID)
	{
		...
		{
			PIXEL_POSITION pos;
			if (!SECTREE_MANAGER::GetInstance()->GetRecallPositionByEmpire(pWeddingInfo->dwMapIndex/10000, 0, pos))
			{
				SysLog("cannot get warp position");
				return;
			}
			ch->SaveExitLocation();
	...
	void TMarriage::RequestEndWedding()
	{
		if (!pWeddingInfo)
			return;
		CManager::GetInstance()->RequestEndWedding(m_pid1, m_pid2);
	}
	...
	{ 
		TMarriage* pMarriageFinded=Get(dwPlayerID);
		if (pMarriageFinded && pMarriageFinded->is_married)
			return true;
	...
	void CManager::Add(uint32_t dwPID1, uint32_t dwPID2, time_t tMarryTime, const char* szName1, const char* szName2)
	{
		...
		{
			SysLog("cannot marry already married character. {} - {}", dwPID1, dwPID2);
			return;
		}
		...
		{
			LPCHARACTER A = CHARACTER_MANAGER::GetInstance()->FindByPID(dwPID1);
			LPCHARACTER B = CHARACTER_MANAGER::GetInstance()->FindByPID(dwPID2);
	...
	void CManager::Update(uint32_t dwPID1, uint32_t dwPID2, int32_t lTotalPoint, uint8_t byMarried)
	{
		...
		{
			SysLog("not under marriage : {} {}", dwPID1, dwPID2);
			return;
		}
	...
	void CManager::WeddingReady(uint32_t dwPID1, uint32_t dwPID2, uint32_t dwMapIndex)
	{
		...
		if (!pMarriage || pMarriage->GetOther(dwPID1) != dwPID2)
		{
			SysLog("wrong marriage {}, {}", dwPID1, dwPID2);
			return;
		}
	...
	void CManager::WeddingEnd(uint32_t dwPID1, uint32_t dwPID2)
	{
		...
		if (!pMarriage->pWeddingInfo)
		{
			SysLog("not under wedding {}, {}", dwPID1, dwPID2);
			return;
		}

		if (map_allow_find(WEDDING_MAP_INDEX))
			if (!WeddingManager::GetInstance()->End(pMarriage->pWeddingInfo->dwMapIndex))
			{
				SysLog("wedding map error: map_index={}", pMarriage->pWeddingInfo->dwMapIndex);
				return;
			}
