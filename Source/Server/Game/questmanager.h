...
namespace quest
{
	...
	class CQuestManager : public Singleton<CQuestManager>
	{
		...
		public:
			...
			//void		SetCurrentNPCCharacterPtr(LPCHARACTER ch) { m_pCurrentNPC = ch; }
			...
			bool		Click(uint32_t pc, LPCHARACTER pNPC);
			...
			bool		GiveItemToPC(uint32_t pc, LPCHARACTER pChr);
		...
		private:
			...
			typedef boost::unordered_map<std::string, int32_t, stringhash> THashMapQuestName;
			typedef boost::unordered_map<uint32_t, std::vector<char> > THashMapQuestStartScript;
