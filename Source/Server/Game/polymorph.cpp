...
bool CPolymorphUtils::PolymorphCharacter(LPCHARACTER pChar, LPITEM pItem, const CMob* pMob)
{
	...
	// Transformation Probability = Character Level - Mob Level + Disguise Level + 29 + Disguise Skill Level
	...
	// Transformation bonus = disguise skill level + disguise book level
...
bool CPolymorphUtils::GiveBook(LPCHARACTER pChar, uint32_t dwMobVnum, uint32_t dwPracticeCount, uint8_t BookLevel, uint8_t LevelLimit)
{
	...
	if (CMobManager::GetInstance()->Get(dwMobVnum) == nullptr)
	{
		SysLog("Wrong Polymorph vnum passed: CPolymorphUtils::GiveBook(PID({}), {} {} {} {})", 
