...
namespace building
{
	...
	class CLand
	{
		public:
			...
			void	InsertObject(LPOBJECT pObj);
		...
		protected:
			TLand			m_data;
			std::map<uint32_t, LPOBJECT>	m_map_pObject;
			std::map<uint32_t, LPOBJECT>	m_map_pObjectByVID;
	...
	class CManager : public Singleton<CManager>
	{
		public:
			...
			void	DeleteObject(uint32_t dwID);
			void	UnregisterObject(LPOBJECT pObj);
		...
		protected:
			...
			std::map<uint32_t, TObjectProto *>	m_map_pObjectProto;

			std::map<uint32_t, CLand *>		m_map_pLand;
			std::map<uint32_t, LPOBJECT>		m_map_pObjByID;
			std::map<uint32_t, LPOBJECT>		m_map_pObjByVID;
	};
}
