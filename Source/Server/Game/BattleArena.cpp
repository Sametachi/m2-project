...
EVENTFUNC(battle_arena_event)
{
	...
	if (pInfo != nullptr)
	{
		switch (pInfo->state)
		{
			...
			case 2:
				{
					...
					quest::CQuestManager::GetInstance()->RequestSetEventFlag("battle_arena", 0);
					// BroadcastNotice(LC_TEXT("몬스터들이 성을 공격하기 시작했습니다."));

					LPSECTREE_MAP sectree = SECTREE_MANAGER::GetInstance()->GetMap(pInfo->nMapIndex);
			...
			case 3 :
				{
					if (SECTREE_MANAGER::GetInstance()->GetMonsterCountInMap(pInfo->nMapIndex) <= 0)
					{
						pInfo->state = 6;
					...
					else
					{
						...
						{
							CBattleArena::GetInstance()->SpawnRandomStone();
						}
					}
				}
			...
			case 4 :
				{
					...
					SECTREE_MANAGER::GetInstance()->PurgeMonstersInMap(pInfo->nMapIndex);
				}
			...
			case 6 :
				{
					...
					CBattleArena::GetInstance()->SpawnLastBoss();
				}
				return test_server ? PASSES_PER_SEC(60) : PASSES_PER_SEC(60*5);
			...
			case 7 :
				{
					...
					if (pInfo->wait_count >= 6)
					{
						...
						SECTREE_MANAGER::GetInstance()->PurgeMonstersInMap(pInfo->nMapIndex);
						SECTREE_MANAGER::GetInstance()->PurgeStonesInMap(pInfo->nMapIndex);
...
bool CBattleArena::Start(int32_t nEmpire)
{
	...
	quest::CQuestManager::GetInstance()->RequestSetEventFlag("battle_arena", m_nMapIndex);
...
void CBattleArena::SpawnRandomStone()
{
	...
	CHARACTER_MANAGER::GetInstance()->SpawnMobRange(
		   	vnum[number(0, 6)],
		   	m_nMapIndex,
...
void CBattleArena::SpawnLastBoss()
{
	...
	CHARACTER_MANAGER::GetInstance()->SpawnMob(vnum, m_nMapIndex, position[idx][0], position[idx][1], 0);
}
