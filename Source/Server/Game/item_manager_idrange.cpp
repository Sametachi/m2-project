...
uint32_t ITEM_MANAGER::GetNewID()
{
	...
	if (m_dwCurrentID >= m_ItemIDRange.dwMax)
	{
		...
		{
			for (int32_t i=0; i < 10; i++) SysLog("ItemIDRange: FATAL ERROR!!! no more item id");
			touch(".killscript");
			thecore_shutdown();
			return 0;
		}
		...
		{
			PyLog("ItemIDRange: First Range is full. Change to SpareRange {} ~ {} {}",
...
bool ITEM_MANAGER::SetMaxItemID(TItemIDRangeTable range)
{
	...
	{
		for (int32_t i=0; i < 10; i++) SysLog("ItemIDRange: FATAL ERROR!!! ITEM ID RANGE is not set.");
		touch(".killscript");
		thecore_shutdown();
		return false;
	}
	...
	PyLog("ItemIDRange: {} ~ {} {}", m_ItemIDRange.dwMin, m_ItemIDRange.dwMax, m_dwCurrentID);
...
bool ITEM_MANAGER::SetMaxSpareItemID(TItemIDRangeTable range)
{
	...
	{
		for (int32_t i=0; i < 10; i++) SysLog("ItemIDRange: FATAL ERROR!!! Spare ITEM ID RANGE is not set");
		return false;
	}

	m_ItemIDSpareRange = range;

	PyLog("ItemIDRange: New Spare ItemID Range Recv {} ~ {} {}",
			m_ItemIDSpareRange.dwMin, m_ItemIDSpareRange.dwMax, m_ItemIDSpareRange.dwUsableItemIDMin);
