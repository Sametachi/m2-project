...
struct command_info cmd_info[] =
{
	...
	{ "mob_ld",		do_mob_ld,			0,			POS_DEAD,	GM_HIGH_WIZARD	}, /* Set the mob's location and direction to summon it /mob_ld vnum x y dir */
	...
	{ "pkmode",		do_pmode,		0,			POS_DEAD,	GM_PLAYER	},
	...
	{ "\n",		NULL,			0,			POS_DEAD,	GM_IMPLEMENTOR	}  /* This must be the last one. */
};
...
void interpreter_set_privilege(const char* cmd, int32_t lvl)
{
	...
	for (i = 0; *cmd_info[i].command != '\n'; ++i)
	{
		if (!str_cmp(cmd, cmd_info[i].command))
		{
			...
			PyLog("Setting command privilege: {} -> {}", cmd, lvl);
			break;
		}
	}
}
...
void interpret_command(LPCHARACTER ch, const char* argument, size_t len)
{
	if (!ch)
	{
		SysLog("NULL CHRACTER");
		return ;
	}

	char cmd[128 + 1];  // The length is intentionally shortened to avoid buffer overflow problems.
	char new_line[256 + 1];
	const char* line;
	int32_t icmd;
	...
	for (icmd = 1; *cmd_info[icmd].command != '\n'; ++icmd)
	{
		...
		{
			if (!strcmp(cmd_info[icmd].command, cmd)) // do_cmd may have to run every command.
				break;
		}
	...
	if (ch->GetPosition() < cmd_info[icmd].minimum_position)
	{
		switch (ch->GetPosition())
		{
			...
			case POS_SITTING:
				 ...
				 */
			default:
				SysLog("unknown position {}", ch->GetPosition());
				break;
		}
	...
	if (strncmp("phase", cmd_info[icmd].command, 5) != 0) // Hidden command processing
		PyLog("COMMAND: {}: {}", ch->GetName(), cmd_info[icmd].command);
	...
	if (ch->GetGMLevel() >= GM_LOW_WIZARD)
	{
		if (cmd_info[icmd].gm_level >= GM_LOW_WIZARD)
		{
			...
			LogManager::GetInstance()->GMCommandLog(ch->GetPlayerID(), ch->GetName(), ch->GetDesc()->GetHostName(), g_bChannel, buf);
		}
	}
}
