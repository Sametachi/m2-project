...
void CBuffOnAttributes::RemoveBuffFromItem(LPITEM pItem)
{
	...
	if (NULL != pItem)
	{
		...
		for (int32_t j = 0; j < m; j++)
		{
			...
			// m_map_additional_attrs Remove the value for the corresponding attribute type from
			// Buff effect reduced by (m_bBuffValue)% of the changed value
			...
			{
				SysLog("Buff ERROR(type {}). This item({}) attr_type({}) was not in buff pool", m_bPointType, pItem->GetVnum(), attr.bType);
				return;
			}
		}
	}
...
void CBuffOnAttributes::AddBuffFromItem(LPITEM pItem)
{
	...
	if (NULL != pItem)
	{
		...
		for (int32_t j = 0; j < m; j++)
		{
			...
			// If there is no value for the corresponding attribute type in m_map_additional_attrs, add it.
			// Add a buff effect equal to (m_bBuffValue)% of the added value
			if (it == m_map_additional_attrs.end())
			{
			...
			// If there is a value for the corresponding attribute type in m_map_additional_attrs, increase the value,
			// Add a buff effect equal to (m_bBuffValue)% of the changed value
			else
			{
				int32_t& sum_of_attr_value = it->second;
				int32_t old_value = sum_of_attr_value * m_bBuffValue / 100;
...
void CBuffOnAttributes::ChangeBuffValue(uint8_t bNewValue)
{
	...
	{
		// Previously, (m_bBuffValue)% of the value of m_map_additional_attrs was included as a buff,
		// Change the value by (bNewValue)%.
