...
void DeleteMarkImage(CGuildMarkImage* pImage)
{
	M2_DELETE(pImage);
}
...
{
	PyLog("GuildMarkImage: creating new file {}", c_szFileName);
	...
	{
		SysLog("GuildMarkImage: cannot initialize image");
		return false;
	}
...
bool CGuildMarkImage::Load(const char* c_szFileName) 
{
	...
	{
		SysLog("GuildMarkImage: {} cannot open file.", c_szFileName);
		return false;
	}

	if (ilGetInteger(IL_IMAGE_WIDTH) != WIDTH)	
	{
		SysLog("GuildMarkImage: {} width must be {}", c_szFileName, WIDTH);
		return false;
	}

	if (ilGetInteger(IL_IMAGE_HEIGHT) != HEIGHT)
	{
		SysLog("GuildMarkImage: {} height must be {}", c_szFileName, HEIGHT);
		return false;
	}
...
bool CGuildMarkImage::SaveMark(uint32_t posMark, uint8_t* pbImage)
{
	...
	{
		SysLog("GuildMarkImage::CopyMarkFromData: Invalid mark position {}", posMark);
		return false;
	}
...
bool CGuildMarkImage::SaveBlockFromCompressedData(uint32_t posBlock, const uint8_t* pbComp, uint32_t dwCompSize)
{
	...
	if (LZO_E_OK != lzo1x_decompress_safe(pbComp, dwCompSize, (uint8_t*) apxBuf, &sizeBuf, CLZO::GetInstance()->GetWorkMemory()))
	{
		SysLog("GuildMarkImage::CopyBlockFromCompressedData: cannot decompress, compressed size = {}", dwCompSize);
		return false;
	}

	if (sizeBuf != sizeof(apxBuf))
	{
		SysLog("GuildMarkImage::CopyBlockFromCompressedData: image corrupted, decompressed size = {}", sizeBuf);
		return false;
	}
...
{
	Pixel apxBuf[SGuildMarkBlock::SIZE];
	PyLog("GuildMarkImage::BuildAllBlocks");
...
void SGuildMarkBlock::Compress(const Pixel* pxBuf)
{
	...
	if (LZO_E_OK != lzo1x_1_compress((const uint8_t*) pxBuf, sizeof(Pixel) * SGuildMarkBlock::SIZE, m_abCompBuf, &m_sizeCompBuf, CLZO::GetInstance()->GetWorkMemory()))
	{
		SysLog("SGuildMarkBlock::Compress: Error! {} > {}", sizeof(Pixel) * SGuildMarkBlock::SIZE, m_sizeCompBuf);
		return;
	}
