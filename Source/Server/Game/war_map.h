...
class CWarMap
{
	public:
		...
		CWarMap(int32_t lMapIndex, const TGuildWarInfo& r_info, TWarMapInfo* pWarMapInfo, uint32_t dwGuildID1, uint32_t dwGuildID2);
		~CWarMap();
		...
		void	SetBeginEvent(LPEVENT pEv);
		void	SetTimeoutEvent(LPEVENT pEv);
		void	SetEndEvent(LPEVENT pEv);
		void	SetResetFlagEvent(LPEVENT pEv);
	...
	private:
		...
		LPEVENT m_pBeginEvent;
		LPEVENT m_pTimeoutEvent;
		LPEVENT m_pEndEvent;
		LPEVENT	m_pResetFlagEvent;
		...
		typedef struct STeamData
		{
			...
			int32_t		iScore;
			LPCHARACTER pChrFlag;
			LPCHARACTER pChrFlagBase;
