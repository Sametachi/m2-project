...
void DESC_P2P::Destroy()
{
	...
	P2P_MANAGER::GetInstance()->UnregisterAcceptor(this);

	fdwatch_del_fd(m_lpFdw, m_sock);

	PyLog("SYSTEM: closing p2p socket. DESC #{}", m_sock);
...
bool DESC_P2P::Setup(LPFDWATCH fdw, socket_t fd, const char* host, uint16_t wPort)
{
	...
	if (strcmp(host, g_szPublicIP)) // Refuse if remote host != g_szPublicIP (only the same machine must be able to connect in here)
	{
		PyLog("SYSTEM: new p2p connection from [{}] to [{}] fd: {} BLOCKED", host, g_szPublicIP, m_sock);
	...
	PyLog("SYSTEM: new p2p connection from [{}] fd: {}", host, m_sock);
	return true;
}
...
void DESC_P2P::SetPhase(int32_t iPhase)
{
	...
	switch (iPhase)
	{
		...
		default:
			SysLog("DESC_P2P::SetPhase : Unknown phase");
			break;
	}
