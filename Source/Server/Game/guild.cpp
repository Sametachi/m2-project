...
CGuild::CGuild(TGuildCreateParameter & cp)
{
	...
	std::unique_ptr<SQLMsg> pmsg (DBManager::GetInstance()->DirectQuery(
	...
	{
		DBManager::GetInstance()->Query("INSERT INTO guild_grade%s VALUES(%u, %d, '%s', %d)", 
				get_table_postfix(),
				m_data.guild_id, 
				i + 1, 
	...
	CHARACTER_MANAGER::GetInstance()->for_each_pc(FGuildNameSender(GetID(), GetName()));
...
void CGuild::RequestAddMember(LPCHARACTER ch, int32_t grade)
{
	...
	{
		SysLog("Already a member in guild {}[{}]", ch->GetName(), ch->GetPlayerID());
		return;
	}
...
void CGuild::AddMember(TPacketDGGuildMember* p)
{
	...
	CGuildManager::GetInstance()->Link(p->dwPID, this);

	SendListOneToAll(p->dwPID);

	LPCHARACTER ch = CHARACTER_MANAGER::GetInstance()->FindByPID(p->dwPID);

	TraceLog("GUILD: AddMember PID {}, grade {}, job {}, level {}, offer {}, name {}",
...
{
	PyLog("Receive Guild P2P RemoveMember");
	TGuildMemberContainer::iterator it;
	...
	CGuildManager::GetInstance()->Unlink(pid);
...
void CGuild::P2PLoginMember(uint32_t pid)
{
	...
	{
		SysLog("GUILD [{}] is not a memeber of guild.", pid);
		return;
	}
...
void CGuild::LoginMember(LPCHARACTER ch)
{
	if (m_member.find(ch->GetPlayerID()) == m_member.end())
	{
		SysLog("GUILD {}[{}] is not a memeber of guild.", ch->GetName(), ch->GetPlayerID());
		return;
	}
...
void CGuild::SendListPacket(LPCHARACTER ch)
{
	/*
	   ...
	   [
	   ...
	   name_flag 1 - Send your name or not
	...
	for (TGuildMemberContainer::iterator it = m_member.begin(); it != m_member.end(); ++it)
	{
		...
		if (test_server)
			PyLog("name {} job {}  ", it->second.name.c_str(), it->second.job);
	}
...
void CGuild::LoadGuildMemberData(SQLMsg* pmsg)
{
	...
	for (uint i = 0; i < pmsg->Get()->uiNumRows; ++i)
	{
		...
		CGuildManager::GetInstance()->Link(pid, this);
	}
}
...
void CGuild::LoadGuildData(SQLMsg* pmsg)
{
	if (pmsg->Get()->uiNumRows == 0)
	{
		SysLog("Query failed: getting guild data {}", pmsg->stQuery.c_str());
		return;
	}
...
void CGuild::Load(uint32_t guild_id)
{
	...
	DBManager::GetInstance()->FuncQuery(std::bind(&CGuild::LoadGuildData, this, std::placeholders::_1),
	...
	PyLog("GUILD: loading guild id %12s {}", m_data.name, guild_id);

	DBManager::GetInstance()->FuncQuery(std::bind(&CGuild::LoadGuildGradeData, this, std::placeholders::_1),
			"SELECT grade, name, auth+0 FROM guild_grade%s WHERE guild_id = %u", get_table_postfix(), m_data.guild_id);

	DBManager::GetInstance()->FuncQuery(std::bind(&CGuild::LoadGuildMemberData, this, std::placeholders::_1),
...
{
	DBManager::GetInstance()->Query("UPDATE guild%s SET level=%d, exp=%u, skill_point=%d WHERE id = %u", get_table_postfix(), m_data.level,m_data.exp, m_data.skill_point,m_data.guild_id);
}
...
void CGuild::SaveSkill()
{
	...
	DBManager::GetInstance()->EscapeString(text, sizeof(text), (const char* ) m_data.abySkill, sizeof(m_data.abySkill));
	DBManager::GetInstance()->Query("UPDATE guild%s SET sp = %d, skill_point=%d, skill='%s' WHERE id = %u",
...
{
	DBManager::GetInstance()->FuncQuery(std::bind(&CGuild::__P2PUpdateGrade, this, std::placeholders::_1),
...
void CGuild::ChangeGradeName(uint8_t grade, const char* grade_name)
{
	...
	if (grade < 1 || grade > 15)
	{
		SysLog("Wrong guild grade value {}", grade);
		return;
	}
	...
	DBManager::GetInstance()->EscapeString(text, sizeof(text), grade_name, strlen(grade_name));
	DBManager::GetInstance()->FuncAfterQuery(FSendChangeGrade(GetID(), grade), "UPDATE guild_grade%s SET name = '%s' where guild_id = %u and grade = %d", get_table_postfix(), text, m_data.guild_id, grade);
...
void CGuild::ChangeGradeAuth(uint8_t grade, uint8_t auth)
{
	...
	DBManager::GetInstance()->FuncAfterQuery(FSendChangeGrade(GetID(),grade), "UPDATE guild_grade%s SET auth = %d where guild_id = %u and grade = %d", get_table_postfix(), auth, m_data.guild_id, grade);
...
void CGuild::SendGuildInfoPacket(LPCHARACTER ch)
{
	...
	PyLog("GMC guild_name {}", m_data.name);
	PyLog("GMC master {}", m_data.master_pid);
...
bool CGuild::OfferExp(LPCHARACTER ch, int32_t amount)
{
	...
	{
		SysLog("Wrong guild offer amount {} by {}[{}]", amount, ch->GetName(), ch->GetPlayerID());
		return false;
	}
...
void CGuild::Disband()
{
	PyLog("GUILD: Disband {}:{}", GetName(), GetID());
	...
	for (TGuildMemberContainer::iterator it = m_member.begin(); it != m_member.end(); ++it)
	{
		CGuildManager::GetInstance()->Unlink(it->first);
	}
...
void CGuild::RequestDisband(uint32_t pid)
{
	...
	building::CManager::GetInstance()->ClearLandByGuildID(GetID());
}
...
void CGuild::DeleteComment(LPCHARACTER ch, uint32_t comment_id)
{
	...
	if (GetMember(ch->GetPlayerID())->grade == GUILD_LEADER_GRADE)
		pmsg = DBManager::GetInstance()->DirectQuery("DELETE FROM guild_comment%s WHERE id = %u AND guild_id = %u",get_table_postfix(), comment_id, m_data.guild_id);
	else
		pmsg = DBManager::GetInstance()->DirectQuery("DELETE FROM guild_comment%s WHERE id = %u AND guild_id = %u AND name = '%s'",get_table_postfix(), comment_id, m_data.guild_id, ch->GetName());
...
{
	LPCHARACTER ch = CHARACTER_MANAGER::GetInstance()->FindByPID(player_id);
	if (ch == nullptr) {
		return;
	}

	std::unique_ptr<SQLMsg> pmsg (DBManager::GetInstance()->DirectQuery("SELECT id, name, content FROM guild_comment%s WHERE guild_id = %u ORDER BY notice DESC, id DESC LIMIT %d", get_table_postfix(), m_data.guild_id, GUILD_COMMENT_MAX_COUNT));
...
void CGuild::SkillLevelUp(uint32_t dwVnum)
{
	...
	if (!pSk)
	{
		SysLog("There is no such guild skill by number {}", dwVnum);
		return;
	}

	if (m_data.abySkill[dwRealVnum] >= pSk->bMaxLevel)
		return;
	...
	PyLog("Guild SkillUp: {} {} level {} type {}", GetName(), pSk->dwVnum, m_data.abySkill[dwRealVnum], pSk->dwType);
}
...
void CGuild::UseSkill(uint32_t dwVnum, LPCHARACTER ch, uint32_t pid)
{
	...
	PyLog("GUILD_USE_SKILL : cname({}), skill({})", ch ? ch->GetName() : "", dwVnum);
	...
	if ((pSk->dwFlag & SKILL_FLAG_SELFONLY))
	{
		if (ch->FindAffect(pSk->dwVnum))
			return;
	...
	double k =1.0*m_data.abySkill[dwRealVnum]/pSk->bMaxLevel;
	pSk->kSPCostPoly.SetVar("k", k);
	int32_t iNeededSP = (int32_t) pSk->kSPCostPoly.Eval();
	...
	switch (dwVnum)
	{
		case GUILD_SKILL_TELEPORT:
			...
			if ((victim = (CHARACTER_MANAGER::GetInstance()->FindByPID(pid))))
				ch->WarpSet(victim->GetX(), victim->GetY());
			else
			{
				if (m_memberP2POnline.find(pid) != m_memberP2POnline.end())
				{
					CCI* pcci = P2P_MANAGER::GetInstance()->FindByPID(pid);
			...
			/*case GUILD_SKILL_ACCEL:
			  ...
			  ch->AddAffect(dwVnum, POINT_MOV_SPEED, m_data.abySkill[dwRealVnum]*3, pSk->dwAffectFlag, (int32_t)pkSk->kDurationPoly.Eval(), 0, false);
			  ch->AddAffect(dwVnum, POINT_ATT_SPEED, m_data.abySkill[dwRealVnum]*3, pSk->dwAffectFlag, (int32_t)pkSk->kDurationPoly.Eval(), 0, false);
			  break;*/
...
void CGuild::Chat(const char* c_pszText)
{
	...
	P2P_MANAGER::GetInstance()->Send(&p1, sizeof(TPacketGGGuild));
	P2P_MANAGER::GetInstance()->Send(&p2, sizeof(TPacketGGGuildChat));
}

LPCHARACTER CGuild::GetMasterCharacter()
{ 
	return CHARACTER_MANAGER::GetInstance()->FindByPID(GetMasterPID()); 
}
...
bool CGuild::ChargeSP(LPCHARACTER ch, int32_t iSP)
{
	...
	DBManager::GetInstance()->SendMoneyLog(MONEY_LOG_GUILD, 1, -gold);
...
void CGuild::SkillUsableChange(uint32_t dwSkillVnum, bool bUsable)
{
	...
	PyLog("CGuild::SkillUsableChange(guild={}, skill={}, usable={})", GetName(), dwSkillVnum, bUsable);
}

void CGuild::SetMemberCountBonus(int32_t iBonus)
{
	m_iMemberCountBonus = iBonus;
	PyLog("GUILD_IS_FULL_BUG : Bonus set to {}(val:{})", iBonus, m_iMemberCountBonus);
}
...
void CGuild::BroadcastMemberCountBonus()
{
	...
	P2P_MANAGER::GetInstance()->Send(&p1, sizeof(TPacketGGGuild));
	P2P_MANAGER::GetInstance()->Send(&m_iMemberCountBonus, sizeof(int32_t));
}
...
void CGuild::RequestDepositMoney(LPCHARACTER ch, int32_t iGold)
{
	...
	LogManager::GetInstance()->CharLog(ch, iGold, "GUILD_DEPOSIT", buf);

	ch->UpdateDepositPulse();
	PyLog("GUILD: DEPOSIT {}:{} player {}[{}] gold {}", GetName(), GetID(), ch->GetName(), ch->GetPlayerID(), iGold);
}
...
void CGuild::RecvWithdrawMoneyGive(int32_t iChangeGold)
{
	...
	if (ch)
	{
		...
		PyLog("GUILD: WITHDRAW {}:{} player {}[{}] gold {}", GetName(), GetID(), ch->GetName(), ch->GetPlayerID(), iChangeGold);
	}
...
bool CGuild::HasLand()
{
	return building::CManager::GetInstance()->FindLandByGuild(GetID()) != nullptr;
}
...
EVENTFUNC(GuildInviteEvent)
{
	...
	if (pInfo == nullptr)
	{
		SysLog("GuildInviteEvent> <Factor> Null pointer");
		return 0;
	}

	CGuild* pGuild = CGuildManager::GetInstance()->FindGuild(pInfo->dwGuildID);

	if (pGuild) 
	{
		PyLog("GuildInviteEvent {}", pGuild->GetName());
...
{
	if (quest::CQuestManager::GetInstance()->GetPCForce(pchInviter->GetPlayerID())->IsRunning())
	{
	    pchInviter->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("[Guild] The person standing opposite you cannot receive the invitation."));
	    return;
	}

	
	if (quest::CQuestManager::GetInstance()->GetPCForce(pchInvitee->GetPlayerID())->IsRunning())
		return;
	...
	switch (errcode) 
	{
		...
		case GERR_GUILD_IS_IN_WAR : pchInviter->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("[Guild] This guild is taking part in a battle at the moment.")); return;
		case GERR_INVITE_LIMIT : pchInviter->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("[Guild] The maximum guild capacity has been reached.")); return;

		default: SysLog("ignore guild join error({})", errcode); return;
	}
...
void CGuild::InviteAccept(LPCHARACTER pchInvitee)
{
	...
	{
		PyLog("GuildInviteAccept from not invited character(invite guild: {}, invitee: {})", GetName(), pchInvitee->GetName());
		return;
	}
	...
	switch (errcode) 
	{
		...
		case GERR_GUILD_IS_IN_WAR : pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("[Guild] This guild is taking part in a battle at the moment.")); return;
		case GERR_INVITE_LIMIT : pchInvitee->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("[Guild] The maximum guild capacity has been reached.")); return;
...
void CGuild::InviteDeny(uint32_t dwPID)
{
	...
	if (itFind == m_GuildInviteEventMap.end()) 
	{
		PyLog("GuildInviteDeny from not invited character(invite guild: {}, invitee PID: {})", GetName(), dwPID);
		return;
	}
...
CGuild::GuildJoinErrCode CGuild::VerifyGuildJoinableCondition(const LPCHARACTER pchInvitee)
{
	...
	{
		TraceLog("GuildName = {}, GetMemberCount() = {}, GetMaxMemberCount() = {} (32 + MAX(level({})-10, 0) * 2 + bonus({})", 
