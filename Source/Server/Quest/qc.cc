...
void parse(char* filename)
{
...

	{
		...
		for (it = state_arg_script_map.begin(); it != state_arg_script_map.end(); ++it)
		{
			...
			{
				for (std::vector<AScript>::size_type i = 0; i < it2->second.size(); ++i)
				{
					std::ostringstream os;
					os << i;
					{
...

	{
		...
		for (it = state_script_map.begin();it!=state_script_map.end();++it)
		{
			...
			for (it2 = it->second.begin();it2!=it->second.end();++it2)
			{
				...
				std::copy(it2->second.begin(),it2->second.end(), std::ostreambuf_iterator<char>(ouf));
			}
		}
	}
